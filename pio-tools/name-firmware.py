...
def bin_map_copy(source, target, env):
    ...
    map_file = os.path.normpath(str(Infinitekpiolib.get_final_map_path(env)))
    bin_file = os.path.normpath(str(Infinitekpiolib.get_final_bin_path(env)))
    ...
    if env["PIOPLATFORM"] == "espressif32":
        if("safeboot" in firmware_name):
            ...
            if SAFEBOOT_SIZE > 851967:
                print(Fore.RED + "!!! Infinitek safeboot size is too big with {} bytes. Max size is 851967 bytes !!! ".format(
                        SAFEBOOT_SIZE
                    )
                )
    ...
    if env["PIOPLATFORM"] == "espressif32":
        ...
        shutil.copy(os.path.normpath(str(Infinitekpiolib.get_source_map_path(env))), map_file)
        if("safeboot" not in firmware_name):
            shutil.copy(factory, one_bin_file)
    else:
        map_firm = os.path.normpath(join(env.subst("$BUILD_DIR"), "firmware.map"))
        shutil.copy(os.path.normpath(str(Infinitekpiolib.get_source_map_path(env))), map_firm)
        shutil.move(os.path.normpath(str(Infinitekpiolib.get_source_map_path(env))), map_file)
