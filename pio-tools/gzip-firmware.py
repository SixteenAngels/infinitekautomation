...
def map_gzip(source, target, env):
    ...
    map_file = pathlib.Path(Infinitekpiolib.get_final_map_path(env))
...
if not Infinitekpiolib.is_env_set(Infinitekpiolib.DISABLE_MAP_GZ, env):
...
if Infinitekpiolib.is_env_set(Infinitekpiolib.ENABLE_ESP32_GZ, env) or env["PIOPLATFORM"] != "espressif32":
    import time
    ...
    def bin_gzip(source, target, env):
        # create string with location and file names based on variant
        bin_file = pathlib.Path(Infinitekpiolib.get_final_bin_path(env))
        ...
        with bin_file.open("rb") as fp:
            with gzip_file.open("wb") as f:
                ...
                gz = Infinitekpiolib.compress(fp.read(), gzip_level)
        ...
        if ORG_FIRMWARE_SIZE > 995326 and env["PIOPLATFORM"] != "espressif32":
            print(Fore.RED + "!!! Infinitek firmware size is too big with {} bytes. Max size is 995326 bytes !!! ".format(
                    ORG_FIRMWARE_SIZE
                )
            )
        else:
    ...
    if not Infinitekpiolib.is_env_set(Infinitekpiolib.DISABLE_BIN_GZ, env):
