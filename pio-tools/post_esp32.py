...
# -  0x8000 | ~\Infinitek\.pio\build\<env name>\partitions.bin
# -  0xe000 | ~\.platformio\packages\framework-arduinoespressif32\tools\partitions\boot_app0.bin
# - 0x10000 | ~\Infinitek\<variants_dir>/<env name>-safeboot.bin
# - 0xe0000 | ~\Infinitek\.pio\build\<env name>/firmware.bin
# - 0x3b0000| ~\Infinitek\.pio\build\<env name>/littlefs.bin
...
if github_actions and os.path.exists(os.path.normpath(os.path.join(".", "firmware", "firmware"))):
    dest_dir = os.path.normpath(os.path.join(os.sep, "home", "runner", ".platformio", "packages", "framework-arduinoespressif32", "variants", "Infinitek"))
...
if not variants_dir:
    variants_dir = os.path.normpath(join(FRAMEWORK_DIR, "variants", "Infinitek"))
...
def esp32_create_chip_string(chip):
    Infinitek_platform_org = env.subst("$BUILD_DIR").split(os.path.sep)[-1]
    Infinitek_platform = Infinitek_platform_org.split('-')[0]
    if "Infinitek" + chip[3:] not in Infinitek_platform: # check + fix for a valid name like 'Infinitek' + '32c3'
        Infinitek_platform = "Infinitek" + chip[3:]
        if "-DUSE_USB_CDC_CONSOLE" not in env.BoardConfig().get("build.extra_flags"):
            print(Fore.YELLOW + "Unexpected naming convention in this build environment:" + Fore.RED, Infinitek_platform_org)
            print(Fore.YELLOW + "Expected build environment name like " + Fore.GREEN + "'Infinitek" + chip[3:] + "-whatever-you-want'")
            print(Fore.YELLOW + "Please correct your actual build environment, to avoid undefined behavior in build process!!")
    return Infinitek_platform
...
def esp32_fetch_safeboot_bin(Infinitek_platform):
    safeboot_fw_url = "http://ota.Infinitek.com/Infinitek32/release/" + Infinitek_platform + "-safeboot.bin"
    safeboot_fw_name = os.path.normpath(join(variants_dir, Infinitek_platform + "-safeboot.bin"))
    ...
    try:
        ...
        print(Fore.RED + "Creation of " + Infinitek_platform + "-factory.bin not possible")
        print(Fore.YELLOW + "Without Internet " + Fore.GREEN + Infinitek_platform + "-safeboot.bin" + Fore.YELLOW + " needs to be compiled before " + Fore.GREEN + Infinitek_platform)
        return False

def esp32_copy_new_safeboot_bin(Infinitek_platform,new_local_safeboot_fw):
...
def esp32_create_combined_bin(source, target, env):
    ...
    Infinitek_platform = esp32_create_chip_string(chip)

    if not os.path.exists(variants_dir):
        os.makedirs(variants_dir)
    if "safeboot" in firmware_name:
        s_flag = esp32_copy_new_safeboot_bin(Infinitek_platform,firmware_name)
    else:
        s_flag = esp32_fetch_safeboot_bin(Infinitek_platform)
