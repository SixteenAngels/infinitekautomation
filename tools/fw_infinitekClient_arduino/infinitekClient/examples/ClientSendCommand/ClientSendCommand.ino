/*
  ClientSendCommand.ino - Example for InfinitekClient to respond to ClientSend ON and
                         ClientSend OFF commands via console or telemetry.
                         In this example the ON and OFF is case sensitive so needs
                         to be sent in capital letters from the Infinitek device.
                         Upon receiving ON/OFF the client will turn the LED on/off
                         respectively.
                         
                         A second function callback is also attached which will
                         respond with a JSON message on Infinitek Teleperiod when
                         requested from the Infinitek device.
...
InfinitekClient client(&Serial);

/*******************************************************************\
 * Normal setup() function for Arduino to configure the serial port
 * speed (which should match what was configured in Infinitek) and
 * attach any callback functions associated with specific requests
 * or commands that are sent by Infinitek.
\*******************************************************************/
...
void setup() {
  ...
  Serial.begin(57600);
  // Attach the function which will respond to the JSON request from Infinitek
  client.attach_FUNC_JSON(user_FUNC_JSON);
  // Attach the function which will be called when the Infinitek device sends data using ClientSend command
  client.attach_FUNC_COMMAND_SEND(user_FUNC_RECEIVE);
}

/*******************************************************************\
 * Function which will be called when this client device receives a
 * ClientSend command from the Infinitek device.
\*******************************************************************/
 ...
 * Infinitek device upon receiving a request to do so
\*******************************************************************/
