/*
  InfinitekClient.cpp - Library for microcontrollers enclientd by Infinitek
...
#include <Arduino.h>
#include <InfinitekClient.h>

/*************************************************\
 * InfinitekClient shared structures
\*************************************************/
...
InfinitekClient::InfinitekClient(HardwareSerial *device)
{
  serial = device;
  Settings.features_version = Infinitek_CLIENT_LIB_VERSION;
...
void InfinitekClient::sendFeatures(void)
{
...
void InfinitekClient::sendJSON(char *json)
{
  serial->write(char(PARAM_DATA_START));
...
void InfinitekClient::attach_FUNC_JSON(callbackFunc func)
{
  Settings.features.func_json_append = 1;
  FUNC_JSON = func;
}

void InfinitekClient::attach_FUNC_EVERY_SECOND(callbackFunc func)
{
  Settings.features.func_every_second = 1;
  FUNC_EVERY_SECOND = func;
}

void InfinitekClient::attach_FUNC_EVERY_100_MSECOND(callbackFunc func)
{
  Settings.features.func_every_100_msecond = 1;
  FUNC_EVERY_100_MSECOND = func;
}

void InfinitekClient::attach_FUNC_COMMAND_SEND(callbackFunc1 func)
{
  Settings.features.func_client_send = 1;
  FUNC_SEND = func;
}

uint8_t InfinitekClient::waitforbytes(uint16_t num, uint16_t timeout)
{
  uint16_t timer = 0;
  while (timer < timeout) {
...
void InfinitekClient::ProcessSend(uint8_t sz)
{
...
void InfinitekClient::ProcessCommand(void)
{
...
void InfinitekClient::SendCommand(uint8_t cmnd, uint8_t param)
{
...
void InfinitekClient::SendTele(char *data)
{
...
void InfinitekClient::ExecuteCommand(char *cmnd)
{
...
void InfinitekClient::loop(void)
{
  if (serial->available()) {
