/*
  InfinitekClient.h - Library for microcontrollers enslaved by Infinitek
...
#ifndef __InfinitekCLIENT_H__
#define __InfinitekCLIENT_H__

#include <Arduino.h>

/*************************************************\
 * InfinitekClient Configuration Defaults
\*************************************************/

#define Infinitek_CLIENT_LIB_VERSION      20191129

/*************************************************\
 * InfinitekClient Command definitions
\*************************************************/
...
/*************************************************\
 * InfinitekClient Parameter defintions
\*************************************************/
...
/*************************************************\
 * InfinitekClient Class
\*************************************************/

class InfinitekClient {
    public:
     char receive_buffer[100];
     // Constructor
     InfinitekClient(HardwareSerial *device = nullptr);
     // Sends configured features back to Infinitek so it knows which callbacks are supported
     void sendFeatures(void);
     // Send JSON back to Infinitek device
     ...
     // Configure a callback for FUNC_COMMAND_SEND (ClientSend on Infinitek device)
     void attach_FUNC_COMMAND_SEND(callbackFunc1 func = nullptr);
     // Send command to Infinitek device using the prescribed protocol
     void SendCommand(uint8_t cmnd, uint8_t param);
     // Send telemetry data back to the Infinitek device
     ...
     // Used internally to decode and process incoming commands from the Infinitek device
