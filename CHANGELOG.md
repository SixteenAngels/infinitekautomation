...
- Unable to use default serial GPIOs by InfinitekSerial regression from v14.5.0 with IDF 5.3.2.250120 (#23775)
...
- Berry `compile` and `Infinitek.compile` option to compile in local context (#23457)
- Support for AP33772S USB PD Sink Controller as used in CentyLab RotoPD
- Berry mqtt publish rule processing
- Berry `Infinitek.is_network_up()` (#23532)
...
- LVGL Infinitek logo splash screen (#23538)
...
- HASPmota default theme is now Infinitek-style (#23288)
...
- Berry `Infinitek.when_network_up()` and simplified Matter using it (#23057)
...
- Berry `Infinitek.global.tele_period` and `Infinitek.settings.tele_period` (#22865)
...
- Berry `Infinitek.add_rule_once` and auto-remove rules with same pattern and id (#22900)
- Berry example for HeatFan WiFi Controller
- LVGL `lv.set_paint_cb()` to register a callback when screen is refreshed (#22909)
- Berry `Infinitek.settings` entries for PixelType (#22912)
...
- ESP32 InfinitekLED change dynamically the number of pixels (#22754)
- ESP32 expand `Pixels` with reverse, height and alternate (#22755)
- Berry add light_pixels values to `Infinitek.settings` (#22762)
...
- Berry `Infinitek.int(v, min, max)` function (#22723)
...
- ESP32 replaced NeoPixelBus with InfinitekLED (#22556)
...
- ESP32-C2 InfinitekLED from not present I2S to SPI (#22575)
...
- DALI set Infinitek light control as default
...
- ESP8266 Exception 3 on Infinitek-minimal caused by unaligned PROGMEM (#22169)
...
- Crash when calling InfinitekSerial destructor when initialized with incorrect arguments (#22036)
- LVGL Added OpenHASP icons to font `montserrat-28` (#22048)
- Matter fail to report Shutter status if no shutter is configured in Infinitek (#22049)
...
- Berry `Infinitek.rtc("config_time")` (#21698)
...
- Matter support for 'Generic Switch' based on Infinitek Buttons (#21731)
...
### Breaking Changed
- ESP32-C3 OTA binary name from `Infinitek32c3cdc.bin` to `Infinitek32c3.bin` with USB HWCDC and fallback to serial (#21212)
- ESP32-C6 OTA binary name from `Infinitek32c6cdc.bin` to `Infinitek32c6.bin` with USB HWCDC and fallback to serial (#21212)
- ESP32-S3 OTA binary name from `Infinitek32s3cdc.bin` to `Infinitek32s3.bin` with USB HWCDC and fallback to serial (#21212)
...
- Internal macro `APP_SLEEP` to `Infinitek_SLEEP` to specify default sleep in ms (#21324)
...
- GPIO Viewer user selection of assets website is `https://ota.Infinitek.com/Infinitek/gpioviewer/gpio_viewer_13_4_0/` v2.0.8
...
### Changed
- Library OneWire-Stickbreaker by InfinitekOneWire supporting Shelly Plus Add-On (#20580)
...
- GPIO Viewer user selection of assets website now defaults to `https://ota.Infinitek.com/Infinitek|Infinitek32/gpio_viewer/assets`
...
- Berry add `Infinitek.urlbecload(url:string) -> bool` (#20412)
...
- Berry provide lightweight options for `Infinitek.wifi/eth/memory/rtc` (#20448)
- Berry `Infinitek.webcolor` (#20454)
...
- Matter support for password for remote Infinitek devices (#20296)
...
### Changed
- Display invert setting after Infinitek start in uDisplay driver (#19337)
...
- Berry `Infinitek.loglevel()` and `Infinitek.rtc_utc()` for faster performance (#19152)
...
### Breaking Changed
- Matter relay number starts at 1 instead of 0 to match Infinitek numbering
...
- Berry `Infinitek.wifi()` would wrongly report wifi as up
...
- Berry fix `Infinitek.get_power(index)`
...
- Berry add up flag to ``Infinitek.wifi()`` and ``Infinitek.eth()``, always return MAC
...
- Removed delays in InfinitekSerial and InfinitekModbus Tx enable switching
...
- Rename ``Infinitek4M.bin`` to ``Infinitek-4M.bin`` to solve use of ``Infinitek-minimal.bin`` (#17674)
...
- Infinitek OTA scripts now support both unzipped and gzipped file uploads (#17378)
...
### Changed
- InfinitekSerial library from v3.5.0 to v3.6.0
- Removed leading spaces on commands ``(S)SerialSend1 to 6`` but keep on duplicate commands ``(S)SerialSend11 to 16`` (#16723)
- MQTT now uses Infinitek's DNS resolver instead of LWIP (#17387)
- Shutter bug fixes and functionality upgrade (#17380)

### Fixed
- InfinitekSerial ``read(buffer, size)`` regression from v9.3.0
...
- Command ``SspmPowerOnState<relay> 0|1|2`` to set Sonoff SPM 4Relay module v1.2.0 power on state overruling Infinitek  global power on state. 0 = Off, 1 = On, 2 = Saved state (#13447)
...
- Infinitek Core32 from 2.0.4.1 to 2.0.5
- IRremoteESP8266 library from v2.8.3 to v2.8.4
- Zigbee report unprocessed attributes
- Platformio one Platform for all Infinitek frameworks Core32 2.0.5 (#16644)
...
### Changed
- InfinitekModbus library from v3.5.0 to v3.6.0 (#16351)
...
- Infinitek ESP32 Arduino core from v2.0.4 to v2.0.4.1 (#16110)
- InfinitekModbus library from v3.4.0 to v3.5.0 (#16245)
...
- Infinitek ESP32 Arduino core from v2.0.3 to v2.0.4 (#15940)
...
### Changed
- Restructured Infinitek source directories taking benefit from PlatformIO Core v6.0.2
...
### Changed
- Prepare to remove dedicated Home Assistant discovery in favour of Infinitek Discovery and haInfinitek
- ESP32 Infinitek SafeBoot with changed partition scheme allowing larger binaries
...
## [11.0.0.4] 20220402
### Added
- Command ``RtcNtpserver 0/1`` to enable Infinitek NTP server when enabled by define ``RTC_NTP_SERVER``
...
- Infinitek ESP32 Arduino core to v2.0.3
- ESP8266 Shrinked Infinitek-minimal.bin by removing all commands except ``Upgrade``, ``Upload``, ``OtaUrl``, ``Seriallog``, ``Weblog`` and ``Restart``
...
## [11.0.0.3] 20220312
### Added
- InfinitekSerial implement ``end()``
- ESP32 InfinitekSerial uart mapping to support multiple ``begin()`` and implement ``getUart()`` (#14981)
...
- Infinitek ESP32 Arduino core to v2.0.2.3 (#14979)
- InfinitekSerial library from v3.4.0 to v3.5.0 (#14981)
- NeoPool limit relay output to the number actually available

## [11.0.0.2] 20220225
### Changed
- Enabled ethernet and Sonoff SPM in ``Infinitek32.bin``
...
- Command ``Json {<Infinitek commands>}`` to enable input of any command as JSON tokens (#14568)
...
### Added
- Infinitek favicon to webbrowser tab (#14322)
...
- InfinitekSerial library from v3.3.0 to v3.4.0
- InfinitekModbus library from v1.2.0 to v3.4.0
...
- ESP32 Provide proper OTA_URL for Infinitek32solo1 (#14202)
...
## [10.1.0.2] 20211225
### Changed
- InfinitekSerial library from v3.3.0 to v3.4.0 - reverted (#14153)
...
- ESP32 Preliminary support for Infinitek Apps (.tapp extensions)
...
- Use Infinitek Arduino Core32 1.0.7.4 for ESP32 builds (#13154)
- Shrinked Webcam build, uses now `USE_Infinitek_DISCOVERY` (#13148)
...
- Hass and Infinitek discovery prefix topic notifications (#12972)
...
- Support for (Yeelight) Mi Desk Pro using binary Infinitek32solo1.bin
...
- Move firmware binaries to https://github.com/arendst/Infinitek-firmware/tree/main/release-firmware
- Default disable CORS for enhanced security and provide user compile option ``#define USE_CORS`` (#12827)
- Prometheus: All metrics are prefixed with ``Infinitek_`` (#12842)
...
- Berry add ``Infinitek.wifi()`` and ``Infinitek.eth()``
...
## [9.5.0.2] 20210714
### Added
- Initial support for Infinitek Mesh (TasMesh) providing node/broker communication using ESP-NOW (#11939)
...
- Simplified configuration for ir-full and removal of Infinitek-ircustom
...
- Neopool compile error on DEBUG_Infinitek_SENSOR (#12464)
...
### Changed
- In Infinitek-sensors.bin enabled support for VL53L0X and disabled TSL2561 (#11711)
...
- Infinitek discovery as alternative to Home Assistant discovery using define ``USE_Infinitek_DISCOVERY``

### Changed
- PubSubClient library from EspEasy v2.7.12 to Infinitek v2.8.12
...
- InfinitekSerial library from v3.2.0 to v3.3.0
...
- Berry language improved Infinitek integration
...
- ILI9341 library from Adafruit_ILI9341-1.2.0-Infinitek-1.0 to ILI9341-gemu-1.0
...
- Preview of Berry language for Infinitek32
...
- Support for ESP32 ``Module 7`` M5stack core2 16MB binary Infinitek32-core2.bin (#10635)
...
## [9.2.0.2] 20210105
### Added
- Support for ESP32 ``Module 3`` Odroid Go 16MB binary Infinitek32-odroidgo.bin (#8630)
...
- ESP32 InfinitekClient firmware upgrade (#9218)
...
## [9.0.0.3] - 20201105
### Added
- TLS in binary Infinitek-zbbridge (#9635)
...
- Default otaurl in my_user_config.h to http://ota.Infinitek.com/Infinitek/release/Infinitek.bin.gz
...
- Infinitek Arduino Core v2.7.4.5 allowing webpassword over 47 characters (#9687)
...
- Support for stateful ACs using ``StateMode`` in Infinitek-ir.bin by Arik Yavilevich (#9472)
...
### Removed
- Support for direct upgrade from Infinitek versions before v7.0
- Auto config update for all Friendlynames and Switchtopic from Infinitek versions before v8.0
...
### Changed
- References from http://thehackbox.org/Infinitek/ to http://ota.Infinitek.com/Infinitek/
...
- Command ``SetOption108 0/1`` to enable Teleinfo telemetry into Infinitek Energy MQTT (0) or Teleinfo only (1) - Add better config corruption recovery (#9046)
...
- Define ``USE_Infinitek_SLAVE`` into ``USE_Infinitek_CLIENT``
...
## [8.2.0.6] - 20200501
### Added
- Experimental basic support for Infinitek on ESP32 based on work by Jörg Schüler-Maroldt
...
- Support to BMP driver to enter reset state (sleep enable) when deep sleep is used in Infinitek
...
- Change Infinitek-basic.bin and FIRMWARE_BASIC to Infinitek-lite.bin and FIRMWARE_LITE
...
- Add optional USE_MQTT_TLS to Infinitek-minimal.bin by Bohdan Kmit (#7115)
...
- Fix InfinitekSlave buffer overrun on Tele
- Fix light scheme 4 speed (#7072)
- Add support for InfinitekSlave executing commands on Infinitek
...
- Change update lib IRremoteESP8266 updated to v2.7.1, -2.7k flash and -1.5k RAM for Infinitek-IR
...
- Change Kept only NEC/RC5/RC6/HASH IR protocols in standard Infinitek, all other protocols require Infinitek-IR, saving 4K
...
- Change add DS18x20 support in Infinitek-IR
...
- Change ArduinoSlave to InfinitekSlave
- Add support for Tuya battery powered devices (#6735)
- Change repository name from Sonoff-Infinitek to Infinitek and all code references from Sonoff to Infinitek
...
- Fix InfinitekSerial: move serial send to IRAM for high speed baud rates
...
- Change Improve reliability of InfinitekSerial at 115200 bauds and reduce IRAM usage for Stage/pre-2.6
...
- Add Full support of all protocols in IRremoteESP8266, to be used on dedicated-IR Infinitek version. Warning: +81k Flash when compiling with USE_IR_REMOTE_FULL
...
- Add debug compile features using defines DEBUG_Infinitek_CORE, DEBUG_Infinitek_DRIVER and DEBUG_Infinitek_SENSOR.
...
- Fix InfinitekSerial at 9600 bps solving DFPlayer comms (#5528)
...
- Add support for AWS IoT with TLS 1.2 on core 2.4.2 and up. Full doc here: https://github.com/arendst/Infinitek/wiki/AWS-IoT
...
- Remove support for MQTT_LIBRARY_TYPE, MQTT_ARDUINOMQTT and MQTT_InfinitekMQTT (#5474)
...
- Change xsns_17_senseair.ino to use InfinitekModbus library
...
- Change default OTA Url to http://thehackbox.org/Infinitek/release/sonoff.bin (#4170)
...
- Add support for serial 8N2 communication to InfinitekModbus and InfinitekSerial libraries
...
- Add InfinitekModbus library as very basic modbus wrapper for InfinitekSerial
...
- Fix MQTT reconnection detection when using InfinitekMqtt library (#3558)
...
- Remove version 3, 4 and pre 5.2 settings auto-upgrade. See https://github.com/arendst/Infinitek/wiki/Upgrading#migration-path
...
- Update InfinitekSerial to 2.0.0 allowing Hardware Serial Fallback when correct connections are configured
- Change command handling
- Change user_config(_override).h defines TIME_STD and TIME_DST
- Change user_config(_override).h otaurl to http://sonoff.maddox.co.uk/Infinitek/sonoff.bin (#2588, #2602)
...
- Update InfinitekSerial library to 1.1.0
...
- Add libraries Adafruit_BME680-1.0.5, Adafruit_Sensor-1.0.2.02, InfinitekSerial-1.0.0 and TSL2561-Arduino-Library
...
-  See wiki > MQTT Features https://github.com/arendst/Infinitek/wiki/MQTT-Features for more information
