/*
  InfinitekLED.h - Lightweight implementation for adressable leds.
...
#ifndef __InfinitekLED_H__
#define __InfinitekLED_H__

enum InfinitekLEDTypesEncoding : uint16_t {
  // bits 0..3 encode for number of bytes per pixel
  InfinitekLed_1_Def  = 0x0,      // Default value - identical to InfinitekLed_3_RGB
  InfinitekLed_3_RGB  = 0x1,      // 3 bytes per pixel
  InfinitekLed_4_WRGB = 0x2,      // 4 bytes per pixel
  // bits 4..6 encode for pixel order
  InfinitekLed_Def = 0b000 << 4,  // Default value - identical to InfinitekLed_GRB
  InfinitekLed_GRB = 0b001 << 4,
  InfinitekLed_RGB = 0b010 << 4,
  InfinitekLed_RBG = 0b011 << 4,
  InfinitekLed_BRG = 0b100 << 4,
  InfinitekLed_BGR = 0b101 << 4,
  InfinitekLed_GBR = 0b110 << 4,
  // bit 7 sets the position for W channel
  InfinitekLed_xxxW = 0b0 << 7,   // W channel after color
  InfinitekLed_Wxxx = 0b1 << 7,   // W channel before color
  // bits 8..15 encode for timing specifics
  InfinitekLed_WS2812 = 0 << 8,
  InfinitekLed_SK6812 = 1 << 8,
  InfinitekLed_TimingEnd = 2 << 8,
};

enum InfinitekLEDHardware : uint32_t {
  // low-order bits are reserved for channels numbers and hardware flags - currenlty not useds
  // bits 16..23
  InfinitekLed_HW_Default = 0x000000,
  InfinitekLed_RMT  = (1 << 0) << 16,
  InfinitekLed_SPI  = (1 << 1) << 16,
  InfinitekLed_I2S  = (1 << 2) << 16,
  InfinitekLed_HW_None = 0xFF << 16,    // indicates that the specified HW is not supported
};
...
enum InfinitekLEDTypes : uint16_t {
  ws2812_grb  = InfinitekLed_3_RGB  | InfinitekLed_GRB | InfinitekLed_WS2812,                     // 1 for backwards compatibility
  sk6812_grbw = InfinitekLed_4_WRGB | InfinitekLed_GRB | InfinitekLed_xxxW | InfinitekLed_SK6812,   // 2 for backwards compatibility
  sk6812_grb  = InfinitekLed_3_RGB  | InfinitekLed_GRB | InfinitekLed_SK6812,
};
...
\*******************************************************************************************/
class InfinitekLEDPusher;   // forward definition
class InfinitekLED {
public:
  InfinitekLED(uint16_t type, uint16_t num_leds);
  ~InfinitekLED();
  ...
  bool Begin(void);
  void SetPusher(InfinitekLEDPusher *pusher);   // needs to be called before `Begin()`, sets the hardware implementation
...
protected:
  ...
  InfinitekLEDPusher *_pusher;          // pixels pusher implementation based on hardware (RMT, I2S...)
};

#endif  // __cplusplus
#endif  // __InfinitekLED_H__
