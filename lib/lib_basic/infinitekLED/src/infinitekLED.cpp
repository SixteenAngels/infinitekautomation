/*
  InfinitekLED.cpp - Lightweight implementation for adressable leds.
...
static const InfinitekLED_Timing InfinitekLED_Timings[] = {
  // WS2812
...
InfinitekLED::InfinitekLED(uint16_t type, uint16_t num_leds) :
  _type(type),
  _timing((type >> 8) & 0xFF),
  _started(false),
  _dirty(true),
  ...
  _pusher(nullptr)
{
  ...
  if (_timing > (InfinitekLed_TimingEnd >> 8)) {
    _timing = 0;
  }
  switch (_type & 0x0F) {
    case InfinitekLed_4_WRGB:
      _pixel_size = 4;
      break;
    case InfinitekLed_1_Def:
    case InfinitekLed_3_RGB:
    default:  // fallback
      _pixel_size = 3;
      break;
  }

  _pixel_matrix = &InfinitekLED_CHANNEL_ORDERS[_pixel_order];
...
void InfinitekLED::_adjustSubType(void) {
  _pixel_order = (_type >> 4) & 0x07;
  _pixel_matrix = &InfinitekLED_CHANNEL_ORDERS[_pixel_order];
  _w_before = _type & 0x08;
}

void InfinitekLED::SetPixelCount(uint16_t num_leds) {
...
void InfinitekLED::SetPixelSubType(uint8_t subtype) {
...
void InfinitekLED::ClearTo(uint32_t wrgb, int32_t first, int32_t last) {
...
void InfinitekLED::SetPixelColor(int32_t index, uint32_t wrgb) {
...
uint32_t InfinitekLED::GetPixelColor(int32_t index) {
  if (index < 0) { index += _pixel_count; }
  if ((index >= 0) && (index < _pixel_count)) {
    uint8_t *buf = _buf_work + index * _pixel_size;
    uint32_t wrgb = 0;
...
void InfinitekLED::SetPusher(InfinitekLEDPusher *pusher) {
  if (_pusher) {
    delete _pusher;
  }
  _pusher = pusher;
...
bool InfinitekLED::Begin(void) {
  if (_pusher) {
    if (_started) {
      return true;
    } else {
      const InfinitekLED_Timing * timing = &InfinitekLED_Timings[_timing];
...
bool InfinitekLED::CanShow(void) const {
  if (_pusher) {
