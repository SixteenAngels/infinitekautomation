/*
  InfinitekLEDPusherRMT.cpp - Implementation to push Leds via RMT channel
...
/*******************************************************************************************\
 * Implementation for InfinitekLEDPusherRMT
 * 
 * Code mostly copied from Infinitek patch to NeoPixelBus applied to support esp-idf 5.x
...
static esp_err_t rmt_new_led_strip_encoder(const led_strip_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder, rmt_symbol_word_t bit0,  rmt_symbol_word_t bit1, rmt_symbol_word_t reset_code) {
  static const char* TAG = "Infinitek_RMT";
...
InfinitekLEDPusherRMT::~InfinitekLEDPusherRMT() {
  if (_channel) {
...
InfinitekLEDPusherRMT::InfinitekLEDPusherRMT(int8_t pin) : _pin(pin) {
  esp_err_t ret = ESP_OK;
...
bool InfinitekLEDPusherRMT::Begin(uint16_t pixel_count, uint16_t pixel_size, const InfinitekLED_Timing * led_timing) {
  if (!_initialized) { return false; }
  InfinitekLEDPusher::Begin(pixel_count, pixel_size, led_timing);
...
bool InfinitekLEDPusherRMT::SetPixelCount(uint16_t pixel_count) {
  if (!_initialized) { return false; }
...
bool InfinitekLEDPusherRMT::CanShow(void) {
...
bool InfinitekLEDPusherRMT::Push(uint8_t *buf) {
  if (!_initialized) { return false; }
