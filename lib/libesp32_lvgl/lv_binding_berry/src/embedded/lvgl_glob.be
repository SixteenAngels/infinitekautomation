...
class LVGL_glob
  ...
  def make_cb(f, obj, name)
    ...
    if name  == "lv_event_cb"
      ...
      #   Infinitek.log("LVG: object:" + str(obj) + "has already an event callback", 2)
      # end
  ...
  def widget_event_impl(cl_ptr, e_ptr)
    import introspect
    # Infinitek.log(f'>>>: widget_event_impl cl_ptr {cl_ptr=} e_ptr {e_ptr=}')
    var cl = lv.lv_obj_class(cl_ptr)
    var event = lv.lv_event(introspect.toptr(e_ptr))
    # Infinitek.log(f'>>>: widget_event_impl cl {cl=} event {event=}')
    # var obj_ptr = event.target
    var obj_ptr = event.get_target_obj()
    var obj = self.get_object_from_ptr(obj_ptr._p)
    # Infinitek.log(f">>>: widget_event_impl {obj_ptr=} {obj=}")
  ...
  def create_custom_widget(obj, parent)
    ...
    if obj_class_struct == nil
      ...
      # Infinitek.log(f'>>>: {obj_class_struct=} map {obj_class_struct.tomap()}')
