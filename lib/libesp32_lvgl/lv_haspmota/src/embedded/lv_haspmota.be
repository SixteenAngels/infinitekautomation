...
class Antiburn
  ...
  def cycle(i)
    ...
    if i < 30
      ...
      Infinitek.set_timer(1000, /->self.cycle(i+1))
    else
      self.stop()
    end
  end
...
class lvh_root
  ...
  def remove_val_rule()
    if self._val_rule != nil
      Infinitek.remove_rule(self._val_rule, self)
    end
  end
  def set_val_rule(t)
    ...
    Infinitek.add_rule(self._val_rule, / val -> self.val_rule_matched(val), self)
  end
  def get_val_rule()
  ...
  def remove_text_rule()
    if self._text_rule != nil
      Infinitek.remove_rule(self._text_rule, self)
    end
  end
  def set_text_rule(t)
    # remove previous rule if any
    ...
    Infinitek.add_rule(self._text_rule, / val -> self.text_rule_matched(val), self)
  end
  def get_text_rule()
...
class lvh_obj : lvh_root
  ...
  def event_cb(event)
    ...
    # defer the actual action to the Infinitek event loop
    ...
    if self.action != "" && code == lv.EVENT_CLICKED
      ...
      Infinitek.defer(/-> hm.do_action(self, code))
    end
    ...
    if event_hasp != nil
      ...
      # print("val=",val)
      Infinitek.defer(def ()
                      Infinitek.publish_rule(tas_event)
                      Infinitek.log(f"HSP: publish {tas_event}", 4)
                    end)
    end
  end
...
class lvh_img : lvh_obj
  ...
  #- ------------------------------------------------------------#
  # `src` virtual setter
  # If source is `Infinitek_logo`, use the embedded logo
  #- ------------------------------------------------------------#
  def set_src(t)
    if (t == 'Infinitek_logo')
      self._lv_obj.set_Infinitek_logo()
    else
...
class lvh_tabview : lvh_obj
  ...
  def set_val(v, stop)
    ...
    if (v_max == 0)
      ...
      if (!stop)
        Infinitek.defer(def () self.set_val(v, true #-stop propagation-#) end)
      end
    else
      if (v == nil)   v = 0           end
      if (v < 0)      v = 0           end
...
class lvh_page
  ...
  def show(anim, duration)
    ...
    Infinitek.defer(/-> Infinitek.publish_rule(event_str_in))
    var event_str_out = format('{"hasp":{"p%i":"in"}}', self._page_id)
    Infinitek.defer(/-> Infinitek.publish_rule(event_str_out))
...
class HASPmota
  ...
  def start(arg1, arg2)
    ...
    var primary_color = self.lvh_root.parse_color(Infinitek.webcolor(10 #-COL_BUTTON-#))
    var secondary_color = self.lvh_root.parse_color(Infinitek.webcolor(11 #-COL_BUTTON_HOVER-#))
    var color_scr = self.lvh_root.parse_color(Infinitek.webcolor(1 #-COL_BACKGROUND-#))
    var color_text = self.lvh_root.parse_color(Infinitek.webcolor(9 #-COL_BUTTON_TEXT-#))
    var color_card = self.lvh_root.parse_color(Infinitek.webcolor(2 #-COL_FORM-#))
    var color_grey = self.lvh_root.parse_color(Infinitek.webcolor(2 #-COL_FORM-#))
    var color_reset = self.lvh_root.parse_color(Infinitek.webcolor(12 #-COL_BUTTON_RESET-#))
    var color_reset_hover = self.lvh_root.parse_color(Infinitek.webcolor(13 #-COL_BUTTON_RESET_HOVER-#))
    var color_save = self.lvh_root.parse_color(Infinitek.webcolor(14 #-COL_BUTTON_SAVE-#))
    var color_save_hover = self.lvh_root.parse_color(Infinitek.webcolor(15 #-COL_BUTTON_SAVE_HOVER-#))
  ...
  def _load(templ_name)
    ...
    while f.tell() < f.size()                 # while we're not at the end of the file
      ...
      if type(jline) == 'instance'
        if Infinitek.loglevel(4)
          if string.endswith(line, "\n")   line = line[0..-2]    end   # remove unwanted last '\n'
          Infinitek.log(f"HSP: parsing line '{line}'", 4)
        end
        ...
        if size(string.tr(line, " \t", "")) > 0
          Infinitek.log(f"HSP: invalid JSON line '{line}'", 2)
        end
      end
      jline = nil
    end
