/*
  InfinitekSerial.h - Implementation of software serial with hardware serial fallback for Infinitek
...
#ifndef InfinitekSerial_h
#define InfinitekSerial_h
/*********************************************************************************************\
 * InfinitekSerial supports up to 115200 baud with default buffer size of 64 bytes using optional no iram
 *
...
class InfinitekSerial : public Stream {
  public:
    InfinitekSerial(int receive_pin, int transmit_pin, int hardware_fallback = 0, int nwmode = 0, int buffer_size = TM_SERIAL_BUFFER_SIZE, bool invert = false);
    virtual ~InfinitekSerial();
