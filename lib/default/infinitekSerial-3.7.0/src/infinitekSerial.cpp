/*
  InfinitekSerial.cpp - Implementation of software serial with hardware serial fallback for Infinitek
...
void IRAM_ATTR callRxRead(void *self) { ((InfinitekSerial*)self)->rxRead(); };

// As the Arduino attachInterrupt has no parameter, lists of objects
// and callbacks corresponding to each possible GPIO pins have to be defined
InfinitekSerial *tms_obj_list[16];
...
static uint32_t Infinitek_serial_uart_bitmap = 0;      // Assigned UARTs

#endif  // ESP32

InfinitekSerial::InfinitekSerial(int receive_pin, int transmit_pin, int hardware_fallback, int nwmode, int buffer_size, bool invert) {
  m_valid = false;
  m_hardserial = false;
  m_hardswap = false;
  m_overflow = false;
...
//  Serial.printf("TSR: Freeing UART%d\n", m_uart);
  ...
  TSerial->end();
  bitClear(Infinitek_serial_uart_bitmap, m_uart);
#endif  // ESP32
}

InfinitekSerial::~InfinitekSerial(void) {
  if (m_valid) {
    end();
  }
}

bool InfinitekSerial::isValidGPIOpin(int pin) {
#ifdef ESP8266
...
void InfinitekSerial::setTransmitEnablePin(int tx_enable_pin) {
#ifdef ESP8266
...
#ifdef ESP32
bool InfinitekSerial::freeUart(void) {
  ...
  if (((pin_soc_rx0 == m_rx_pin) && (pin_soc_tx0 == m_tx_pin)) ||
    ...
    bitSet(Infinitek_serial_uart_bitmap, m_uart);
    return true;
  } else {
    // Find a free UART which may end up as UART0
    for (uint32_t i = SOC_UART_HP_NUM -1; i >= 0; i--) {
      if (0 == bitRead(Infinitek_serial_uart_bitmap, i)) {
...
void InfinitekSerial::Esp32Begin(void) {
  // Workaround IDF #14787 introduced in Infinitek v14.5.0, Core 3.1.1, IDF 5.3.2.250120
...
size_t InfinitekSerial::setRxBufferSize(size_t size) {
...
bool InfinitekSerial::begin(uint32_t speed, uint32_t config) {
...
void InfinitekSerial::setReadChunkMode(bool mode) {
  m_very_high_speed = mode;
}

bool InfinitekSerial::hardwareSerial(void) {
#ifdef ESP8266
  return m_hardserial;
#endif  // ESP8266
#ifdef ESP32
...
bool InfinitekSerial::overflow(void) {
  if (m_hardserial) {
#ifdef ESP8266
...
void InfinitekSerial::flush(void) {
  if (m_hardserial) {
#ifdef ESP8266
    Serial.flush();
#endif  // ESP8266
...
size_t InfinitekSerial::read(char* buffer, size_t size) {
  if (m_hardserial) {
#ifdef ESP8266
...
void IRAM_ATTR InfinitekSerial::_fast_write(uint8_t b) {
...
size_t InfinitekSerial::write(uint8_t b) {
...
// See https://github.com/arendst/Infinitek/discussions/22272
int32_t InfinitekSerial::setConfig(uint32_t config) {
...
#ifdef ESP8266
void IRAM_ATTR InfinitekSerial::rxRead(void) {
  if (!m_nwmode) {
    uint32_t start = ESP.getCycleCount();
