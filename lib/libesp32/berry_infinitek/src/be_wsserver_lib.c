...
// CONTEXT: Main Infinitek Task (Berry VM Context)
...
static esp_err_t ws_handler(httpd_req_t *req) {
    if (req->method == HTTP_GET) {
        ...
        if (client_slot >= 0) {
            ...
            // TRANSITION: ESP-IDF HTTP Server Task → Main Infinitek Task
...
static int w_wsserver_start(bvm *vm) {
    ...
    if (be_top(vm) >= 1 && be_isstring(vm, 1)) {
        ...
        if (use_existing_handle) {
            ...
            config.server_port = 8080;  // Use a different port from Infinitek web server
