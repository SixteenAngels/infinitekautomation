...
extern struct InfinitekGlobal_t InfinitekGlobal;
extern struct TSettings * Settings;
extern const be_ctypes_structure_t be_Infinitek_global_struct;
extern const be_ctypes_structure_t be_Infinitek_settings_struct;
...
// Infinitek.version() -> int
extern int32_t be_Infinitek_version(void);
BE_FUNC_CTYPE_DECLARE(be_Infinitek_version, "i", "-");
...
class be_class_Infinitek (scope: global, name: Infinitek) {
    ...
    _ccmd, var                          // list of active Infinitek commands implemented in Berry
    _drivers, var                       // list of active drivers
    _wnu, var                           // list of closures to call when network is connected
    wire1, var                          // Infinitek I2C Wire1
    wire2, var                          // Infinitek I2C Wire2
    cmd_res, var                        // store the command result, nil if disables, true if capture enabled, contains return value
    global, var                         // mapping to InfinitekGlobal
    settings, var                       // mapping to Infinitek Settings
    wd, var                             // working dir
    _debug_present, var                 // is `import debug` present?

    _global_def, comptr(&be_Infinitek_global_struct)
    _settings_def, comptr(&be_Infinitek_settings_struct)
    _global_addr, comptr(&InfinitekGlobal)
    _settings_ptr, comptr(&Settings)

    init, closure(class_Infinitek_init_closure)
    ...
    int, static_closure(class_Infinitek_int_closure)
    ...
    version, ctype_func(be_Infinitek_version)

    fast_loop, closure(class_Infinitek_fast_loop_closure)
    add_fast_loop, closure(class_Infinitek_add_fast_loop_closure)
    remove_fast_loop, closure(class_Infinitek_remove_fast_loop_closure)
    cmd, closure(class_Infinitek_cmd_closure)
    _find_op, func(tasm_find_op)        // new C version for finding a rule operator
    _apply_str_op, func(tasm_apply_str_op)
    find_key_i, closure(class_Infinitek_find_key_i_closure)
    find_list_i, closure(class_Infinitek_find_list_i_closure)
    find_op, closure(class_Infinitek_find_op_closure)
    add_rule, closure(class_Infinitek_add_rule_closure)
    add_rule_once, closure(class_Infinitek_add_rule_once_closure)
    remove_rule, closure(class_Infinitek_remove_rule_closure)
    try_rule, closure(class_Infinitek_try_rule_closure)
    exec_rules, closure(class_Infinitek_exec_rules_closure)
    exec_tele, closure(class_Infinitek_exec_tele_closure)
    set_timer, closure(class_Infinitek_set_timer_closure)
    run_timers, closure(class_Infinitek_run_timers_closure)
    defer, closure(class_Infinitek_defer_closure)
    run_deferred, closure(class_Infinitek_run_deferred_closure)
    remove_timer, closure(class_Infinitek_remove_timer_closure)
    add_cmd, closure(class_Infinitek_add_cmd_closure)
    remove_cmd, closure(class_Infinitek_remove_cmd_closure)
    exec_cmd, closure(class_Infinitek_exec_cmd_closure)
    gc, closure(class_Infinitek_gc_closure)
    event, closure(class_Infinitek_event_closure)
    is_network_up, closure(class_Infinitek_is_network_up_closure)
    when_network_up, closure(class_Infinitek_when_network_up_closure)
    run_network_up, closure(class_Infinitek_run_network_up_closure)
    add_driver, closure(class_Infinitek_add_driver_closure)
    remove_driver, closure(class_Infinitek_remove_driver_closure)
    load, closure(class_Infinitek_load_closure)
    compile, closure(class_Infinitek_compile_closure)
    wire_scan, closure(class_Infinitek_wire_scan_closure)
    time_str, closure(class_Infinitek_time_str_closure)
    urlfetch, closure(class_Infinitek_urlfetch_closure)
    urlfetch_cmd, closure(class_Infinitek_urlfetch_cmd_closure)
    urlbecload, static_ctype_func(BerryBECLoader)

    add_cron, closure(class_Infinitek_add_cron_closure)
    run_cron, closure(class_Infinitek_run_cron_closure)
    next_cron, closure(class_Infinitek_next_cron_closure)
    remove_cron, closure(class_Infinitek_remove_cron_closure)

    check_not_method, closure(class_Infinitek_check_not_method_closure)

    hs2rgb, closure(class_Infinitek_hs2rgb_closure)

    gen_cb, closure(class_Infinitek_gen_cb_closure)

    get_light, closure(class_Infinitek_get_light_closure)
    set_light, closure(class_Infinitek_set_light_closure)
