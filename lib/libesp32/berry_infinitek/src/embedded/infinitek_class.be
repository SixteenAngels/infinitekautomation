...
class Infinitek
  ...
  var global          # mapping to InfinitekGlobal
  var settings
  var wd              # when load() is called, `Infinitek.wd` contains the name of the archive. Ex: `/M5StickC.autoconf#`
  var _debug_present  # is `import debug` present?

  def init()
    # instanciate the mapping object to InfinitekGlobal
  ...
  #-
    assert(Infinitek.find_op("Dimmer>50") == ['Dimmer', '>', '50'])
    assert(Infinitek.find_op("Dimmer") == ['Dimmer', nil, nil])
    assert(Infinitek.find_op("Status!==Connected") == ['Status', '!==', 'Connected'])
  -#
  def find_op(item)
  ...
  Infinitek.add_rule("A#B", f1, "f1")
  Infinitek.add_rule_once("A#B", f2, "f2")

  var r
  r = Infinitek.publish_rule('{"A":{"B":1}}')
  ...
  def exec_rules(ev_json, exec_rule)
    ...
    if self._rules || save_cmd_res != nil # if there is a rule handler, or we record rule results
      ...
      # record the rule payload for Infinitek.cmd()
      if save_cmd_res != nil
        self.cmd_res = ev
      end
  ...
  def defer(f)
    ...
    self._defer.push(f)
    Infinitek.global.deferred_ready = 1
  end
  ...
  def run_deferred()
    if self._defer
      ...
      if size(self._defer) == 0
        Infinitek.global.deferred_ready = 0
      end
    end
  end
  ...
  # Infinitek.wire_scan(addr:int [, index:int]) -> wire1 or wire2 or nil
  ...
  #    Infinitek.compile("autoexec.be")      -- compiles autoexec.be to autoexec.bec
  #
  ...
  # fast_loop() is a trimmed down version of event() called at every Infinitek loop iteration
  ...
  def add_fast_loop(cl)
    ...
    self.global.fast_loop_enabled = 1      # enable fast_loop at global level: `InfinitekGlobal.fast_loop_enabled = true`
    self._fl.push(cl)
  end
  ...
  def is_network_up()
    return Infinitek.wifi()['up'] || Infinitek.eth()['up']
  end
  ...
  def cmd(command, mute)
    ...
    var maxlog_level = Infinitek.global.maxlog_level
    if mute                 # mute logging
      if maxlog_level >= 2 Infinitek.global.maxlog_level = 1 end
    end
    ...
    # restore log_level
    if mute
      Infinitek.global.maxlog_level = maxlog_level
    end
    return ret
  end

  # Infinitek.int(v, min, max)
  ...
  assert(Infinitek.int(4) == 4)
  assert(Infinitek.int(nil) == nil)
  assert(Infinitek.int(-3) == -3)
  assert(Infinitek.int(4.5) == 4)
  assert(Infinitek.int(true) == 1)
  assert(Infinitek.int(false) == 0)

  # normal behavior
  assert(Infinitek.int(4, 0, 10) == 4)
  assert(Infinitek.int(0, 0, 10) == 0)
  assert(Infinitek.int(10, 0, 10) == 10)
  assert(Infinitek.int(10, 0, 0) == 0)
  assert(Infinitek.int(10, 10, 10) == 10)
  assert(Infinitek.int(-4, 0, 10) == 0)
  assert(Infinitek.int(nil, 0, 10) == 0)

  # missing min or max
  assert(Infinitek.int(4, nil, 10) == 4)
  assert(Infinitek.int(14, nil, 10) == 10)
  assert(Infinitek.int(nil, nil, 10) == nil)
  assert(Infinitek.int(4, 0, nil) == 4)
  assert(Infinitek.int(-4, 0, nil) == 0)
  assert(Infinitek.int(nil, 0, nil) == nil)

  # max < min
  assert(Infinitek.int(4, 10, 0) == 10)
  assert(Infinitek.int(nil, 10, 0) == 10)

  -#

  # set_light and get_light deprecetaion
  def get_light(l)
    print('Infinitek.get_light() is deprecated, use light.get()')
    import light
    if l != nil
      return light.get(l)
    else
  ...
  def set_light(v,l)
    print('Infinitek.set_light() is deprecated, use light.set()')
    import light
    if l != nil
  ...
  def hs2rgb(hue,sat)
    ...
    if sat > 0
      ...
      var f = hue % 60    # 0..59
      var p = 255 - sat
      var q = Infinitek.scale_uint(f, 0, 60, 255, p)    # 0..59
      var t = Infinitek.scale_uint(f, 0, 60, p, 255)
  ...
  # urlfetch - add a Infinitek command to download a file from a URL to file-system
