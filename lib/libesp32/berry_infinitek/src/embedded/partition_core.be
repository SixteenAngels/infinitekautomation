...
class Partition
  ...
  def resize_max_flash_size_k()
    ...
    if (flash_size_k != flash_size_real_k) && flash_definition_sector != nil
      ...
      if flash_size_code != nil
        ...
        Infinitek.log(format("UPL: changing flash definition from 0x02X to 0x%02X", old_def, flash_def[3]), 3)
      else
  ...
  def resize_fs_to_max()
    try
      ...
      Infinitek.log(format("BRY: Trying to expand FS by %i kB", unallocated), 2)
      ...
      Infinitek.log(format("BRY: Exception> '%s' - %s", e, m), 2)
    end
  end
