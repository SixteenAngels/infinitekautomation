...
autoconf_module.init = def (m)
  ...
  class Autoconf
    ...
    def init()
      ...
      while i < size(dir)
        if string.find(dir[i], ".autoconf") > 0   # does the file contain '*.autoconf', >0 to skip `.autoconf`
          if entry != nil
            ...
            Infinitek.log(format("CFG: multiple autoconf files found, aborting ('%s' + '%s')", entry, dir[i]), 2)
            self._error = true
            return nil
          end
          entry = dir[i]
      ...
      if entry == nil
        Infinitek.log("CFG: No '*.autoconf' file found", 3)
        return nil
      end
    ...
    def load_templates()
      import json
      try 
        var url = format("https://raw.githubusercontent.com/Infinitek/autoconf/main/%s_manifest.json", Infinitek.arch())
        Infinitek.log(format("CFG: loading '%s'", url), 3)
        # load the template
        var cl = webclient()
        cl.begin(url)
        var r = cl.GET()
        if r != 200
          Infinitek.log(format("CFG: return_code=%i", r), 2)
          return nil
        end
        var s = cl.get_string()
        cl.close()
        # convert to json
        var j = json.load(s)
        Infinitek.log(format("CFG: loaded '%s'", str(j)), 3)
        ...
        Infinitek.log(format("CFG: exception '%s' - '%s'", e, m), 2)
        return nil
      end
    end
    ...
    def page_autoconf_ctl()
      ...
      try
        if webserver.has_arg("reapply")
          Infinitek.log("CFG: removing first time marker", 2);
          ...
          Infinitek.log("CFG: removing autoconf files", 2);
          ...
          if arch_name != "reset"
            var url = format("https://raw.githubusercontent.com/Infinitek/autoconf/main/%s/%s.autoconf", Infinitek.arch(), arch_name)
            Infinitek.log(format("CFG: downloading '%s'", url), 2);
    ...
    def preinit()
      ...
      if path.exists(fname)
        Infinitek.log("CFG: loading "+fname, 3)
        load(fname)
        Infinitek.log("CFG: loaded  "+fname, 3)
      end
    end

    #####################################################################################################
    # run_bat
    # load and run `<fname>.bat` file as Infinitek commands
    #####################################################################################################
    def run_bat(fname)    # read a '*.bat' file and run each command
      ...
      try
        ...
        while true
          ...
          if size(line) > 0
            Infinitek.cmd(line)             # run the command
          end
        end
        f.close()                         # close, we don't expect exception with read-only, could be added later though
      except .. as e, m
        Infinitek.log(format('CFG: could not run %s (%s - %s)', fname, e, m), 1)
    ...
    def autoexec()
      ...
      if self.is_first_time() && path.exists(fname)
        ...
        self.run_bat(fname)
        Infinitek.log("CFG: 'init.bat' done, restarting", 2)
        reboot = true
      ...
      if path.exists(fname) && (self.is_first_time() || !path.exists('display.ini'))
        ...
        try
          ...
          f_out.close()
          reboot = true
          Infinitek.log("CFG: 'display.ini' extracted, restarting", 2)
        except .. as e, m
      ...
      if path.exists(fname)
        Infinitek.log("CFG: running "+fname, 3)
        self.run_bat(fname)
        Infinitek.log("CFG: ran  "+fname, 3)
      end
