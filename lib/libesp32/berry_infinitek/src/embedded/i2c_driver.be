#-------------------------------------------------------------
 - IMPORTANT
 - THIS CLASS IS ALREADY BAKED IN Infinitek
 -
...
class I2C_Driver
  ...
  #- Init and look for device
   ...
   -   i2c_index : Infinitek I2C index, see `I2CDEVICES.md` (int)
   --#
  def init(name_or_detect, addr, i2c_index)
    #- check if the i2c index is disabled by Infinitek configuration -#
    if i2c_index != nil && !Infinitek.i2c_enabled(i2c_index) return end

    self.addr = addr                            #- address for AXP192 -#
    self.wire = Infinitek.wire_scan(self.addr)    #- get the right I2C bus -#
