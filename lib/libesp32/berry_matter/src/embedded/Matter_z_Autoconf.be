#
# Matter_z_Autoconf.be - implements autoconfiguration of Matter from Infinitek native features
#
...
class Matter_Autoconf
  ...
  def instantiate_plugins_from_config(config)
    ...
    Infinitek.publish_result('{"Matter":{"Initialized":1}}', 'Matter')
  end
  ...
  def autoconf_device_map()
    ...
    var relay_count = size(Infinitek.get_power())
    ...
    # auto-detect sensors
    var sensors = json.load(Infinitek.read_sensors())
    ...
    # Infinitek.publish_result('{"Matter":{"Initialized":1}}', 'Matter')    # MQTT is not yet connected
    return m
  end
