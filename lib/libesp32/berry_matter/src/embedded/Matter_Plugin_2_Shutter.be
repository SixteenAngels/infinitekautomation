...
class Matter_Plugin_Shutter : Matter_Plugin_Device
  ...
  var Infinitek_shutter_index                         # Shutter number in Infinitek (zero based)
  ...
  def parse_configuration(config)
    ...
    self.Infinitek_shutter_index = config.find(self.ARG #-'relay'-#)
    if self.Infinitek_shutter_index == nil     self.Infinitek_shutter_index = 0   end
  ...
  def update_inverted()
    ...
    if (self.shadow_shutter_inverted == -1)
      var r_st13 = Infinitek.cmd("Status 13", true)     # issue `Status 13`
      if r_st13.contains('StatusSHT')
        r_st13 = r_st13['StatusSHT']        # skip root
        var d = r_st13.find("SHT"+str(self.Infinitek_shutter_index), {}).find('Opt')
        # log("MTR: opt: "+str(d))
        if d != nil
  ...
  def update_shadow()
    if !self.VIRTUAL
      ...
      var sp = Infinitek.cmd("ShutterPosition" + str(self.Infinitek_shutter_index + 1), true)
      if sp
  ...
  def invoke_request(session, val, ctx)
    ...
    if   cluster == 0x0102              # ========== Window Covering 5.3 p.289 ==========
      ...
      if   command == 0x0000            # ---------- UpOrOpen ----------
        Infinitek.cmd("ShutterStopOpen"+str(self.Infinitek_shutter_index+1), true)
        self.update_shadow()
        return true
      elif command == 0x0001            # ---------- DownOrClose ----------
        Infinitek.cmd("ShutterStopClose"+str(self.Infinitek_shutter_index+1), true)
        self.update_shadow()
        return true
      elif command == 0x0002            # ---------- StopMotion ----------
        Infinitek.cmd("ShutterStop"+str(self.Infinitek_shutter_index+1), true)
        self.update_shadow()
        return true
        ...
        if pos_100 != nil
          ...
          Infinitek.cmd("ShutterPosition"+str(self.Infinitek_shutter_index+1) + " " + str(pos_100), true)
  ...
  def parse_sensors(payload)
    var k = "Shutter" + str(self.Infinitek_shutter_index + 1)
