...
class Matter_Plugin_Light1 : Matter_Plugin_Light0
  ...
  # var Infinitek_relay_index                             # Relay number in Infinitek (1 based), nil for internal light
  ...
  def parse_configuration(config)
    ...
    # with Light0 we always need relay number but we don't for Light1/2/3 so self.Infinitek_relay_index may be `nil`
    if self.BRIDGE
      self.Infinitek_relay_index = int(config.find(self.ARG #-'relay'-#, nil))
      if (self.Infinitek_relay_index != nil && self.Infinitek_relay_index <= 0)    self.Infinitek_relay_index = 1    end
    else
      if (self.Infinitek_relay_index == nil) && (self.TYPE == "light1")   # only if `light1` and not for subclasses
        var light_index_arg = config.find(self.ARG #-'light'-#)
        if (light_index_arg == nil)
          if (Infinitek.get_option(68) == 0)    # if default mode, and `SO68 0`, check if we have split RGB/W
            import light
            if (light.get(1) != nil)
  ...
  def update_shadow()
    if !self.VIRTUAL && !self.BRIDGE
      ...
      if light_status != nil
        ...
        if bri != nil
          bri = Infinitek.scale_uint(bri, 0, 255, 0, 254)
  ...
  def set_bri(bri_254, pow)
    ...
    if self.BRIDGE
      var dimmer = Infinitek.scale_uint(bri_254, 0, 254, 0, 100)
      ...
      import light
      var bri_255 = Infinitek.scale_uint(bri_254, 0, 254, 0, 255)
      if pow == nil
  ...
  def invoke_request(session, val, ctx)
    ...
    if   cluster == 0x0008              # ========== Level Control 1.6 p.57 ==========
      ...
      if   command == 0x0000            # ---------- MoveToLevel ----------
        ...
        self.publish_command('Bri', bri_254, 'Dimmer', Infinitek.scale_uint(bri_254, 0, 254, 0, 100))
        return true
        ...
        ctx.log = "bri:"+str(bri_254)
        self.publish_command('Power', onoff ? 1 : 0, 'Bri', bri_254, 'Dimmer', Infinitek.scale_uint(bri_254, 0, 254, 0, 100))
        return true
  ...
  def parse_status(data, index)
    ...
    if index == 11                              # Status 11
      ...
      if dimmer != nil
        var bri = Infinitek.scale_uint(dimmer, 0, 100, 0, 254)
        if bri != self.shadow_bri
          self.attribute_updated(0x0008, 0x0000)
          self.shadow_bri = bri
        end
  ...
  def web_value_dimmer()
    ...
    if self.shadow_bri != nil
      var bri = Infinitek.scale_uint(self.shadow_bri, 0, 254, 0, 100)
