...
class Matter_HTTP_async : Matter_TCP_async
  ...
  def receive()
    ...
    if (Infinitek.millis() - self.time_start) > self.timeout
      # connection timeout
      # print("Connected timeout in", Infinitek.millis() - self.time_start, "ms")
      self.status = -3
      self.close()
      self.http_status = -2
      ...
      while avail > 0
        var s = self.tcp.read()
        # print("read size=", size(s), "avail=", avail, "in", Infinitek.millis() - self.time_start, "ms")
        self.response += s
        new_data += size(s)
      ...
      if new_data > 0
        # print("read size=", new_data, "in", Infinitek.millis() - self.time_start, "ms")
