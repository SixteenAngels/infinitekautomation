...
class Matter_Commissioning
  ...
  def every_second()
    if self.commissioning_open != nil && Infinitek.time_reached(self.commissioning_open)    # timeout reached, close provisioning
  ...
  def start_root_basic_commissioning(timeout_s)
    ...
    Infinitek.publish_result(format('{"Matter":{"Commissioning":1,"PairingCode":"%s","QRCode":"%s"}}', pairing_code, qr_code), 'Matter')
  ...
  def start_basic_commissioning(timeout_s, iterations, discriminator, salt, w0, L, admin_fabric)
    self.commissioning_open = Infinitek.millis() + timeout_s * 1000
    ...
    Infinitek.when_network_up(def () self.mdns_announce_PASE() end)
  end
  ...
  def stop_basic_commissioning()
    ...
    if self.is_root_commissioning_open()
      Infinitek.publish_result('{"Matter":{"Commissioning":0}}', 'Matter')
    end
  ...
  def start_operational_discovery_deferred(fabric)
    # defer to next click
    Infinitek.defer(/-> self.start_operational_discovery(fabric))
  end
  ...
  def start_commissioning_complete_deferred(session)
    # defer to next click
    Infinitek.defer(/-> self.start_commissioning_complete(session))
  end
  ...
  def start_mdns_announce_hostnames()
    Infinitek.when_network_up(def ()

      if Infinitek.wifi('up')
