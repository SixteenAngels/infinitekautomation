#
# Matter_HTTP_remote.be - implements an interface to query remotely Infinitek device via HTTP
#
...
class Matter_HTTP_remote : Matter_HTTP_async
  ...
  def parse_status_http(data, index)
    var changed = false
    if index == 0                               # Status
      var device_name = data.find("DeviceName")              # we consider 'Infinitek' as the non-information default
      if device_name == "Infinitek"   device_name = nil   end
  ...
  def device_is_alive(alive)
    if alive
      # device is known to be reachable
      self.reachable = true
      self.reachable_utc = Infinitek.rtc_utc()
    else
  ...
  #
  # cmd: the Infinitek command as string
  ...
  def probe_async(cmd)
    import string
    import webserver
    if !Infinitek.wifi()['up'] && !Infinitek.eth()['up']    return nil    end       # no network
  ...
  def call_sync(cmd, timeout)
    ...
    log(format("MTR: HTTP sync-resp  in %i ms from %s: [%i] '%s'", Infinitek.millis() - self.time_start, self.addr, size(self.payload), payload_short), 3)
    return ret
  end
  ...
  def event_http_finished()
    ...
    if size(payload_short) > 30   payload_short = payload_short[0..29] + '...'   end
    log(format("MTR: HTTP async-resp in %i ms from %s: [%i] '%s'", Infinitek.millis() - self.time_start, self.addr, size(self.payload), payload_short), 3)
  ...
  def scheduler()
    ...
    if cmd == nil                                   # if no priotity, find first eligible
      for k: self.probe_next_timestamp_map.keys()
        if Infinitek.time_reached(self.probe_next_timestamp_map[k])
          cmd = k
          break
        end
      end
    ...
    self.probe_next_timestamp_map[cmd] = Infinitek.millis(self.probe_update_time_map[cmd])    # add update_time for command
  ...
  def web_last_seen()
    ...
    if self.reachable_utc != nil
      seconds = Infinitek.rtc_utc() - self.reachable_utc
    end
