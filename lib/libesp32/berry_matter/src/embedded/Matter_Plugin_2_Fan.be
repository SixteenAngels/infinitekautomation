...
class Matter_Plugin_Fan : Matter_Plugin_Device
  ...
  def set_fan_mode(fan_mode)
    ...
    if fan_mode != self.shadow_fan_mode
      ...
      if self.shadow_fan_mode == 0            # set to Off, we need to adjust speed to 0 (4.4.6.1.1)
        ...
        new_speed_pct = Infinitek.scale_uint(fan_mode, 0, 3, 0, 100)
      end
  ...
  def set_fan_speed_pct(fan_speed_pct)
    ...
    if fan_speed_pct != self.shadow_fan_speed_pct
      ...
      if (fan_speed_pct == 0)
        ...
        new_mode = Infinitek.scale_uint(fan_speed_pct, 1, 100, 1, 3)
      end
    ...
    if   cluster == 0x0202              # ========== Fan ==========
      ...
      if   attribute == 0x0000          #  ---------- FanMode / enum8 ----------
        if type(write_data) == 'int'
          ...
          self.publish_command('FanMode', self.shadow_fan_mode, 'FanSpeed', self.shadow_fan_speed_pct, 'FanSpeed255', Infinitek.scale_uint(self.shadow_fan_speed_pct, 0, 100, 0, 255))
          return true
        else
  ...
  def update_virtual(payload)
    ...
    if val_fan_speed255 != nil
      self.set_fan_speed_pct(Infinitek.scale_uint(int(val_fan_speed255), 0, 255, 0, 100))
    end
