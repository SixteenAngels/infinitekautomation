#
# Matter_UI.be - WebUI for Matter configuration in Infinitek
#
...
class Matter_UI
  ...
  def show_enable()
    ...
    webserver.content_send("<fieldset><legend><b>&nbsp;Matter &nbsp;</b></legend>"
                           "<p style='width:320px;'>Check the <a href='https://Infinitek.github.io/docs/Matter/' target='_blank'>Matter documentation</a>.</p>"
  ...
  def show_commissioning_info()
    ...
    var seconds_left = (self.device.commissioning.commissioning_open - Infinitek.millis()) / 1000
  ...
  def show_plugins_configuration()
    ...
    # Add remote endpoint
                           "<hr><p><b>Add Remote Infinitek or OpenBK</b></p>"
                           ...
                           "Auto-configure remote Infinitek</button></form><hr>"
      ...
      if webserver.has_arg("passcode") || webserver.has_arg("discriminator")
        ...
        if matter_enabled_requested != self.matter_enabled
          if matter_enabled_requested
            ...
            Infinitek.cmd("SetOption" + str(matter.MATTER_OPTION) + " 1")
          else
            log(format("MTR: /matterc received '%s' command", 'disable'), 3)
            Infinitek.cmd("SetOption" + str(matter.MATTER_OPTION) + " 0")
          end
          #- and force restart -#
          webserver.redirect("/?rst=")
