...
class Matter_Plugin_Sensor_Waterleak : Matter_Plugin_Sensor_Boolean
  ...
  def web_values()
    ...
    webserver.content_send(format("Waterleak%i %s", self.Infinitek_switch_index, self.web_value_onoff(self.shadow_bool_value)))
  end
  ...
  def web_values_prefix()
    ...
    if !name
      name = "Switch" + str(self.Infinitek_switch_index)
    end
