...
class Matter_Plugin
  ...
  # MVC Model
  #
  # Model linking the plugin to the Infinitek behavior
  #############################################################
...
# EVENT_CRITICAL=2
  ...
  def every_250ms()
    if self.update_next == nil
      ...
      if Infinitek.time_reached(self.update_next)
        if self.tick != self.device.tick
          self.update_shadow()                                # call update_shadow if not already called
        end
        self.update_next = Infinitek.millis(self.UPDATE_TIME)   # rearm timer
      end
    end
  end
