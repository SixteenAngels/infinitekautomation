...
class Matter_Plugin_Root : Matter_Plugin
  ...
  def init(device, endpoint, config)
    ...
    self.publish_event(0x0028, 0x00, matter.EVENT_CRITICAL, matter.TLV.Matter_TLV_item().set(matter.TLV.U4, Infinitek.version()))   # Event StartUp - Software Version
  ...
  def read_attribute(session, ctx, tlv_solo)
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- NetworkInterfaces ----------
        ...
        return tlv_solo.set(TLV.U2, Infinitek.cmd("Status 1", true)['StatusPRM']['BootCount'])
      elif attribute == 0x0002          #  ---------- UpTime u16 ----------
        return tlv_solo.set(TLV.U4, Infinitek.cmd("Status 11", true)['StatusSTS']['UptimeSec'])
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- UTCTime / epoch_us ----------
        var epoch_us = int64(Infinitek.rtc_utc()) * int64(1000000)
      ...
      # TODO add some missing args
        ...
        var epoch_us = int64(Infinitek.rtc('local')) * int64(1000000)
        return tlv_solo.set(TLV.U8, epoch_us)     # TODO test the conversion of int64()
      end
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- DataModelRevision / CommissioningWindowStatus ----------
        ...
        return tlv_solo.set(TLV.UTF1, "Infinitek")
        ...
        return tlv_solo.set(TLV.UTF1, Infinitek.cmd("DeviceName", true)['DeviceName'])
        ...
        return tlv_solo.set(TLV.UTF1, Infinitek.cmd("FriendlyName", true)['FriendlyName1'])
        ...
        return tlv_solo.set(TLV.UTF1, Infinitek.cmd("Status 2", true)['StatusFWR']['Hardware'])
      elif attribute == 0x0009          #  ---------- SoftwareVersion / u32 ----------
        return tlv_solo.set(TLV.U4, Infinitek.version())
      elif attribute == 0x000A          #  ---------- SoftwareVersionString / string ----------
        var version_full = Infinitek.cmd("Status 2", true)['StatusFWR']['Version']
        ...
        return tlv_solo.set(TLV.UTF1, Infinitek.wifi().find("mac", ""))
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- DefaultOTAProviders / list[ProviderLocationStruct] ----------
        ...
        return tlv_solo.set(TLV.BOOL, 0)  # we claim that update is not possible, would require to go to Infinitek UI
    ...
    # ====================================================================================================
      ...
      if   attribute == 0x0000          #  ---------- ActiveLocale / string ----------
        return tlv_solo.set(TLV.UTF1, Infinitek.locale())
      elif attribute == 0x0001          #  ---------- SupportedLocales / list[string] ----------
        var locl = TLV.Matter_TLV_array()
        locl.add_TLV(nil, TLV.UTF1, Infinitek.locale())
        return locl
      end
  ...
  def invoke_request(session, val, ctx)
    ...
    if   cluster == 0x0030              # ========== GeneralCommissioning cluster 11.9 p.627 ==========
      ...
      if   command == 0x0002            # ---------- CertificateChainRequest ----------
        ...
        att_elts.add_TLV(3, TLV.U4, Infinitek.rtc_utc())     # timestamp in epoch-s
        ...
        for fab: self.device.sessions.active_fabrics()
          if fab.get_fabric_index() == index
            ...
            Infinitek.set_timer(2000, def () self.device.remove_fabric(fab) end)
