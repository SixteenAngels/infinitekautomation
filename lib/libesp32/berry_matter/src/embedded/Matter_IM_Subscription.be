...
class Matter_IM_Subscription_Shop
  ...
  def every_50ms()
    ...
    while idx < size(self.subs)
      ...
      if (!sub.wait_status && Infinitek.time_reached(sub.not_before))           # is the update window open (not in progress and minimum time has passed)
    ...
    while idx < size(self.subs)
      var sub = self.subs[idx]
      if !sub.wait_status && Infinitek.time_reached(sub.expiration)
