...
class Matter_Device
  ...
  def init()
    import crypto
    if !Infinitek.get_option(matter.MATTER_OPTION)
    ...
    Infinitek.when_network_up(def () self.start() end)    # start when network is connected
  ...
  def read_sensors_scheduler()
    ...
    if (self.probe_sensor_timestamp == 0) || (Infinitek.time_reached(self.probe_sensor_timestamp))
      self._trigger_read_sensors()
      # set new next timestamp
      self.probe_sensor_timestamp = Infinitek.millis(self.probe_sensor_time)
    end
  end
  ...
  def _trigger_read_sensors()
    import json
    var rs_json = Infinitek.read_sensors()
    if Infinitek.loglevel(3)
  ...
  #
  # Called by Infinitek loop as a Infinitek driver.
  ...
  def register_commands()
    Infinitek.add_cmd("MtrJoin", /cmd_found, idx, payload, payload_json -> self.MtrJoin(cmd_found, idx, payload, payload_json))
    Infinitek.add_cmd("MtrUpdate", /cmd_found, idx, payload, payload_json -> self.MtrUpdate(cmd_found, idx, payload, payload_json))
    Infinitek.add_cmd("MtrInfo", /cmd_found, idx, payload, payload_json -> self.MtrInfo(cmd_found, idx, payload, payload_json))
  end
  ...
  def MtrUpdate(cmd_found, idx, payload, payload_json)
    if payload_json == nil    return Infinitek.resp_cmnd_str("Invalid JSON")    end

    var key_ep = Infinitek.find_key_i(payload_json, 'Ep')
    var key_name = Infinitek.find_key_i(payload_json, 'Name')
    if key_ep || key_name
      var pl = nil                  # plugin instance

      if key_ep
        var ep = int(payload_json[key_ep])
        if ep <= 0  return Infinitek.resp_cmnd_str("Invalid 'Ep' attribute")         end
      ...
      if (pl == nil)          return Infinitek.resp_cmnd_str("Invalid Device")          end
      if (!pl.VIRTUAL)        return Infinitek.resp_cmnd_str("Device is not virtual")   end
      ...
      for k: payload_json.keys()
        var cleaned_command_idx = Infinitek.find_list_i(uc, k)
        if (cleaned_command_idx == nil)
          Infinitek.resp_cmnd_str(f"Invalid attribute '{k}'")
          return
        end
      ...
      if state_json
        ...
        return Infinitek.resp_cmnd(cmnd_status)
      else
        return Infinitek.resp_cmnd_done()
      end
    end

    Infinitek.resp_cmnd_str("Missing 'Device' attribute")
  end
  ...
  def MtrInfo_one(ep)
    ...
    if state_json
      ...
      # publish
      # Infinitek.publish_rule(mtr_info)
      Infinitek.publish_result(mtr_info, "")
    end
  end
