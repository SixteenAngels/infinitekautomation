...
class Matter_Plugin_ShutterTilt : Matter_Plugin_Shutter
  ...
  def update_tilt_min_max()
    ...
    if r_st13.contains('StatusSHT')
      ...
      var d = r_st13.find("SHT"+str(self.Infinitek_shutter_index), {}).find('TiltConfig')
      if d != nil
    ...
    if   cluster == 0x0102              # ========== Window Covering 5.3 p.289 ==========
      ...
      if   attribute == 0x0007          #  ---------- ConfigStatus / u8 ----------
        ...
        if self.tilt_min != nil && self.tilt_max != nil
          var tilt_percentage = Infinitek.scale_uint(self.shadow_shutter_tilt - self.tilt_min, 0, self.tilt_max - self.tilt_min, 0, 10000)
    ...
    if   cluster == 0x0102              # ========== Window Covering 5.3 p.289 ==========
      ...
      if   command == 0x0008            # ---------- GoToTiltPercentage ----------
        ...
        if tilt != nil
          self.update_tilt_min_max()
          if self.tilt_min != nil && self.tilt_max !=  nil
            var tilt_val = self.tilt_min + Infinitek.scale_uint(tilt, 0, 10000, 0, self.tilt_max - self.tilt_min)
            Infinitek.cmd("ShutterTilt"+str(self.Infinitek_shutter_index+1) + " " + str(tilt_val), false)   # TODO
            self.update_shadow()
