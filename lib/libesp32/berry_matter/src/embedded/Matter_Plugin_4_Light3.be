...
class Matter_Plugin_Light3 : Matter_Plugin_Light1
  ...
  # var virtual                                       # (bool) is the device pure virtual (i.e. not related to a device implementation by Infinitek)
  ...
  def update_shadow()
    if !self.VIRTUAL && !self.BRIDGE
      ...
      if light_status != nil
        ...
        if hue != nil     hue = Infinitek.scale_uint(hue, 0, 360, 0, 254)   else hue = self.shadow_hue      end
        if sat != nil     sat = Infinitek.scale_uint(sat, 0, 255, 0, 254)   else sat = self.shadow_sat      end
  ...
  def set_hue_sat(hue_254, sat_254)
    ...
    if self.BRIDGE
      if hue_254 != nil
        var hue_360 = Infinitek.scale_uint(hue_254, 0, 254, 0, 360)
      ...
      if sat_254 != nil
        var sat_100 = Infinitek.scale_uint(sat_254, 0, 254, 0, 100)
      ...
      var hue_360 = (hue_254 != nil) ? Infinitek.scale_uint(hue_254, 0, 254, 0, 360) : nil
      var sat_255 = (sat_254 != nil) ? Infinitek.scale_uint(sat_254, 0, 254, 0, 255) : nil
  ...
  def parse_status(data, index)
    ...
    if index == 11                              # Status 11
      ...
      if hsb
        ...
        if hue != nil     hue = Infinitek.scale_uint(hue, 0, 360, 0, 254)   else hue = self.shadow_hue      end
        if sat != nil     sat = Infinitek.scale_uint(sat, 0, 100, 0, 254)   else sat = self.shadow_sat      end
        if hue != self.shadow_hue   self.attribute_updated(0x0300, 0x0000)   self.shadow_hue = hue   end
        if sat != self.shadow_sat   self.attribute_updated(0x0300, 0x0001)   self.shadow_sat = sat   end
      end
    end
  ...
  def web_value_RGB()
    if self.shadow_hue != nil && self.shadow_sat != nil
      ...
      l.set_huesat(Infinitek.scale_uint(self.shadow_hue, 0, 254, 0, 360), Infinitek.scale_uint(self.shadow_sat, 0, 254, 0, 255))
