#
# Matter_Plugin_Sensor_GenericSwitch_Btn.be - implements the behavior for a Generic Switch mapped to a Infinitek Button
#
...
class Matter_Plugin_Sensor_GenericSwitch_Btn : Matter_Plugin_Device
  ...
  var Infinitek_switch_index                          # Switch number in Infinitek (one based)
  var shadow_position
  ...
  def parse_configuration(config)
    ...
    self.Infinitek_switch_index = int(config.find(self.ARG #-'relay'-#, 1))
    if self.Infinitek_switch_index <= 0    self.Infinitek_switch_index = 1    end
  end
  ...
  def button_handler(button, mode, state, press_counter)
    # if Infinitek.loglevel(3) && (mode != 0)      # only if actual action
