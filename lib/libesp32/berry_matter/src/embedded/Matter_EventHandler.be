...
class Matter_EventQueued
  ...
  def init(event_no, endpoint, cluster, event_id, is_urgent, priority, data0, data1, data2)
    ...
    # epoch_time
    self.epoch_timestamp = Infinitek.rtc('utc')
    if (self.epoch_timestamp < 1700000000)
      self.epoch_timestamp = Infinitek.rtc('config_time')    # no valid time, take the last config time
    end
    self.data0 = data0
    self.data1 = data1
    self.data2 = data2
...
class Matter_EventHandler
  ...
  #####################################################################
  def dump()
    Infinitek.log(f"MTR: Events queues sizes: critical {size(self.queue_critical)}, info {size(self.queue_info)}, debug {size(self.queue_debug)}", 2)
    ...
    Infinitek.log(f"MTR: Events by types: critical {cnt[2]}, info {cnt[1]}, debug {cnt[0]}", 2)
  end
