...
class Matter_Plugin_Light0 : Matter_Plugin_Device
  ...
  var Infinitek_relay_index                             # Relay number in Infinitek (1 based), may be nil for Lights 1/2/3 internal
  ...
  def update_shadow()
    if !self.VIRTUAL && !self.BRIDGE
      if (self.Infinitek_relay_index != nil)
        var pow = Infinitek.get_power(self.Infinitek_relay_index - 1)
        if pow != nil
  ...
  def set_onoff(pow)
    if self.BRIDGE
      var ret = self.call_remote_sync("Power" + str(self.Infinitek_relay_index), pow ? "1" : "0")
      if ret != nil
      ...
      if (self.Infinitek_relay_index != nil)
        Infinitek.set_power(self.Infinitek_relay_index - 1, bool(pow))
        self.update_shadow()
      else
        import light
  ...
  def parse_status(data, index)
    if index == 11                              # Status 11
      ...
      if self.Infinitek_relay_index == 1 && data.contains("POWER")        # special case, can be `POWER` or `POWER1`
        state = (data.find("POWER") == "ON")
      else
        state = (data.find("POWER" + str(self.Infinitek_relay_index)) == "ON")
      end
  ...
  def web_values_prefix()
    ...
    if !name
      name = "Power" + str(self.Infinitek_relay_index)
    end
