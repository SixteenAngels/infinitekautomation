...
class Matter_TCP_async
  ...
  def begin()
    ...
    self.time_start = Infinitek.millis()
    self.status = 0

    if (self.tcp.connect(self.addr, self.port))
      if self.fast_loop
        Infinitek.remove_fast_loop(self.fast_loop)
        Infinitek.add_fast_loop(self.fast_loop)
      else
  ...
  def loop()
    ...
    if prev_connected == nil                          # not yet connected
      if self.tcp_connected == true
        ...
        self.close()
        return
      elif (Infinitek.millis() - self.time_start) > self.timeout
        # connection timeout
        self.status = -3
    ...
    if (Infinitek.millis() - self.time_start) > self.timeout
      self.close()
      self.status = -3
      self.event_timeout()
      return
