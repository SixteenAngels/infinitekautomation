...
class Matter_UDPPacket_sent
  ...
  def init(msg)
    ...
    # other information
    self.retries = 0
    self.next_try = Infinitek.millis() + matter.UDPServer._backoff_time(self.retries)
  end
...
class Matter_UDPServer
  ...
  #############################################################
  # Starts the server.
  # Registers as device handler to Infinitek
  #
  ...
  def start(cb)
    if !self.listening
      ...
      Infinitek.add_fast_loop(self.loop_cb)
    end
  end
  ...
  def stop()
    if self.listening
      ...
      Infinitek.remove_fast_loop(self.loop_cb)
    end
  end
  ...
  def _resend_packets()
    ...
    while (idx < size(self.packets_sent)) && (idx < 4)              # limit to 4 packets in output queue
      ...
      if Infinitek.time_reached(packet.next_try)
        if packet.retries <= self.RETRIES
          log(f"MTR: .          Resending packet id={packet.msg_id} {packet.retries=}", 3)
          self.send(packet)
          packet.next_try = Infinitek.millis() + self._backoff_time(packet.retries)
          packet.retries += 1
          idx += 1
        else
