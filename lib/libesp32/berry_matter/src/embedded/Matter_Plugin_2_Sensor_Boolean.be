...
class Matter_Plugin_Sensor_Boolean : Matter_Plugin_Device
  ...
  def parse_configuration(config)
    ...
    self.Infinitek_switch_index = int(config.find(self.ARG #-'switch'-#, 1))
  ...
  def update_shadow()
    ...
    if !self.VIRTUAL
      var switch_str = "Switch" + str(self.Infinitek_switch_index)

      var j = Infinitek.cmd("Status 10", true)
  ...
  def parse_status(data, index)
    if index == 10                             # Status 10
      ...
      state = (data.find("Switch" + str(self.Infinitek_switch_index)) == "ON")
