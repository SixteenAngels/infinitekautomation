...
class Matter_IM_Message
  ...
  def reset(msg, opcode, reliable)
    ...
    self.expiration = Infinitek.millis() + self.MSG_TIMEOUT
  ...
  def ack_received(msg)
    ...
    if self.finishing                 # if finishing, we are waiting for final Ack before removing from queue
      ...
      self.expiration = Infinitek.millis() + self.MSG_TIMEOUT     # else give more time to the timer
    end
  ...
  def status_ok_received(msg)
    ...
    self.expiration = Infinitek.millis() + self.MSG_TIMEOUT     # give more time
    if msg
