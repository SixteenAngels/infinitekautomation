...
class Matter_Plugin_Sensor : Matter_Plugin_Device
  ...
  var Infinitek_sensor_filter                         # Rule-type filter to the value, like "ESP32#Temperature"
  var Infinitek_sensor_matcher                        # Actual matcher object
  ...
  def parse_configuration(config)
    ...
    self.Infinitek_sensor_filter = config.find(self.ARG#-'filter'-#)
    if self.Infinitek_sensor_filter
      self.Infinitek_sensor_matcher = Infinitek.Rule_Matcher.parse(self.Infinitek_sensor_filter)
    end
  ...
  # parse sensor
  #
  # The device calls regularly `Infinitek.read_sensors()` and converts
  # it to json.
  def parse_sensors(payload)
    if !self.VIRTUAL && self.Infinitek_sensor_matcher
      var val = self.Infinitek_sensor_matcher.match(payload)
  ...
  def parse_status(data, index)
    if index == 10                               # Status 10
      ...
      if self.Infinitek_sensor_matcher
        var val = self.pre_value(real(self.Infinitek_sensor_matcher.match(data)))
        if val != nil
  ...
  def filter_name_html()
    if self.Infinitek_sensor_filter
      import string
      import webserver
      return webserver.html_escape(string.split(self.Infinitek_sensor_filter, '#')[0])
    end
    return ""
  end
