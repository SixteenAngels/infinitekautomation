  ...
  def paint(frame)
    ...
    if (slew_size > 0)
      ...
      while (i < pulse_max)
        ...
        frame[i] = frame.blend(back_color, color, Infinitek.scale_int(i, pos - slew_size - 1, pos, 255, 0))
        # blend
        i += 1
      end
      # Slew 2
      ...
      while (i < pulse_max)
        # blend
        frame[i] = frame.blend(back_color, color, Infinitek.scale_int(i, pos + pulse_size - 1, pos + pulse_size + slew_size, 0, 255))
        i += 1
      end
    end
  ...
  def paint(frame)
    ...
    while (pos < pixel_size) && (nb_pulse != 0)    # we iterate on pos (by `period` ranges)
      ...
      if type(self.color) == 'int'
        ...
        self.color.get_color(Infinitek.scale_int(i, 0, pulse_size - 1, 0, 255))
      end
      pos += period
      nb_pulse -= 1
    end
