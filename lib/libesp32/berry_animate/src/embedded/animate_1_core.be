...
class Animate_core
  ...
  def set_strip_bri()
    self.strip.set_bri(Infinitek.scale_uint(self.bri, 0, 100, 0, 255))
  end
  ...
  def start()
    ...
    Infinitek.add_fast_loop(self.fast_loop_cb)
  end
  ...
  def stop()
    ...
    Infinitek.remove_fast_loop(self.fast_loop_cb)
  end
  ...
  def fast_loop()
    if self.running && Infinitek.time_reached(self.fast_loop_next) && self.strip.can_show()
      # prepare environnement
      self.frame.fill_pixels(self.back_color)
      # run animators first
      var i = 0
      var now = Infinitek.millis()
