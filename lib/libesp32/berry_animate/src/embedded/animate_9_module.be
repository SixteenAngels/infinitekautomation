...
class Animate_animator
  ...
  var origin            # origin in ms of the current cycle, as per Infinitek.millis() reference
  ...
  def start(millis)
    ...
    if millis == nil    millis = Infinitek.millis()   end
    self.origin = millis
    self.running = true
  end
...
class Animate_palette : Animate_animator
  ...
  def parse_palette(min, max)
    ...
    if self.palette.get(0, 1) != 0
      ...
      idx = 0
      while idx < slots
        arr[idx] = Infinitek.scale_int(cur_ticks, 0, total_ticks, min, max)
      ...
      while idx < slots
        ...
        arr[idx] = Infinitek.scale_int(val, 0, 255, min, max)
        idx += 1
      end
    end
    # print(f"ANM: {arr=}")
  ...
  def set_value(value)
    ...
    var scale_int = Infinitek.scale_int
  ...
  def animate(millis)
    ...
    var scale_uint = Infinitek.scale_uint
    if past >= duration_ms
    ...
    var bri = self.bri
    if bri != 100
      r = Infinitek.scale_uint(r, 0, 100, 0, bri)
      g = Infinitek.scale_uint(g, 0, 100, 0, bri)
      b = Infinitek.scale_uint(b, 0, 100, 0, bri)
    end
...
class Animate_oscillator : Animate_animator
  ...
  def animate(millis)
    ...
    duration_ms_mid = Infinitek.scale_uint(self.duty_cycle, 0, 100, 0, duration_ms)
    if past >= duration_ms
      self.origin += (past / duration_ms) * duration_ms
      past = past % duration_ms
      # handle end of cycle
    ...
    if self.phase > 0
      past_with_phase += Infinitek.scale_uint(self.phase, 0, 100, 0, duration_ms)
      if (past_with_phase > duration_ms)    past_with_phase -= duration_ms    end   # if overflow, take modulus
    end

    if   self.form == 1 #-SAWTOOTH-#
      value = Infinitek.scale_int(past_with_phase, 0, duration_ms - 1, a, b)
    elif self.form == 2 #-TRIANGLE-#
      if past_with_phase < duration_ms_mid
        value = Infinitek.scale_int(past_with_phase, 0, duration_ms_mid - 1, a, b)
      else
        value = Infinitek.scale_int(past_with_phase, duration_ms_mid, duration_ms - 1, b, a)
      end
      ...
      var angle = Infinitek.scale_int(past_with_phase, 0, duration_ms - 1, 0, 32767)
      var x = Infinitek.sine_int(angle - 8192)   # -4096 .. 4096, dephase from cosine to sine
      value = Infinitek.scale_int(x, -4096, 4096, a, b)
    end
    self.value = value
