...
class BounceAnimation : animation.animation
  ...
  def _initialize_buffers()
    ...
    var pixels_per_second = Infinitek.scale_uint(self.bounce_speed, 0, 255, 0, 20)
  ...
  def on_param_changed(name, value)
    ...
    if name == "bounce_speed"
      ...
      var pixels_per_second = Infinitek.scale_uint(value, 0, 255, 0, 20)
  ...
  def _update_physics(dt_ms)
    ...
    if current_gravity > 0
      var gravity_accel = Infinitek.scale_uint(current_gravity, 0, 255, 0, 1000)  # pixels/sec²
    ...
    if bounced && current_damping < 255
      var damping_factor = Infinitek.scale_uint(current_damping, 0, 255, 0, 255)
      self.current_velocity = Infinitek.scale_uint(self.current_velocity, 0, 255, 0, damping_factor)
      if self.current_velocity < 0
        self.current_velocity = -Infinitek.scale_uint(-self.current_velocity, 0, 255, 0, damping_factor)
      end
    end
