...
class PaletteWaveAnimation : PalettePatternAnimation
  ...
  def _update_value_buffer(time_ms)
    ...
    var position = Infinitek.scale_uint(time_ms % wave_period, 0, wave_period, 0, 1000) / 1000.0
    ...
    while i < strip_length
      ...
      var angle = Infinitek.scale_uint(pos_in_wave, 0, wave_length, 0, 32767)  # 0 to 2π in fixed-point
      var sine_value = Infinitek.sine_int(angle)  # -4096 to 4096
      
      # Map sine value from -4096..4096 to 0..255
      var byte_value = Infinitek.scale_int(sine_value, -4096, 4096, 0, 255)
      self.value_buffer[i] = byte_value
      i += 1
    end
  end
...
class PaletteGradientAnimation : PalettePatternAnimation
  ...
  def _update_value_buffer(time_ms)
    ...
    if shift_period > 0
      var temporal_position = Infinitek.scale_uint(time_ms % shift_period, 0, shift_period, 0, 1000) / 1000.0
      temporal_offset = temporal_position * effective_spatial_period
    end
    
    # Calculate the phase shift offset in pixels
    var phase_offset = Infinitek.scale_uint(phase_shift, 0, 100, 0, effective_spatial_period)
    ...
    while i < strip_length
      ...
      var byte_value = Infinitek.scale_uint(int(spatial_pos), 0, effective_spatial_period - 1, 0, 255)
      self.value_buffer[i] = byte_value
      i += 1
    end
  end
...
class PaletteMeterAnimation : PalettePatternAnimation
  ...
  def _update_value_buffer(time_ms)
    ...
    var meter_position = Infinitek.scale_uint(current_value, 0, 100, 0, strip_length)
