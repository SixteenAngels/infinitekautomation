...
class GradientAnimation : animation.animation
  ...
  def update(time_ms)
    ...
    if movement_speed > 0
      ...
      var cycles_per_second = Infinitek.scale_uint(movement_speed, 0, 255, 0, 10)
  ...
  def _calculate_gradient(time_ms)
    ...
    while i < strip_length
      ...
      if color_param == nil
        ...
        var hue = Infinitek.scale_uint(gradient_pos, 0, 255, 0, 359)
        import light_state
        ...
        var r = Infinitek.scale_uint(intensity, 0, 255, 0, (color_param >> 16) & 0xFF)
        var g = Infinitek.scale_uint(intensity, 0, 255, 0, (color_param >> 8) & 0xFF)
        var b = Infinitek.scale_uint(intensity, 0, 255, 0, color_param & 0xFF)
  ...
  def _calculate_linear_position(pixel, strip_length)
    var strip_pos = Infinitek.scale_uint(pixel, 0, strip_length - 1, 0, 255)
    ...
    if direction <= 128
      ...
      var start_offset = Infinitek.scale_uint(direction, 0, 128, 0, 128)
      strip_pos = (strip_pos + start_offset) % 256
    else
      # Reverse direction
      var reverse_amount = Infinitek.scale_uint(direction, 128, 255, 0, 255)
      strip_pos = 255 - ((strip_pos + reverse_amount) % 256)
    end
    
    # Apply spread (compress or expand the gradient)
    strip_pos = Infinitek.scale_uint(strip_pos, 0, 255, 0, spread)
  ...
  def _calculate_radial_position(pixel, strip_length)
    ...
    distance = Infinitek.scale_uint(distance, 0, 128, 0, spread)
    if distance > 255
      distance = 255
    end
