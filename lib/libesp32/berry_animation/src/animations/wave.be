...
class WaveAnimation : animation.animation
    ...
    while i < 256
      ...
      if current_wave_type == 0
        ...
        if i < 64
          ...
          value = Infinitek.scale_uint(quarter, 0, 64, 128, 255)
        elif i < 128
          # Second quarter: mirror first quarter
          value = Infinitek.scale_uint(128 - i, 0, 64, 128, 255)
        elif i < 192
          # Third quarter: negative first quarter
          value = Infinitek.scale_uint(i - 128, 0, 64, 128, 0)
        else
          # Fourth quarter: negative second quarter
          value = Infinitek.scale_uint(256 - i, 0, 64, 128, 0)
        end
      elif current_wave_type == 1
        # Triangle wave
        if i < 128
          value = Infinitek.scale_uint(i, 0, 128, 0, 255)
        else
          value = Infinitek.scale_uint(256 - i, 0, 128, 0, 255)
        end
  ...
  def update(time_ms)
    ...
    if current_wave_speed > 0
      ...
      var cycles_per_second = Infinitek.scale_uint(current_wave_speed, 0, 255, 0, 10)
  ...
  def _calculate_wave(time_ms)
    ...
    while i < strip_length
      ...
      var x = Infinitek.scale_uint(i, 0, strip_length - 1, 0, 255)
      ...
      var scaled_amplitude = Infinitek.scale_uint(current_amplitude, 0, 255, 0, 128)
      var final_value = 0
      
      if wave_value >= 128
        # Upper half of wave
        var upper_amount = wave_value - 128
        upper_amount = Infinitek.scale_uint(upper_amount, 0, 127, 0, scaled_amplitude)
        final_value = current_center_level + upper_amount
      else
        # Lower half of wave
        var lower_amount = 128 - wave_value
        lower_amount = Infinitek.scale_uint(lower_amount, 0, 128, 0, scaled_amplitude)
      ...
      if final_value > 10  # Threshold to avoid very dim colors
        ...
        if animation.is_color_provider(current_color) && current_color.get_color_for_value != nil
          ...
          r = Infinitek.scale_uint(final_value, 0, 255, 0, r)
          g = Infinitek.scale_uint(final_value, 0, 255, 0, g)
          b = Infinitek.scale_uint(final_value, 0, 255, 0, b)
