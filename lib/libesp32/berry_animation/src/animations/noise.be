...
class NoiseAnimation : animation.animation
  ...
  def _noise_1d(x)
    ...
    var lerp_amount = Infinitek.scale_uint(int(fx * 256), 0, 256, 0, 255)
    return Infinitek.scale_uint(lerp_amount, 0, 255, a, b)
  end
  ...
  def _fractal_noise(x, time_offset)
    ...
    while octave < current_octaves
      var sample_x = Infinitek.scale_uint(x * frequency, 0, 255 * 255, 0, 255) + time_offset
      var noise_val = self._noise_1d(sample_x)
      
      value += Infinitek.scale_uint(noise_val, 0, 255, 0, amplitude)
      max_value += amplitude
      
      amplitude = Infinitek.scale_uint(amplitude, 0, 255, 0, current_persistence)
    ...
    if max_value > 0
      value = Infinitek.scale_uint(value, 0, max_value, 0, 255)
    end
  ...
  def update(time_ms)
    ...
    if current_speed > 0
      ...
      var units_per_second = Infinitek.scale_uint(current_speed, 0, 255, 0, 5)
