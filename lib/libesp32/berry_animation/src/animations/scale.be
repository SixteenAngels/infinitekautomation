...
class ScaleAnimation : animation.animation
  ...
  def update(time_ms)
    ...
    if current_scale_speed > 0 && current_scale_mode > 0
      ...
      var cycles_per_second = Infinitek.scale_uint(current_scale_speed, 0, 255, 0, 2)
  ...
  def _get_current_scale_factor()
    ...
    if current_scale_mode == 0
      ...
      return Infinitek.scale_uint(sine_val, 0, 255, 64, 255)  # 0.5x to 2.0x
    elif current_scale_mode == 2
      # Grow from 0.5x to 2.0x
      return Infinitek.scale_uint(self.scale_phase, 0, 255, 64, 255)
    else
      # Shrink from 2.0x to 0.5x
      return Infinitek.scale_uint(255 - self.scale_phase, 0, 255, 64, 255)
    end
  end
  ...
  def _sine(angle)
    ...
    if angle < 64
      return Infinitek.scale_uint(quarter, 0, 64, 128, 255)
    elif angle < 128
      return Infinitek.scale_uint(128 - angle, 0, 64, 128, 255)
    elif angle < 192
      return Infinitek.scale_uint(angle - 128, 0, 64, 128, 0)
    else
      return Infinitek.scale_uint(256 - angle, 0, 64, 128, 0)
    end
  end
  ...
  def _calculate_scale()
    ...
    var center_pixel = Infinitek.scale_uint(current_scale_center, 0, 255, 0, current_strip_length - 1)
    ...
    while i < current_strip_length
      ...
      var scaled_distance = Infinitek.scale_uint(distance_from_center * 128, 0, 128 * 128, 0, current_scale * 128) / 128
