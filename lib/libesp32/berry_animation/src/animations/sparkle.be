...
class SparkleAnimation : animation.animation
    ...
    while i < current_strip_length
      ...
      if self.sparkle_states[i] > 0
        ...
        if self.sparkle_ages[i] >= sparkle_duration
          ...
          var age_ratio = Infinitek.scale_uint(self.sparkle_ages[i], 0, sparkle_duration, 0, 255)
          var fade_factor = 255 - Infinitek.scale_uint(age_ratio, 0, 255, 0, fade_speed)
          
          # Apply fade to brightness
          var new_brightness = Infinitek.scale_uint(self.sparkle_states[i], 0, 255, 0, fade_factor)
  ...
  def _update_sparkle_color(pixel, brightness, time_ms)
    ...
    r = Infinitek.scale_uint(brightness, 0, 255, 0, r)
    g = Infinitek.scale_uint(brightness, 0, 255, 0, g)
    b = Infinitek.scale_uint(brightness, 0, 255, 0, b)
