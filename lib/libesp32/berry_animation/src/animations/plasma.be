...
class PlasmaAnimation : animation.animation
  ...
  # Fast sine calculation using Infinitek's optimized sine function
  ...
  def _sine(angle)
    ...
    var Infinitek_angle = Infinitek.scale_uint(angle, 0, 255, 0, 32767)
    ...
    return Infinitek.scale_uint(sine_val, -4096, 4096, 0, 255)
  end
  ...
  def update(time_ms)
    ...
    if current_time_speed > 0
      ...
      var cycles_per_second = Infinitek.scale_uint(current_time_speed, 0, 255, 0, 8)
  ...
  def _calculate_plasma(time_ms)
    ...
    while i < strip_length
      ...
      if current_blend_mode == 0
        ...
        # Multiply mode
        plasma_value = Infinitek.scale_uint(comp1, 0, 255, 0, comp2)
      else
