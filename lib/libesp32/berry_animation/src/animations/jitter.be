...
class JitterAnimation : animation.animation
  ...
  def update(time_ms)
    ...
    if jitter_frequency > 0
      ...
      var hz = Infinitek.scale_uint(jitter_frequency, 0, 255, 0, 30)
  ...
  def _update_jitter()
    ...
    var max_offset = Infinitek.scale_uint(jitter_intensity, 0, 255, 0, 10)
  ...
  def _calculate_jitter()
    ...
    while i < current_strip_length
      ...
      if jitter_type == 0 || jitter_type == 3
        # Position jitter
        var jitter_pixels = Infinitek.scale_uint(self.jitter_offsets[i], -10, 10, -position_range / 10, position_range / 10)
  ...
  def _apply_color_jitter(color, pixel_index)
    ...
    if jitter_type == 1 || jitter_type == 3
      ...
      var color_jitter = Infinitek.scale_uint(color_range, 0, 255, 0, 30)
    ...
    if jitter_type == 2 || jitter_type == 3
      ...
      var brightness_jitter = Infinitek.scale_uint(brightness_range, 0, 255, 0, 50)
      ...
      r = Infinitek.scale_uint(r, 0, 255, 0, brightness_factor)
      g = Infinitek.scale_uint(g, 0, 255, 0, brightness_factor)
      b = Infinitek.scale_uint(b, 0, 255, 0, brightness_factor)
    end
