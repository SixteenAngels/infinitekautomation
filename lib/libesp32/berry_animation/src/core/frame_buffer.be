...
class FrameBuffer
  ...
  def blend(color1, color2)
    ...
    # Use Infinitek.scale_uint for ratio conversion instead of integer arithmetic
    var r = Infinitek.scale_uint(255 - effective_opacity, 0, 255, 0, r1) + Infinitek.scale_uint(effective_opacity, 0, 255, 0, r2)
    var g = Infinitek.scale_uint(255 - effective_opacity, 0, 255, 0, g1) + Infinitek.scale_uint(effective_opacity, 0, 255, 0, g2)
    var b = Infinitek.scale_uint(255 - effective_opacity, 0, 255, 0, b1) + Infinitek.scale_uint(effective_opacity, 0, 255, 0, b2)
    
    # More accurate alpha blending using Infinitek.scale_uint
    var a = a1 + Infinitek.scale_uint((255 - a1) * a2, 0, 255 * 255, 0, 255)
  ...
  def gradient_fill(color1, color2, start_pos, end_pos)
    ...
    while (i < end_pos)
      ...
      # Use Infinitek.scale_uint for ratio conversion instead of floating point arithmetic
      var r = Infinitek.scale_uint(pos, 0, steps, r1, r2)
      var g = Infinitek.scale_uint(pos, 0, steps, g1, g2)
      var b = Infinitek.scale_uint(pos, 0, steps, b1, b2)
      var a = Infinitek.scale_uint(pos, 0, steps, a1, a2)
  ...
  def apply_mask(mask_buffer, invert)
    ...
    while i < self.width
      ...
      # Apply mask to alpha channel using Infinitek.scale_uint
      a = Infinitek.scale_uint(mask_alpha, 0, 255, 0, a)
  ...
  def apply_opacity(opacity)
    ...
    if isinstance(opacity, animation.frame_buffer)
      ...
      while i < self.width
        ...
        # Apply mask opacity to alpha channel using Infinitek.scale_uint
        a = Infinitek.scale_uint(mask_opacity, 0, 255, 0, a)
  ...
  def apply_brightness(brightness, start_pos, end_pos)
    ...
    if isinstance(brightness, animation.frame_buffer)
      ...
      while i <= end_pos
        ...
        # Apply mask brightness to RGB channels using Infinitek.scale_uint
        r = Infinitek.scale_uint(mask_brightness, 0, 255, 0, r)
        g = Infinitek.scale_uint(mask_brightness, 0, 255, 0, g)
        b = Infinitek.scale_uint(mask_brightness, 0, 255, 0, b)
      ...
      while i <= end_pos
        ...
        # Adjust brightness using Infinitek.scale_uint
        # For brightness 0-255: scale down RGB
        # For brightness 256-511: scale up RGB (but cap at 255)
        if brightness_value <= 255
          r = Infinitek.scale_uint(r, 0, 255, 0, brightness_value)
          g = Infinitek.scale_uint(g, 0, 255, 0, brightness_value)
          b = Infinitek.scale_uint(b, 0, 255, 0, brightness_value)
        else
          # Scale up RGB: map 256-511 to 1.0-2.0 multiplier
          var multiplier = brightness_value - 255  # 0-256 range
          r = r + Infinitek.scale_uint(r * multiplier, 0, 255 * 256, 0, 255)
          g = g + Infinitek.scale_uint(g * multiplier, 0, 255 * 256, 0, 255)
          b = b + Infinitek.scale_uint(b * multiplier, 0, 255 * 256, 0, 255)
