...
class OscillatorValueProvider : animation.value_provider
  ...
  def produce_value(name, time_ms)
    ...
    var duration_ms_mid = Infinitek.scale_uint(duty_cycle, 0, 100, 0, duration)
    ...
    # Apply phase shift
    if phase > 0
      past_with_phase += Infinitek.scale_uint(phase, 0, 100, 0, duration)
    ...
    if form == animation.SAWTOOTH
      self.value = Infinitek.scale_int(past_with_phase, 0, duration - 1, min_value, max_value)
    elif form == animation.TRIANGLE
      if past_with_phase < duration_ms_mid
        self.value = Infinitek.scale_int(past_with_phase, 0, duration_ms_mid - 1, min_value, max_value)
      else
        self.value = Infinitek.scale_int(past_with_phase, duration_ms_mid, duration - 1, max_value, min_value)
      end
      ...
      var angle = Infinitek.scale_uint(past_with_phase, 0, duration - 1, 0, 32767)
      var x = Infinitek.sine_int(angle - 8192)   # -4096 .. 4096, dephase from cosine to sine
      self.value = Infinitek.scale_int(x, -4096, 4096, min_value, max_value)
    elif form == animation.SINE
      # Map timing to 0..32767 for sine calculation
      var angle = Infinitek.scale_uint(past_with_phase, 0, duration - 1, 0, 32767)
      var x = Infinitek.sine_int(angle)   # -4096 .. 4096, pure sine wave
      self.value = Infinitek.scale_int(x, -4096, 4096, min_value, max_value)
    elif form == animation.EASE_IN
      # Quadratic ease-in: starts slow, accelerates
      var t = Infinitek.scale_uint(past_with_phase, 0, duration - 1, 0, 255)  # 0..255
      var eased = (t * t) / 255  # t^2 scaled back to 0..255
      self.value = Infinitek.scale_int(eased, 0, 255, min_value, max_value)
      ...
      if t == 0
        ...
        var decay = Infinitek.scale_uint(255 - t, 0, 255, 255, 32)  # Exponential decay approximation
        var freq_angle = Infinitek.scale_uint(t, 0, 255, 0, 32767 * 6)  # High frequency oscillation
        var oscillation = Infinitek.sine_int(freq_angle % 32767)  # -4096 to 4096
        var elastic_offset = (oscillation * decay) / 4096  # Scale oscillation by decay
        var base_progress = Infinitek.scale_int(t, 0, 255, 0, max_value - min_value)
      ...
      if t < 128  # First big bounce (0-50% of time)
        ...
        var segment_t = Infinitek.scale_uint(t - 192, 0, 63, 0, 255)
        var bounce_val = 255 - ((255 - segment_t) * (255 - segment_t)) / 255
        bounced_t = 255 - ((255 - bounce_val) * 64) / 255  # Settle towards full value
      end
      
      self.value = Infinitek.scale_int(bounced_t, 0, 255, min_value, max_value)
    end
