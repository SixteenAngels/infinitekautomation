...
class BreatheColorProvider : animation.oscillator_value
  ...
  def produce_value(name, time_ms)
    ...
    if current_curve_factor > 1
      ...
      var curve_input = Infinitek.scale_uint(normalized_value, 0, 255, 0, 8192)
      ...
      curved_value = Infinitek.scale_uint(curve_input, 0, 8192, 0, 255)
    end
    
    # Now map the curved value to the brightness range
    var brightness = Infinitek.scale_uint(curved_value, 0, 255, self.min_brightness, self.max_brightness)
