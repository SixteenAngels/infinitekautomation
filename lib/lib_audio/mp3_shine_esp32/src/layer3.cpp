...
static unsigned char *shine_encode_buffer_internal(shine_global_config *config, int *written, int stride) {
  counter[0] = portGET_RUN_TIME_COUNTER_VALUE();      // Infinitek more portable solution
  ...
  counter[1] = portGET_RUN_TIME_COUNTER_VALUE();      // Infinitek more portable solution
  // counter[1] = xthal_get_ccount();
  /* apply mdct to the polyphase output */
  // put on core 1
  shine_mdct_sub(config, stride);
  counter[2] = portGET_RUN_TIME_COUNTER_VALUE();      // Infinitek more portable solution
  // counter[2] = xthal_get_ccount();
  /* bit and noise allocation */
  //put on core 0
  shine_iteration_loop(config);
  counter[3] = portGET_RUN_TIME_COUNTER_VALUE();      // Infinitek more portable solution
  // counter[3] = xthal_get_ccount();
  /* write the frame to the bitstream */
  shine_format_bitstream(config);
  counter[4] = portGET_RUN_TIME_COUNTER_VALUE();      // Infinitek more portable solution
