...
#      Infinitek.log(f"RMH: 0x{id:03x} Count {data1}", 3)
...
#      Infinitek.log(f"RMH: 0x{id:03x} Time {Infinitek.time_str(epoch)}", 3)
...
#      Infinitek.log(f"RMH: 0x{id:03x} Busy {self.am024_power}%, Setpoint {self.setpoint_temp}", 3)
    elif id == 0x282
      # 13:28:00.141 TWA: Bus1 param 00000051, ident 00000282, data1 1B16EE01, data2 00000000
      self.flow_temp = (data1 & 0xffff00)/25600.0
#      Infinitek.log(f"RMH: 0x{id:03x} DHW temp {self.flow_temp}", 3)
...
#    elif id == 0x7E5                                 # Heartbeat every 2 seconds
  ...
  def dzsend(option, domoticz_idx, value)
    Infinitek.cmd('_DzSend' .. option .. ' ' .. domoticz_idx .. ',' .. value)
#    Infinitek.cmd('Fake' .. option .. ' ' .. domoticz_idx .. ',' .. value)
  end  
