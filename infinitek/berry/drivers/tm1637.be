#-
Simplified Infinitek TM1637 driver written in Berry
...
Add custom commands to the native Infinitek console:
...
def tm_add_custom_commands(dio, clk)
    ...
    tm.clear()
    tm.set_on(4)
    Infinitek.add_cmd('tmprint', def(cmd, idx, payload)
        tm.print(payload) ? Infinitek.resp_cmnd_done() : Infinitek.resp_cmnd_failed()
    end)
    #  0-8 range, 0 to 'OFF'
    Infinitek.add_cmd('tmbrightness', def(cmd, idx, payload)
        tm.set_on(int(payload)) ? Infinitek.resp_cmnd_done() : Infinitek.resp_cmnd_failed()
    end)
    log("Infinitek custom commands registered: TmPrint, TmBrightness")
end
