...
class Core200S
    ...
    def init_cmds()
        if self.init_counter == 7
            ...
            self.set_wifi_led(Infinitek.wifi("up"))
    ...
    def json_append()
        Infinitek.response_append(",\"C200S\":{")

        Infinitek.response_append(f"\"FanSpeedMode\":\"{self.fan_speed_mode}\"")
        Infinitek.response_append(f",\"FanMode\":\"{self.fan_mode}\"")
        Infinitek.response_append(f",\"NightLightMode\":\"{self.nl_mode}\"")
        Infinitek.response_append(f",\"ChildLockMode\":\"{self.cl_mode}\"")
        Infinitek.response_append(f",\"SleepMode\":\"{self.sl_mode}\"")
        Infinitek.response_append(f",\"DisplayMode\":\"{self.dp_mode}\"")
        Infinitek.response_append(f",\"DisplayAutoOff\":\"{self.dp_auto_off_mode}\"")
        Infinitek.response_append(f",\"TimerTotal\":{self.timer_total}")
        Infinitek.response_append(f",\"TimerRemaining\":{self.timer_remaining}")

        Infinitek.response_append("}")
    end
    ...
    def web_sensor()
        ...
        if self.timer_total != 0
            ...
            Infinitek.web_send_decimal(timer_msg)
        else
            Infinitek.web_send_decimal("{s}Timer{m}not active{e}")
        end
    end
...
Infinitek.add_driver(c200s)

# valid payloads low, med, full, toggle
Infinitek.add_cmd('C2S_fan_speed', def(cmd, idx, payload)
    var res = false
    if string.startswith(payload, "toggle")
        ...
        Infinitek.resp_cmnd(format("{\"FanSpeedMode\": \"%s\"}", c200s.fan_speed_mode))
        return
    end
...
Infinitek.add_cmd('C2S_fan_mode', def(cmd, idx, payload)
    var res = false
    if string.startswith(payload, "toggle")
        ...
        Infinitek.resp_cmnd(format("{\"FanMode\": \"%s\"}", c200s.fan_mode))
        return
    end
...
# valid payloads on, off, toggle
Infinitek.add_cmd('C2S_child_lock', def(cmd, idx, payload)
    var res = false
    if string.startswith(payload, "toggle")
        ...
        Infinitek.resp_cmnd(format("{\"ChildLockMode\": \"%s\"}", c200s.cl_mode))
        return
    end
...
Infinitek.add_cmd('C2S_night_light', def(cmd, idx, payload)
    var res = false
    if string.startswith(payload, "toggle")
        ...
        Infinitek.resp_cmnd(format("{\"NightLightMode\": \"%s\"}", c200s.nl_mode))
        return
    end
...
# valid payloads on, off, toggle
Infinitek.add_cmd('C2S_sleep_mode', def(cmd, idx, payload)
    var res = false
    if string.startswith(payload, "toggle")
        ...
        Infinitek.resp_cmnd(format("{\"SleepMode\": \"%s\"}", c200s.sl_mode))
        return
    end
...
# valid payloads on, off, toggle
Infinitek.add_cmd('C2S_display_auto_off', def(cmd, idx, payload)
    var res = false
    if string.startswith(payload, "toggle")
        ...
        Infinitek.resp_cmnd(format("{\"DisplayAutoOff\": \"%s\"}", c200s.dp_auto_off_mode))
        return
    end
...
Infinitek.add_cmd('C2S_timer', def(cmd, idx, payload)
    if payload != ""
        ...
        Infinitek.resp_cmnd(format("{\"TimerTotal\":%d,\"TimerRemaining\":%d}", c200s.timer_total, c200s.timer_remaining))
    end
end)
