#-------------------------------------------------------------
 - Specialized driver for AXP192 of M5Stack Tough
 -------------------------------------------------------------#
class AXP192_M5Stack_Tough : AXP192
  def init()
    super(self).init()
    ...
    if self.wire
      ...
      #  Start time: 512ms
      #  Long press: 1s
      #  Set the automatic shutdown function when the key-press duration is longer than the shutdown duration: True
      #  PWROK signal delay after power on: 64ms
      #  Shutdown time setting: 4s
      ...
      Infinitek.delay(100)   # wait for 100ms
  ...
  def set_displaydimmer(x)
    var v = Infinitek.scale_uint(x, 0, 100, 2500, 3300)
    self.set_ldo_voltage(3, v)
  end
