...
class PN532
  ...
  def command(payload, timeout_ms)
    ...
    ########
    # check for Ack
    var until_ms = Infinitek.millis(10)       # Ack max timeout is 10ms for 115200
  ...
  def _read_response(cmd_excpected, timeout_ms)
    var until_ms = Infinitek.millis(timeout_ms != nil ? timeout_ms : 50)
    var b
  ...
  def _read(len, until_ms)
    if (until_ms == nil)      until_ms = Infinitek.millis(15)     end
    
    if (len <= 0)
      return self.ser.read()
    else
      while !Infinitek.time_reached(until_ms)
  ...
  def set_timer_clear_uid(ms)
    ms = Infinitek.int(ms, 1000, nil)  # ensure the value is int and within boundaries
  ...
  def read_passive_target_id(card_baud_rate)
    ...
    try
      var b = self.command(cmd, 25)       # aggressive 25ms timeout since we don't want to stop Infinitek for too long - response is normally recevied in 15ms
  ...
  def publish_card_detected(new_card)
    ...
    if new_card
      ...
      Infinitek.publish_rule(event_card)
    else
      self.card_detected()
    end
    # reset timer
    # remove any previous timer
    Infinitek.remove_timer(self._TIMER_ID)
    Infinitek.set_timer(self.timer_clear_uid, def () self.reset_uid() end, self._TIMER_ID)
  end
