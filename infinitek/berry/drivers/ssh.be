...
class SFTP
    ...
    def long_name(url)
        ...
        var m = Infinitek.strftime("%B", date)[0..2]
        var dt = Infinitek.strftime("%d %H:%M", date)
        var pre = "-"
...
class HANDSHAKE
    ...
    static V_S = "SSH-2.0-InfinitekSSH_0.1" # server's identification string (CR and LF excluded)
...
class SESSION
    ...
    static banner = "  / \\    Secure Wireless Serial Interface\n"
                    ...
                    "  \\_/    Copyright (C) 2025 Infinitek %s\n"
    ...
    def send_banner()
        ...
        var s2 = Infinitek.cmd("status 2")["StatusFWR"]
        var hw = s2["Hardware"]
        var vs = s2["Version"]
        var strict_mode = ""
            ...
            if self.session.client_pub_key
                ...
                for key:persist.known_hosts
                    if key == self.session.client_pub_key
                        Infinitek.resp_cmnd_str("SSH: key already known")
                        return
                    end
                end
                persist.known_hosts.push(self.session.client_pub_key)
                persist.save(true)
                Infinitek.resp_cmnd_str("SSH: key saved")
            end
        end
    end
end
...
Infinitek.add_cmd("ssh_key_save", /->ssh.key_save())
