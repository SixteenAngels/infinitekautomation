...
#   import Shift595
#   Infinitek.add_driver(Shift595(srclk,ser,rclk,count,inverted))
# where
...
class Shift595
  ...
  def init(srclk, ser, rclk, count, inverted)
    ...
    self.first = Infinitek.global.devices_present
    Infinitek.global.devices_present += count
  ...
  def set_power_handler(cmd, idx)
    var new_state = Infinitek.get_power()[self.first..self.first+self.count]
