...
class Audio_SLM
  ...
  #   Infinitek.cmd("SaveData 0")
    ...
    Infinitek.delay(300)                                      # wait for 300 ms for mems microphone to stabilize
    self.fast_loop_closure = def () self.fast_loop() end
    #Infinitek.add_fast_loop(self.fast_loop_closure)
    Infinitek.add_driver(self)
    Infinitek.set_timer(100, /-> Infinitek.add_fast_loop(self.fast_loop_closure))   # delay start by 1 second
  end

  def stop()
    self.audio_input.stop()
    Infinitek.remove_fast_loop(self.fast_loop_closure)
