...
class webui_status
  ...
  def web_status_line_left()
    ...
    if Infinitek.wifi('up')
      webserver.content_send(format(self.HTTP_HEAD_STYLE_WIFI, Infinitek.webcolor(2 #-COL_FORM-#), Infinitek.webcolor(18 #-COL_TITLE-#)))
      var rssi = Infinitek.wifi('rssi')
      webserver.content_send(format("<div class='wifi' title='RSSI %d%%, %d dBm' style='padding:0 2px 0 2px;'><div class='arc a3 %s'></div><div class='arc a2 %s'></div><div class='arc a1 %s'></div><div class='arc a0 active'></div></div>",
                                    Infinitek.wifi('quality'), rssi,
                                    rssi >= -55 ? "active" : "",
                                    rssi >= -70 ? "active" : "",
                                    rssi >= -85 ? "active" : ""))
    end
    # display free heap
    webserver.content_send(f"<span>&nbsp;{Infinitek.memory('heap_free')}k</span>")
   end
end
return webui_status()
