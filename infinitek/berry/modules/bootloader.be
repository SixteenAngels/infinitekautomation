...
class bootloader
  ...
  def flash(url)
    ...
    # address to flash the bootloader
    var addr = self.get_bootloader_address()
    if addr == nil    Infinitek.log("OTA: can't find address for bootloader", 2) return false end

    var bl = open(fname, "r")
    if bl.readbytes(size(self._sign)) != self._sign
      Infinitek.log("OTA: file does not contain a bootloader signature", 2)
      return false
    end
    bl.seek(0)    # reset to start of file

    var bl_size = bl.size()
    if bl_size <= 8291   Infinitek.log("OTA: wrong bootloader size "+str(bl_size), 2) return false end
    if bl_size > (0x8000 - addr)  Infinitek.log("OTA: bootloader is too large "+str(bl_size / 1024)+"kB", 2) return false end

    Infinitek.log("OTA: Flashing bootloader", 2)
    ...
    bl.close()
    Infinitek.log("OTA: Booloader flashed, please restart", 2)
    return true
  end
end
...
bootloader().flash("bootloader-Infinitek-c3.bin")
