...
class Partition_wizard_UI
  static app_size_min = 832    # Min OTA size - let's set it at a safe 896KB for minimal Infinitek32 with TLS
  static app_size_max = 3072   # Max OTA size - (4096 - 896 - 128)
  static _default_safeboot_URL = "http://ota.Infinitek.com/Infinitek32/release/Infinitek32%s-safeboot.bin"
  ...
  def init()
    ...
    if persist.find("factory_migrate") == true
      ...
      Infinitek.log("UPL: Resuming after step 1", 2)
      persist.remove("factory_migrate")
      persist.save()

      # continue the migration process 5 seconds after Wifi is connected
      def continue_after_5s()
        Infinitek.remove_rule("parwiz_5s1")      # first remove rule to avoid firing it again at Wifi reconnect
        Infinitek.remove_rule("parwiz_5s2")      # first remove rule to avoid firing it again at Wifi reconnect
        Infinitek.set_timer(5000, /-> self.do_safeboot_partitioning())  # delay by 5 s
      end
      Infinitek.add_rule("Wifi#Connected=1", continue_after_5s, "parwiz_5s1")
      Infinitek.add_rule("Wifi#Connected==1", continue_after_5s, "parwiz_5s2")
  ...
  def get_max_flash_size_k(p)
    var flash_size_k = Infinitek.memory()['flash']
    var flash_size_real_k = Infinitek.memory().find("flash_real", flash_size_k)
  ...
  def resize_max_flash_size_k(p)
    ...
    if (flash_size_k != flash_size_real_k) && flash_definition_sector != nil
      ...
      if flash_size_code != nil
        ...
        Infinitek.log(string.format("UPL: changing flash definition from 0x02X to 0x%02X", old_def, flash_def[3]), 3)
      else
  ...
  static def copy_ota(from_addr, to_addr, sz)
    ...
    Infinitek.log(string.format("UPL: Copy flash from 0x%06X to 0x%06X (size: %ikB)", from_addr, to_addr, sz / 1024), 2)
    while size_left > 0
      ...
      if ((offset-4096) / 102400) < (offset / 102400)
        Infinitek.log(string.format("UPL: Progress %ikB", offset/1024), 3)
      end
    end
    Infinitek.log("UPL: done", 2)
  end
  ...
  def do_step_1(p)
    ...
    Infinitek.log("UPL: restarting on `app1`", 2)
    Infinitek.cmd("Restart 1")
  end
  ...
  def do_step_2(p, safeboot_url)
    ...
    if safeboot_url == nil || safeboot_url == ""
      ...
      Infinitek.log("UPL: no `safeboot` URL, defaulting to "+safeboot_url, 2)
    end
    ...
    Infinitek.log(string.format("UPL: flashing `safeboot` from %s %ikB", safeboot_url, (safeboot_size / 1024) + 1), 2)
    var app0 = p.get_ota_slot(0)
  ...
  def do_step_3(p)
    ...
    while idx < size(p.slots) - 1   # skip explicitly the last partition
      if p.slots[idx].is_spiffs()
        p.slots.remove(idx)
        Infinitek.log("UPL: removesd unused SPIFFS partition", 2)
      else
        idx += 1
      end
    end
  ...
  def show_migrate_to_factory(p)
    ...
    webserver.content_send("<p>Please see <a href='https://Infinitek.github.io/docs/Safeboot/' target='_blank'>Safeboot layout documentation</a></p>")
    ...
    var ota_url = Infinitek.cmd("OtaUrl").find("OtaUrl", "")
  ...
  def show_current_partitions(p)
    ...
    if self.has_factory_layout(p)
      webserver.content_send("<p>This device uses the <a href='https://Infinitek.github.io/docs/Safeboot/' target='_blank'>Safeboot</a> layout</p>")
    end
  ...
  def page_part_mgr()
    ...
    webserver.content_send_style()                  #- send standard Infinitek styles -#
  ...
  def support_grow_on_mount()
    import introspect
    if introspect.get(Infinitek, "version") != nil
      if Infinitek.version() >= 0x0D010003
        return true
      end
    end
    return false
    ...
    try
      ...
      #---------------------------------------------------------------------#
        ...
        if ota_url != nil && ota_url != ""
          Infinitek.cmd("OtaUrl "+ota_url)
        end

        Infinitek.set_timer(5000, /-> self.do_safeboot_partitioning())
      ...
      Infinitek.log(string.format("BRY: Exception> '%s' - %s", e, m), 2)
      #- display error page -#
  ...
  def do_safeboot_partitioning()
    ...
    if step1_state == false
      Infinitek.log("UPL: Starting step 1", 2)
      try
        self.do_step_1(p)
      except .. as e, m
        Infinitek.log(string.format("UPL: error (%s) %s", e, m), 2)
        return m
      end
      return false
    end
    Infinitek.log("UPL: Step 1 Done", 2)

    # STEP 2
    var step2_state = self.test_step_2(p)
    if type(step2_state) == 'string'  return step2_state end
    if step2_state == false
      Infinitek.log("UPL: Starting step 2", 2)
      import persist
    ...
    Infinitek.log("UPL: Step 3 Done", 2)
  ...
  #- ---------------------------------------------------------------------- -#
  #- this is called at Infinitek start-up, as soon as Wifi/Eth is up and web server running -#
  def web_add_handler()
    import webserver
...
#- create and register driver in Infinitek -#
if Infinitek
  import partition_core
  var partition_wizard_ui = partition_wizard.Partition_wizard_UI()
  Infinitek.add_driver(partition_wizard_ui)
