...
#- register buttons -#
var btns = []         # relay buttons are added to this list to match with Infinitek relays
...
def btn_clicked_cb(obj, event)
  ...
  if btn_idx != nil
    ...
    Infinitek.set_power(btn_idx, checked)                            # toggle the value
  end
end
...
def create_btn_relay(label)
  ...
  btn.add_event_cb(btn_clicked_cb, lv.EVENT_CLICKED, 0)       # set callback to update Infinitek relays
...
def btns_update()
  var power_list = Infinitek.get_power()                                            # get a list of booleans with status of each relay
  for b:btns
...
def btns_update_loop()
  btns_update()
  Infinitek.set_timer(500, btns_update_loop)
end
