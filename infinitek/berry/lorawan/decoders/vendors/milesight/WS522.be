  ...
  static def decodeUplink(Name, Node, RSSI, FPort, Bytes)
    ...
    while i < (Bytes.size()-1)
      ...
      if channel_id == 0x03 && channel_type == 0x74
        voltage_ls = Infinitek.rtc('local')
      ...
      # ACTIVE POWER
        ...
        active_power_ls = Infinitek.rtc('local')
      ...
      # POWER FACTOR
        ...
        power_factor_ls = Infinitek.rtc('local')
      ...
      # ENERGY SUM
        ...
        energy_sum_ls = Infinitek.rtc('local')
      ...
      # CURRENT
        ...
        current_ls = Infinitek.rtc('local')
      ...
      # STATE
        ...
        button_state_ls = Infinitek.rtc('local')
    ...
    if valid_values
      if !command_init
          ...
          Infinitek.remove_cmd( pfx + 'DelTask' )
          Infinitek.add_cmd( pfx + 'DelTask',
            def (cmd, idx, payload)
              return lwdecode.SendDownlink(global.ws522Nodes, cmd, idx, 'FF2300FF', 'Done')
            end
          )

          Infinitek.remove_cmd( pfx + 'OcAlarm' )
          Infinitek.add_cmd( pfx + 'OcAlarm',
            def (cmd, idx, payload)
              var parts = string.split(payload,',')
              if parts.size() != 2 
                return Infinitek.resp_cmnd_str("Usage: enable,threshold (enable: 0/1, threshold: 0-255)")
              end
