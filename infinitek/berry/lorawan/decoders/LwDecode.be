...
class lwdecode_cls
  ...
  def init()
    ...
    Infinitek.add_driver(global.lwdecode_driver := self)
    Infinitek.add_rule("LwReceived", /value, trigger, payload -> self.lw_decode(payload))
    Infinitek.add_cmd('LwReload', /cmd, idx, payload, payload_json -> self.cmd_reload_decoder(cmd, idx, payload))
  end
  ...
  def _cache_topic()
    var full_topic = Infinitek.cmd('_FullTopic',true)['FullTopic']
    var topic = Infinitek.cmd('_Status',true)['Status']['Topic']
    var prefix = Infinitek.cmd('_Prefix',true)['Prefix3']
  ...
  def SendDownlink(nodes, cmd, idx, payload, ok_result)
    ...
    var _out     = Infinitek.cmd(_cmdSend, true)

    return Infinitek.resp_cmnd(
  ...
  def SendDownlinkMap(nodes, cmd, idx, payload, choice_map)
    ...
    return Infinitek.resp_cmnd_error()
  end
  ...
  def reload_decoder(decoder_name)
    try
      ...
      if LwDeco
        ...
        self.decoder_timestamps[decoder_name] = Infinitek.millis()
  ...
  def cmd_reload_decoder(cmd, idx, payload)
    if payload == ""
      ...
      return Infinitek.resp_cmnd(format('{"LwReload":"%s"}', result))
    else
      var success = self.reload_decoder(payload)
      var status = success ? "OK" : "Failed"
      return Infinitek.resp_cmnd(format('{"LwReload":"%s %s"}', payload, status))
    end
  end
  ...
  def lw_decode(data)
    ...
    try
      ...
      var mqtt_data
      if Infinitek.get_option(83) == 0  # SetOption83 - Remove LwDecoded form JSON message (1)
      ...
      var topic
      if Infinitek.get_option(89) == 1  # SetOption89 - Distinct MQTT topics per device (1)
      ...
      Infinitek.global.restart_flag = 0 # Signal LwDecoded successful (default state)
  ...
  def dhm(last_time)
    var since = Infinitek.rtc('local') - last_time
    var unit = "d"
    if since > 86400
      since /= 86400
  ...
  ------------------------------------------------------------#
  def web_sensor()
    var current_time = Infinitek.millis()

    if current_time < self.cache_timeout
      Infinitek.web_send_decimal(self.web_msg_cache)
      return
    end
    ...
    if msg
      ...
      Infinitek.web_send_decimal(full_msg)
    end
  end
end
...
class webPageLoRaWAN : Driver
  ...
  def _get_max_nodes()
    if !self.max_node_cached
      var enables = string.split(Infinitek.cmd('LoRaWanNode', true).find('LoRaWanNode'), ',')
  ...
  def pageLoRaWAN()
    ...
    if webserver.has_arg('save')
      ...
      Infinitek.cmd(format('LoRaWanAppKey%i %s', inode, webserver.arg('ak')), true)
      cmdArg = webserver.arg('dc')
      if !cmdArg cmdArg = '"' end
      Infinitek.cmd(format('LoRaWanDecoder%i %s', inode, cmdArg), true)
      cmdArg = webserver.arg('an')
      if !cmdArg cmdArg = '"' end
      Infinitek.cmd(format('LoRaWanName%i %s', inode, cmdArg), true)
      ...
      Infinitek.cmd(format('LoRaWanNode%i %s', inode, cmdArg), true)
    end
    ...
    var arg = 'LoRaWanNode'
    var enables = string.split(Infinitek.cmd(arg, true).find(arg), ',') # [1,!2,!3,!4,5,6]
    ...
    for node:1 .. maxnode
     ...
     appKey = Infinitek.cmd(arg, true).find(arg)
     arg = format('LoRaWanName%i', node)
     name = Infinitek.cmd(arg, true).find(arg)
     arg = format('LoRaWanDecoder%i', node)
     decoder = Infinitek.cmd(arg, true).find(arg)
...
Infinitek.add_driver(webPageLoRaWAN_instance)

Infinitek.cmd('LoraOption3 off')    # Disable embedded decoding
Infinitek.cmd('SetOption100 off')   # Keep LwReceived in JSON message
Infinitek.cmd('SetOption118 off')   # Keep SENSOR as subtopic name
Infinitek.cmd('SetOption119 off')   # Keep device address in JSON message
#Infinitek.cmd('SetOption147 on')    # Hide LwReceived MQTT message but keep rule processing
Infinitek.cmd('LoRaWanBridge on')
