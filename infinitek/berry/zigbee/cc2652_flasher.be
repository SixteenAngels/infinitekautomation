...
# normal zigbee functions requires a Infinitek restart.
#
...
#   Dumping the complete file is done as follows (it takes 3 minutes during which Infinitek is unresponsive):
...
# This takes 3 minutes during which Infinitek is unresponsive
#
import cc2652_flasher as cc
cc.start()
...
class cc2652_flasher
  ...
  def init(rx, tx, rst, bsl)
    ...
    # Infinitek.log(format("FLH: cc2652_flasher rx=%i tx=%i rst=%i bsl=%i", self.rx, self.tx, self.rst, self.bsl), 3)
  ...
  def reset_bsl()
    ...
    Infinitek.delay(10)               # wait 10ms
    gpio.digital_write(self.rst, 1)
    Infinitek.delay(100)               # wait 100ms
  ...
  def recv_raw(timeout)
    var due = Infinitek.millis() + timeout
    while !Infinitek.time_reached(due)
      ...
      Infinitek.delay(5)        # check every 5ms
    end
