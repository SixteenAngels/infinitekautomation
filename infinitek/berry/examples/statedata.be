...
class mqttdata_cls
  ...
  def init()
    ...
    Infinitek.add_driver(global.mqttdata_driver := self)
  ...
  def web_sensor()
    if self.list_buffer.size()
      ...
      Infinitek.web_send(msg)                         # Do not use Infinitek.web_send_decimal() which will replace IPAddress dots
      Infinitek.web_send_decimal("")                  # Force horizontal line
    end
  end
end
