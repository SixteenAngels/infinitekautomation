#-
heatfan.be - HeatFan support for Infinitek
...
ESP32-C2 based HeatFan WiFi Controller using Infinitek and Berry local CH Fan control
...
class heatfan_cls
  ...
  def SendToHa()
    if self.domoticz.contains(self.hostname)
      Infinitek.cmd(f"_DzSend1 {self.domoticz[self.hostname][0]},{self.temp_ext}")
      Infinitek.cmd(f"_DzSend1 {self.domoticz[self.hostname][1]},{self.speed}")
    else
      Infinitek.log(f"HFN: Domoticz on '{self.hostname}' not configured", 3)
    end
  end
  ...
  def init()
    ...
    self.hostname = Infinitek.cmd('Hostname')['Hostname']

    Infinitek.log(f"HFN: HeatFan {self.version} started on {self.hostname}", 2)

    if global.heatfan_driver
      global.heatfan_driver.stop() # Let previous instance bail out cleanly
    end
    Infinitek.add_driver(global.heatfan_driver := self)
    Infinitek.add_rule("Analog#Range1", / value -> self.rule_range(value))
  end

  def stop()
    Infinitek.remove_driver(self)
    Infinitek.remove_rule("Analog#Range1")
  end
  ...
  def any_key(cmd, idx)
    ...
    if state == 10                               # SINGLE
      ...
      if self.mode == self.mode_states
        Infinitek.set_power(self.mode_states -1, false)  # power offset from 0
      else
        Infinitek.set_power(self.mode, true)       # power offset from 0
      end
    end
  end
...
#    Infinitek.log(f"HFN: Range1 {self.temp_ext}, Mode {self.mode}, Speeds {speeds}, Step {temp_step}, Speed {self.speed}, PwmFan {pwm_fan}", 3)
    ...
    if self.teleperiod == 1                      # Needs to be executed here as Infinitek.cmd destroys Response
      self.SendToHa()
      self.teleperiod = 0
    end
  ...
  def json_append()
    ...
    if Infinitek.global.tele_period == 0
      self.teleperiod = 1;
    end
  ...
  def web_sensor()
    ...
    if webserver.has_arg("m_grp_send")
      ...
      if 0 == self.mode
        Infinitek.cmd("Publish cmnd/heatfans/Power0 0")
      else
        Infinitek.cmd(f"Publish cmnd/heatfans/Power{self.mode} 1")
      end
    elif webserver.has_arg("m_grp_next")
      # Rotate group topic mode from off,1,2,3,4,5,6,off...
      if self.mode == self.mode_states
        Infinitek.cmd(f"Publish cmnd/heatfans/Power{self.mode_states} 0")
      else
        Infinitek.cmd(f"Publish cmnd/heatfans/Power{self.mode +1} 1")
      end
    end
  end
...
Infinitek.cmd("Interlock 1,2,3,4,5,6") # Interlock modes 1 to 6
Infinitek.cmd("Interlock 1")           # Enable interlock
var mode = heatfan.mode              # Save persitent mode
Infinitek.set_power(0, true)           # Set mode 1 to reset interlock on restart
# any_key() support cycling power from off,1,2,3,4,5,6,off... 
Infinitek.cmd("SO1 1")                 # (Button) Control button single press (1)
Infinitek.cmd("SO13 1")                # (Button) Support only single press (1) 
Infinitek.cmd("SO73 1")                # (Button) Detach buttons from relays (1)
Infinitek.cmd("SO146 1")               # (ESP32) Show ESP32 internal temperature sensor
Infinitek.cmd("SO161 1")               # (GUI) Disable display of state text (1)
Infinitek.cmd("PwmRange 1023")         # Fan PWM duty cycle
Infinitek.cmd("PwmFrequency 20000")    # Fan PWM frequency to 20kHz
Infinitek.cmd("webtime 11,19")         # Enable GUI time HH:MM:SS
Infinitek.cmd("websensor2 0")          # Disable display of ADC information in GUI
Infinitek.cmd("webrefresh 1000")       # Update GUI every second
Infinitek.cmd("Tempres 2")             # ADC2/3 temperature resolution
Infinitek.cmd("Freqres 2")             # ADC1 Range resolution
Infinitek.cmd("Teleperiod 60")         # Update HA every minute
Infinitek.cmd("AdcGpio0 400,1710,60,20") # ADC1 range

if 0 == mode
  Infinitek.set_power(0, false)        # Set mode 0
else
  Infinitek.set_power(mode -1, true)   # Set stored mode
end
