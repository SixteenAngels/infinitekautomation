...
if Infinitek.version() >= 0xD030002
  if !Infinitek.wifi()['up']
    Infinitek.add_rule("Wifi#Connected", def ()
      global._debug_panel = debug_panel(5550)
      Infinitek.remove_rule("Wifi#Connected", "debug_panel_start")
    end, "debug_panel_start")
  else
    global._debug_panel = debug_panel(5550)
  end
else
  log("BRY: 'debug_panel' requires Infinitek v13.3.0.2")
end
