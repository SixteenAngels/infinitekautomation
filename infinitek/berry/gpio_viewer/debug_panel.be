#
# debug_panel.be - implements a small panel on top of the Infinitek UI to view real-time information
#
  ...
  static class feeder
    ...
    def send_feed()
      ...
      if cnx.buf_out_empty()
        ...
        payload1 .. "\r\nevent:free_heap\r\ndata:"
        server.bytes_append_int(payload1, Infinitek.memory('heap_free'), '---')
        payload1 .. " KB\r\n\r\n"
        # payload = f"id:{Infinitek.millis()}\r\n"
        #           "event:free_heap\r\n"
        #           "data:{Infinitek.memory().find('heap_free', 0)} KB\r\n\r\n"
        cnx.write(payload1)
        ...
        server.bytes_append_int(payload1, Infinitek.millis())
        payload1 .. "\r\nevent:wifi_rssi\r\ndata:"
        server.bytes_append_int(payload1, Infinitek.wifi('quality'), '--')
        payload1 .. "%\r\n\r\n"

        # payload = f"id:{Infinitek.millis()}\r\n"
        #           "event:wifi_rssi\r\n"
        #           "data:{Infinitek.wifi().find('quality', '--')}%\r\n\r\n"
        cnx.write(payload1)
      end
    end
