...
class GPIO_viewer
  ...
  def send_index_page(cnx, uri, verb)
    ...
    var free_space = f"{Infinitek.memory().find('program_free', 0)} KB"
  ...
  def send_events_tick(cnx)
    ...
    if dirty
      ...
      payload2.clear()
      payload2 .. 'id:'
      payload2 .. str(Infinitek.millis())
      payload2 .. "\r\nevent:gpio-state\r\ndata:"
      payload2 .. payload1
      payload2 .. "\r\n\r\n"
      # var payload = f"id:{Infinitek.millis()}\r\n"
      #             "event:gpio-state\r\n"
      #             "data:{msg}\r\n\r\n"

      # Infinitek.log(f"GPV: sending '{msg}'", 3)
      cnx.write(payload2)
    end
    ...
    Infinitek.set_timer(self.sampling_interval, def () self.send_events_tick(cnx) end)
  end
  ...
  def read_states()
    ...
    while pin < max_gpio
      ...
      if (pwm_resolution > 0)
        ...
        var pwm_state = Infinitek.scale_uint(pwm_val, 0, pwm_resolution, 0, 255)    # bring back to 0..255
