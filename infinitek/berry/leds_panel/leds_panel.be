  ...
  class webserver_async
    ...
    static class webserver_async_cnx
      ...
      static var CODE_TO_STRING = {
        ...
        # 400: "Bad Request",
        # 401: "Unauthorized",
        # 403: "Payment Required",      # not sure it's useful in Infinitek context
        404: "Not Found",
      ...
      def init(server, cnx)
        ...
        Infinitek.add_fast_loop(self.fastloop_cb)
      end
      ...
      def loop()
        if self.cnx == nil    # if connection is closed, this instance is marked for deletion
          Infinitek.remove_fast_loop(self.fastloop_cb)  # remove from fast_loop
    ...
    #############################################################
    var local_port                                  # listening port, 80 is already used by Infinitek
    ...
    def close()
      ...
      Infinitek.remove_fast_loop(self.fastloop_cb)
...
class leds_panel
  ...
  def update()
    ...
    self.h = Infinitek.settings.light_pixels_height_1 + 1
    self.w = self.strip.pixel_count() / (Infinitek.settings.light_pixels_height_1 + 1)
    self.cell_size = Infinitek.int(self.HTML_WIDTH / self.w, 4, 25)
  ...
  static class feeder
    ...
    def send_feed()
      ...
      if cnx.buf_out_empty()
        ...
        payload1.clear()
        payload1 .. "id:"
        server.bytes_append_int(payload1, Infinitek.millis())
        payload1 .. "\r\nevent:leds\r\ndata:"
        ...
        payload1 .. ',"rev":'
        server.bytes_append_int(payload1, Infinitek.settings.light_pixels_reverse)
        payload1 .. ',"alt":'
        server.bytes_append_int(payload1, Infinitek.settings.light_pixels_alternate)
        payload1 .. ',"hex":"'
  ...
  def send_iframe_code()
    import webserver
    self.update()
    var ip = Infinitek.wifi().find('ip')
    if (ip == nil)
      ip = Infinitek.eth().find('ip')
    end
    if (ip != nil)
      ...
      webserver.content_send(
        f'<table style="width:100%;">'
          '<tbody>'
            '<tr>'
              '<td>'
                '<fieldset style="background-color:{Infinitek.webcolor(1)};">'
