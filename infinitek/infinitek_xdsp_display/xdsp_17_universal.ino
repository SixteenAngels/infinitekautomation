/*
  xdsp_17_universal.ino -  universal display driver support for Infinitek
...
Renderer *Init_uDisplay(const char *desc) {
  if (InfinitekGlobal.gpio_optiona.udisplay_driver || desc) {
...
#ifdef USE_UFILESYS
    if (ffsp  && !InfinitekGlobal.no_autoexec && !ddesc) {
      File fp;
    ...
    // Replace Infinitek vars before passing to driver uDisplay.cpp
...
void UDISP_PrintLog(void) {
  ...
  if (disp_refresh || printlog_mutex || InfinitekGlobal.restart_flag || InfinitekGlobal.ota_state_flag) {
    return;
  }
  printlog_mutex = true;
