...
extern "C" {
  ...
  void* be_audio_output_i2s_init(void) {
    return audio_i2s.out;               // return the singleton of InfinitekI2S which is already initialized
  }

  // AudioOutputI2S.deinit()-> void
  void be_audio_output_i2s_deinit(InfinitekI2S * out) {
    out->stop();
  }

  // AudioOutputI2S.begin() -> bool
  int be_audio_output_i2s_begin(InfinitekI2S* out) {
    if (I2SPrepareTx()) { return false; }
    return out->begin();
  }

  // AudioOutputI2S.stop() -> bool
  int be_audio_output_i2s_stop(InfinitekI2S* out) {
    return out->stop();
  }
  ...
  int i2s_output_i2s_init(bvm *vm) {
    InfinitekI2S * audio = new InfinitekI2S();
  ...
  int i2s_output_i2s_deinit(bvm *vm) {
    int argc = be_top(vm);
    be_getmember(vm, 1, ".p");
    InfinitekI2S * audio = (InfinitekI2S *) be_tocomptr(vm, -1);
    if (audio) {
      delete audio;
      // clear
  ...
  void* be_audio_input_i2s_init(void) {
    return audio_i2s.in;               // return the singleton of InfinitekI2S which is already initialized
  }

  // AudioInputI2S.deinit()-> void
  void be_audio_input_i2s_deinit(InfinitekI2S * in) {
    in->stop();
  }

  // AudioInputI2S.begin() -> bool
  int be_audio_input_i2s_begin(bvm *vm, InfinitekI2S* in) {
  ...
  int be_audio_input_i2s_stop(InfinitekI2S* in) {
    in->stopRx();
    return true;
  }
  ...
  int be_audio_input_i2s_get_rate(InfinitekI2S* in) {
    return in->getRxRate();
  }

  // AudioInputI2S.get_bits_per_sample() -> int
  int be_audio_input_i2s_get_bits_per_sample(InfinitekI2S* in) {
    return in->getRxBitsPerSample();
  }

  // AudioInputI2S.get_channels() -> int
  int be_audio_input_i2s_get_channels(InfinitekI2S* in) {
    return in->getRxChannels();
  }

  // AudioInputI2S.get_gain() -> real
  float be_audio_input_i2s_get_gain(InfinitekI2S* in) {
    return in->getRxGain();
  }

  // AudioInputI2S.set_gain(gain:real) -> bool
  int be_audio_input_set_gain(InfinitekI2S* in, float gain) {
    in->setRxGain(gain);
    return true;
  }

  // AudioInputI2S.get_dc_offset() -> float
  int be_audio_input_i2s_get_dc_offset(InfinitekI2S* in) {
    return in->getRxDCOffset();
  }

  // AudioInputI2S.get_lowpass_alpha() -> float
  float be_audio_input_i2s_get_lowpass_alpha(InfinitekI2S* in) {
    return in->getRxLowpassAlpha();
  }

  // AudioInputI2S.set_lowpass_alpha(alpha:float) -> bool
  int be_audio_input_i2s_set_lowpass_alpha(InfinitekI2S* in, float alpha) {
  ...
  int be_audio_input_i2s_read_bytes(bvm *vm) {
    ...
    be_getmember(vm, 1, ".p");
    InfinitekI2S *in = (InfinitekI2S*) be_tocomptr(vm, -1);
    be_pop(vm, 1);
