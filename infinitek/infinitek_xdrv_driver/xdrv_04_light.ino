/*
  xdrv_04_light.ino - PWM, WS2812 and sonoff led support for Infinitek
...
class LightStateClass {
  private:
    ...
    bool     _power = false;    // power indicator, used only in virtual lights, Infinitek tracks power separately
  ...
  public:
    ...
    // power accessors, for virtual lights only (has no effect on Infinitek lights)
    ...
    // reachable accessors, for virtual lights only (has no effect on Infinitek lights)
...
public:
  ...
  void loadSettings() {
    ...
    AddLog(LOG_LEVEL_DEBUG_MORE, "LightControllerClass::loadSettings light_type/sub (%d %d)",
      InfinitekGlobal.light_type, Light.subtype);
#endif
    if (_pwm_multi_channels) {
...
bool LightModuleInit(void)
{
  ...
  if (Settings->flag.pwm_control) {                     // SetOption15 - Switch between commands PWM or COLOR/DIMMER/CT/CHANNEL
    for (uint32_t i = 0; i < LST_MAX; i++) {
      if (PinUsed(GPIO_PWM1, i)) { InfinitekGlobal.light_type++; }  // Use Dimmer/Color control for all PWM as SetOption15 = 1
    }
  }

  InfinitekGlobal.light_driver = 0;
  if (XlgtCall(FUNC_MODULE_INIT)) {
    // serviced
  }
#ifdef ESP8266
  else if (SONOFF_BN == InfinitekGlobal.module_type) {   // PWM Single color led (White)
    InfinitekGlobal.light_type = LT_PWM1;
  }
  else if (SONOFF_LED == InfinitekGlobal.module_type) {  // PWM Dual color led (White warm and cold)
    if (!InfinitekGlobal.my_module.io[4]) {              // Fix Sonoff Led instabilities
      pinMode(4, OUTPUT);                              // Stop floating outputs
      digitalWrite(4, LOW);
    }
    if (!InfinitekGlobal.my_module.io[5]) {
      pinMode(5, OUTPUT);                              // Stop floating outputs
      digitalWrite(5, LOW);
    }
    if (!InfinitekGlobal.my_module.io[14]) {
      pinMode(14, OUTPUT);                             // Stop floating outputs
      digitalWrite(14, LOW);
    }
    InfinitekGlobal.light_type = LT_PWM2;
  }
#endif  // ESP8266
#ifdef USE_PWM_DIMMER
  else if (PWM_DIMMER == InfinitekGlobal.module_type) {
    InfinitekGlobal.light_type = Settings->pwm_dimmer_cfg.pwm_count + 1;
  }
#endif  // USE_PWM_DIMMER

  if (InfinitekGlobal.light_type > LT_BASIC) {
    UpdateDevicesPresent(1);
  }

  // post-process for lights
  uint32_t pwm_channels = (InfinitekGlobal.light_type & 7) > LST_MAX ? LST_MAX : (InfinitekGlobal.light_type & 7);
  if (Settings->flag3.pwm_multi_channels) {  // SetOption68 - Enable multi-channels PWM instead of Color PWM
    ...
    if ((Settings->flag4.virtual_ct) && (LST_RGB <= pwm_channels)) {
      ...
      InfinitekGlobal.light_type += 5 - pwm_channels;               // pretend it is a 5 channels bulb
    }
#endif // USE_LIGHT_VIRTUAL_CT
  }

  return (InfinitekGlobal.light_type > LT_BASIC);
}
...
void LightInit(void)
{
  ...
  Light.device = InfinitekGlobal.devices_present;
  Light.subtype = (InfinitekGlobal.light_type & 7) > LST_MAX ? LST_MAX : (InfinitekGlobal.light_type & 7); // Always 0 - LST_MAX (5)
  ...
  if ((LST_SINGLE <= Light.subtype) && Light.pwm_multi_channels) {
    ...
    Light.device = InfinitekGlobal.devices_present - Light.subtype + 1; // adjust if we also have relays
  ...
  LightCalcPWMRange();
#ifdef DEBUG_LIGHT
  AddLog(LOG_LEVEL_DEBUG_MORE, "LightInit Light.pwm_multi_channels=%d Light.subtype=%d Light.device=%d InfinitekGlobal.devices_present=%d",
    Light.pwm_multi_channels, Light.subtype, Light.device, InfinitekGlobal.devices_present);
#endif
  ...
  if (InfinitekGlobal.light_type < LT_PWM6) {           // PWM
    for (uint32_t i = 0; i < InfinitekGlobal.light_type; i++) {
  ...
  Light.devices = InfinitekGlobal.devices_present - Light.device +1;  // Last time that devices_present is not increments by display
...
uint8_t LightGetBri(uint8_t device) {
  ...
  if (Light.pwm_multi_channels) {
    if ((device >= Light.device) && (device < Light.device + LST_MAX) && (device <= InfinitekGlobal.devices_present)) {
...
#ifdef DEBUG_LIGHT
  AddLog(LOG_LEVEL_DEBUG, "LightPreparePower power=%d Light.power=%d", InfinitekGlobal.power, Light.power);
#endif
...
#ifdef DEBUG_LIGHT
  AddLog(LOG_LEVEL_DEBUG, "LightPreparePower End power=%d Light.power=%d", InfinitekGlobal.power, Light.power);
#endif
  Light.power = InfinitekGlobal.power >> (Light.device - 1);  // reset next state, works also with unlinked RGB/CT
...
void LightListenDDP()
{
  ...
  if (InfinitekGlobal.restart_flag || InfinitekGlobal.global_state.network_down) {
...
//  Light.power = XdrvMailbox.index;
  ...
  // Ex: 3 Relays and 4 PWM - InfinitekGlobal.devices_present = 7, Light.device = 4, Light.subtype = 4
  // Result: mask = 0b00001111 = 0x0F, shift = 3.
  // Power bits we consider are: 0b01111000 = 0x78
  // If regular situation: InfinitekGlobal.devices_present == Light.subtype
...
void LightAnimate(void)
{
  ...
  if ((Settings->light_scheme > LS_POWER && Light.power) || Light.fade_running) {
    if (InfinitekGlobal.sleep > PWM_MAX_SLEEP) {
      sleep_previous = InfinitekGlobal.sleep;     // save previous value of sleep
      InfinitekGlobal.sleep = PWM_MAX_SLEEP;      // set a maximum value (in milliseconds) to sleep to ensure that animations are smooth
    }
    if (Settings->save_data) {
      // Postpone save_data during animation
      InfinitekGlobal.save_data_counter = 2;
    }
  } else {
    if (sleep_previous > 0) {
      InfinitekGlobal.sleep = sleep_previous;
  ...
  if ((Settings->light_scheme < LS_MAX) || power_off) {  // exclude WS281X Neopixel schemes
    ...
    if (Light.update) {
      ...
      if (!LightGetFadeSetting() || InfinitekGlobal.skip_light_fade || power_off || (!Light.fade_initialized)) { // no fade
    ...
    if (InfinitekGlobal.gpio_optiona.pwm1_input && !Light.power && !Light.fade_running) {  // GPIO Option_A1
...
#ifdef ESP8266
  if ((PHILIPS == InfinitekGlobal.module_type) || (Settings->flag4.pwm_ct_mode)) {
#else
...
void LightStopFade(void) {
  ...
  if (PWM_DIMMER == InfinitekGlobal.module_type && !Light.power) PWMDimmerSetPower();
#endif  // USE_PWM_DIMMER
}
  ...
  if (0 == Light.fade_duration) {
    ...
    if (distance > 0) {
      ...
      if (Settings->save_data) {
        ...
        // AddLog(LOG_LEVEL_INFO, PSTR("delay_seconds %d, save_data_counter %d"), delay_seconds, InfinitekGlobal.save_data_counter);
        if (InfinitekGlobal.save_data_counter < delay_seconds) {
          InfinitekGlobal.save_data_counter = delay_seconds;      // pospone
        }
      }
    } else {
...
void LightSetOutputs(const uint16_t *cur_col_10) {
  ...
  if (InfinitekGlobal.light_type < LT_PWM6) {   // only for direct PWM lights, not for Tuya, Armtronix...

    int32_t channel_ct = ChannelCT();  // Channel for PWM CT or -1 if no CT or regular CT
#ifdef USE_PWM_DIMMER
    uint16_t max_col = 0;
#ifdef USE_I2C
    if (InfinitekGlobal.gpio_optiona.linkind_support) {  // Option_A6
...
#ifdef ESP32
        InfinitekGlobal.pwm_value[i] = cur_col;   // mark the new expected value
        // AddLog(LOG_LEVEL_DEBUG_MORE, "analogWrite-%i 0x%03X", i, cur_col);
#else // ESP32
        if (!Settings->flag4.zerocross_dimmer) {
          AnalogWrite(Pin(GPIO_PWM1, i), bitRead(InfinitekGlobal.pwm_inverted, i) ? Settings->pwm_range - cur_col : cur_col);
          // AddLog(LOG_LEVEL_DEBUG_MORE, "analogWrite-%i 0x%03X", bitRead(InfinitekGlobal.pwm_inverted, i) ? Settings->pwm_range - cur_col : cur_col);
        }
#endif // ESP32
      }
    }
    ...
    if (PWM_DIMMER == InfinitekGlobal.module_type) {
      InfinitekGlobal.pwm_dimmer_led_bri = change10to8(max_col);
...
void LightHandleDevGroupItem(void)
{
  ...
  switch (XdrvMailbox.command_code) {
    case DGR_ITEM_EOL:
      ...
      if (more_to_come) {
        InfinitekGlobal.skip_light_fade = true;
      }
      ...
      InfinitekGlobal.skip_light_fade = false;
...
void CmndDimmer(void)
{
  ...
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 100)) {
    ...
    Light.update = true;
    if (InfinitekGlobal.skip_light_fade) LightAnimate();
  } else {
...
bool Xdrv04(uint32_t function)
{
  ...
  else if (InfinitekGlobal.light_type) {
    switch (function) {
      case FUNC_SERIAL:
