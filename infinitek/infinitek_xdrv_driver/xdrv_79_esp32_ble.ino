/*
  xdrv_79_esp32_ble.ino - BLE via ESP32 support for Infinitek
...
12:45:12 MQT: tele/Infinitek_esp32/BLE = {"BLEOperation":{"opid":"11","stat":"7","state":"DONENOTIFIED","MAC":"4C65A8DAF43A","svc":"00001530-1212-efde-1523-785feabcd123","char":"00001531-1212-efde-1523-785feabcd123","notifychar":"00001531-1212-efde-1523-785feabcd123","write":"00","notify":"100003"}}
...
TaskHandle_t InfinitekMainTask;
...
const char *BLERestartInfinitekReason = BLE_RESTART_TEAMOTA_REASON_UNKNOWN;
...
void initSafeLog(){
  InfinitekMainTask = xTaskGetCurrentTaskHandle();
...
int SafeAddLog_P(uint32_t loglevel, PGM_P formatP, ...) {
  ...
  if ((loglevel > Settings->weblog_level) &&
      (loglevel > InfinitekGlobal.seriallog_level) &&
      (loglevel > Settings->mqttlog_level) &&
      (loglevel > InfinitekGlobal.syslog_level)){
    return added;
  }
...
//  if (InfinitekGlobal.ota_state_flag) return;

  TasAutoMutex localmutex(&BLEOperationsRecursiveMutex, "BLEPostAdd");
  if (BLEAdvertismentDetailsJsonSet){

//    strncpy(InfinitekGlobal.mqtt_data, BLEAdvertismentDetailsJson, sizeof(InfinitekGlobal.mqtt_data));
//    InfinitekGlobal.mqtt_data[sizeof(InfinitekGlobal.mqtt_data)-1] = 0;
...
static void BLEInit(void) {
  ...
  if (InfinitekGlobal.global_state.wifi_down && InfinitekGlobal.global_state.eth_down) { return; }

  if (!InfinitekGlobal.global_state.wifi_down) {
    InfinitekGlobal.wifi_stay_asleep = true;
...
static void BLETaskRunTaskDoneOperation(BLE_ESP32::generic_sensor_t** op, NimBLEClient **ppClient){
  if ((*ppClient)->isConnected()){
    ...
    do {
      ...
      if (waits == 60){
        AddLog(LOG_LEVEL_ERROR,PSTR("BLE: >60s waiting -> BLE Failed, restart Infinitek %d"), waits);
        BLEStop = 1;
        BLEStopAt = esp_timer_get_time();

        BLERestartInfinitek = 10;
        BLERestartInfinitekReason = BLE_RESTART_TEAMOTA_REASON_BLE_DISCONNECT_FAIL;
        break;
      }
...
static void BLEOperationTask(void *pvParameters){
  ...
  for(;;){
    ...
    if (BLERestartNimBLE){
      BLERestartNimBLE = 0;
      BLERestartInfinitek = 10;
      BLERestartInfinitekReason = BLE_RESTART_TEAMOTA_REASON_RESTARTING_BLE_TIMEOUT;
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: BLETask: Restart NimBLE - restart Infinitek in 10 if not complt"));
      BLE_ESP32::BLETaskStopStartNimBLE(&pClient);
      BLERestartInfinitekReason = BLE_RESTART_TEAMOTA_REASON_UNKNOWN;
      BLERestartInfinitek = 0;
      BLEResets ++;
    }
  }
...
/***********************************************************************\
 * Regular Infinitek called functions
 *
\***********************************************************************/
...
static void BLEEverySecond(bool restart){
  ...
  if (BLERestartInfinitek){
    BLERestartInfinitek--;
    // 2 seconds to go, post to BLE topic on MQTT our reason
    if (BLERestartInfinitek == 2){
      if (!BLERestartInfinitekReason) BLERestartInfinitekReason = BLE_RESTART_TEAMOTA_REASON_UNKNOWN;
      Response_P(PSTR("{\"reboot\":\"%s\"}"), BLERestartInfinitekReason);
      MqttPublishPrefixTopicRulesProcess_P(TELE, PSTR("BLE"), Settings->flag.mqtt_sensor_retain);
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: Failure! Restarting Infinitek in %d seconds because %s"), BLERestartInfinitek, BLERestartInfinitekReason);
    }

    if (!BLERestartInfinitek){
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: Failure! Restarting Infinitek because %s"), BLERestartInfinitekReason);
      // just a normal restart
      InfinitekGlobal.restart_flag = 1;
    }
  }
...
// details look like:
// MQT: tele/Infinitek_esp32/BLE = {"details":{"mac":"001A22092C9A","p":"0C0943432D52542D4D2D424C450CFF0000000000000000000000"}}
...
static void mainThreadBLETimeouts() {
  ...
  if (BLEStop == 1){
    if (BLEStopAt + 30L*1000L*1000L < now){ // if asked to stop > 30s ago...
      AddLog(LOG_LEVEL_ERROR,PSTR("BLE: Stop Timeout - restart Infinitek"));
      BLERestartInfinitek = 2;
      BLEStopAt = now;
    }
  ...
  // restart Infinitek completely. (belt and braces)
  uint64_t bleLoopTimeout = ((uint64_t)BLEMaxTaskLoopTime)*1000L*1000L;
  if (BLELastLoopTime + bleLoopTimeout < now){
    BLELastLoopTime = now; // initialise the time of the last advertisment
    BLERestartInfinitek = 10;
    AddLog(LOG_LEVEL_DEBUG,PSTR("BLE: BLETask stall > 120s, restart Infinitek in 10s"));
    BLERestartInfinitekReason = BLE_RESTART_TEAMOTA_REASON_BLE_LOOP_STALLED;
  }
}
