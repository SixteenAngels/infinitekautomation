/*
  xdrv_78_telnet.ino - Telnet console support for Infinitek
...
void TelnetLoop(void) {
  ...
  if ((Telnet.server) && (Telnet.server->hasClient())) {
    ...
    if (Telnet.client) {
      SetMinimumSeriallog();
      Telnet.client.printf("Infinitek %s %s (%s) %s\r\n", NetworkHostname(), InfinitekGlobal.version, GetBuildDateAndTime().c_str(), GetDeviceHardware().c_str());
...
bool Xdrv78(uint32_t function) {
  ...
  switch (function) {
    ...
    case FUNC_NETWORK_UP:
      if (!Telnet.server && (Telnet.port > 0) && !InfinitekGlobal.restart_flag) {
        TelnetStart();
      }
      break;
    case FUNC_COMMAND:
