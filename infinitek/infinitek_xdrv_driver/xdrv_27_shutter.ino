/*
  xdrv_27_Shutter.ino - Shutter/Blind support for Infinitek
  ...
  switch (Shutter[i].switch_mode) {
    case SHT_SWITCH:
      for (int8_t k=0;k<2;k++) {
        if ((1 << (Settings->shutter_startrelay[i]+k-1)) & InfinitekGlobal.power) {
    ...
    case SHT_PULSE:
      ...
      if ((SRC_PULSETIMER == InfinitekGlobal.last_source || SRC_SHUTTER == InfinitekGlobal.last_source || SRC_WEBGUI == InfinitekGlobal.last_source)) {
        ExecuteCommandPowerShutter(cur_relay, 1, SRC_SHUTTER);
        // switch off direction relay to make it power less
        if (((1 << (Settings->shutter_startrelay[i])) & InfinitekGlobal.power)  && Settings->shutter_startrelay[i]+1 != cur_relay) {
...
void ShutterStartInit(uint32_t i, int32_t direction, int32_t target_pos)
{
  ...
  if ( ( ( (1 == direction) && ((Shutter[i].open_max - Shutter[i].real_position)  <= Shutter[i].min_realPositionChange))
    ...
    if (Settings->save_data) {
      ...
      InfinitekGlobal.save_data_counter = Settings->save_data + (move_duration / 10) +1;
    }
  }
...
void ShutterRelayChanged(void)
{
  ...
  char stemp1[10];
	...
	for (uint32_t i = 0; i < InfinitekGlobal.shutters_present; i++) {
		power_t powerstate_local = (InfinitekGlobal.power >> (Settings->shutter_startrelay[i] -1)) & 3;
    // SRC_IGNORE added because INTERLOCK function bite causes this as last source for changing the relay.
		//uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (Settings->shutter_startrelay[i] -1)) & 3) && SRC_IGNORE != InfinitekGlobal.last_source && SRC_SHUTTER != InfinitekGlobal.last_source && SRC_PULSETIMER != InfinitekGlobal.last_source ;
    uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (Settings->shutter_startrelay[i] -1)) & 3) && SRC_SHUTTER != InfinitekGlobal.last_source && SRC_PULSETIMER != InfinitekGlobal.last_source ;
...
bool ShutterButtonHandler(void)
{
  ...
  if (!Settings->flag.button_single) {                    // SetOption13 (0) - Allow multi-press
    if (Button.window_timer[button_index]) {
      ...
      if (!InfinitekGlobal.restart_flag && !Button.hold_timer[button_index] && (Button.press_counter[button_index] > 0)) {
        if (Button.press_counter[button_index]<99) {
...
void CmndShutterPosition(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.shutters_present)) {
    if (!(Settings->shutter_options[XdrvMailbox.index-1] & 2)) {
      ...
      target_pos_percent = ((Settings->shutter_options[index] & 1) && (    (SRC_SERIAL     != InfinitekGlobal.last_source)
                                                                        && (SRC_WEBGUI     != InfinitekGlobal.last_source)
                                                                        && (SRC_WEBCOMMAND != InfinitekGlobal.last_source)
      ...
      if ( (target_pos_percent >= 0) && (target_pos_percent <= 100) &&
        ...
        if (Shutter[index].direction != new_shutterdirection) {
          ...
          switch (ShutterGlobal.position_mode) {
            ...
            case SHT_TIME:
              if (!ShutterGlobal.skip_relay_change) {
                if ( (InfinitekGlobal.power >> (Settings->shutter_startrelay[index] -1)) & 3 > 0 ) {
...
void CmndShutterFrequency(void)
{
  if ((XdrvMailbox.payload > 0) && (XdrvMailbox.payload <= 20000)) {
    ...
    if (InfinitekGlobal.shutters_present < 4) {
  ...
  if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
    ...
    switch (function) {
      ...
      case FUNC_JSON_APPEND:
        if (!sensor_data_reported || InfinitekGlobal.tele_period != 2) {
      ...
      case FUNC_SET_POWER:
        ...
        save_powermatrix = InfinitekGlobal.power;
