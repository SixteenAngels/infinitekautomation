...
bool ArtNetStart(void) {
  ArtNetLoadSettings();
  if (!artnet_udp_connected && !InfinitekGlobal.restart_flag) {
    ...
    if (ArtNetUdp->beginMulticast(IPAddress(USE_LIGHT_ARTNET_MCAST), artnet_conf.port)) {
      ...
      if (InfinitekGlobal.sleep > WS2812_ARTNET_MAX_SLEEP) {
        InfinitekGlobal.sleep = WS2812_ARTNET_MAX_SLEEP;
      }
...
//    Settings->light_scheme = LS_POWER;         // restore default scheme
    InfinitekGlobal.sleep = Settings->sleep;     // Restore sleep value
