/*
  xdrv_07_ufs_domoticz.ino - domoticz support for Infinitek
    ...
    if (DomoticzRelayIdx(device -1)) {
      ...
      if (IsModuleIfan() && (device > 1)) {
        ...
        Response_P(DOMOTICZ_MESSAGE, (int)DomoticzRelayIdx(device -1),
                                     (InfinitekGlobal.power & (1 << (device -1))) ? 1 : 0,
                                     (InfinitekGlobal.light_type) ? svalue : "",
                                     DomoticzBatteryQuality(), DomoticzRssiQuality());
        MqttPublish(domoticz_in_topic);
#ifdef USE_SONOFF_IFAN
      }
...
void DomoticzInit(void) {
  if (Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    ...
    Domoticz->devices = InfinitekGlobal.devices_present;
    if (Domoticz->devices) {
