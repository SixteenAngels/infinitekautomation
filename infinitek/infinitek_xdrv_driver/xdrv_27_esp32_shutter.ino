/*
  xdrv_27_esp32_shutter.ino - Shutter/Blind support for Infinitek
...
void ExecuteCommandPowerShutter(uint32_t device, uint32_t state, uint32_t source)
{
  ...
  if (device <= InfinitekGlobal.devices_present) ExecuteCommandPower(device,state,source);
}
...
bool ShutterButtonHandlerMulti(void)
{
  ...
  if (!Shutter[shutter_index].button_simu_pressed) {
    ...
    InfinitekGlobal.last_source = SRC_BUTTON;
    ...
    if (ShutterSettings.shutter_startrelay[i] && (ShutterSettings.shutter_startrelay[i] <= 32 )) {
      ...
      switch (ShutterGlobal.position_mode) {
        case SHT_PWM_VALUE:
          ...
          InfinitekGlobal.pwm_value[i] = Shutter[i].pwm_value;
          AddLog(LOG_LEVEL_DEBUG, PSTR("SHT: Shtr%d pwmset: %d -> %d"), i+1, InfinitekGlobal.pwm_cur_value[i], InfinitekGlobal.pwm_value[i]);
          PwmApplyGPIO(false);
        break;
        case SHT_TIME:
	  ...
	  // Limit the numbers of shutter to 4 because only 4 counters can be defined
          InfinitekGlobal.shutters_present = tmin(InfinitekGlobal.shutters_present, 4);
        break; 
      }
...
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.shutters_present)) {
...
void ShutterPowerOff(uint8_t i)
{
  ...
  AddLog(LOG_LEVEL_DEBUG, PSTR("SHT: Stop %d Mode %d time %d, last source %d"), i+1,Shutter[i].switch_mode, Shutter[i].time, InfinitekGlobal.last_source); // fix log to indicate correct shutter number
  ...
  InfinitekGlobal.rules_flag.shutter_moved = 1;
  switch (Shutter[i].switch_mode) {
    case SHT_SWITCH:
      for (int8_t k = 0; k < 2; k++) {
        if ((1 << (ShutterSettings.shutter_startrelay[i] + k - 1)) & InfinitekGlobal.power) {
    ...
    case SHT_PULSE:
      ...
      switch (InfinitekGlobal.last_source) {
        case SRC_PULSETIMER:
        case SRC_SHUTTER:
        case SRC_WEBGUI:
          ...
          if (((1 << (ShutterSettings.shutter_startrelay[i])) & InfinitekGlobal.power)  && ShutterSettings.shutter_startrelay[i] + 1 != cur_relay) {
            ExecuteCommandPowerShutter(ShutterSettings.shutter_startrelay[i] + 1, 0, SRC_SHUTTER);
          }
          break;
        default:
          InfinitekGlobal.last_source = SRC_SHUTTER;
      }
    break;
  }
  ...
  for (uint8_t j = 0 ; j < InfinitekGlobal.shutters_present ; j++) {
...
void ShutterRelayChanged(void)
{
  ...
  char stemp1[10];
	...
	for (uint32_t i = 0; i < InfinitekGlobal.shutters_present; i++) {
		power_t powerstate_local = (InfinitekGlobal.power >> (ShutterSettings.shutter_startrelay[i] - 1)) & 3;
    // SRC_IGNORE added because INTERLOCK function bite causes this as last source for changing the relay.
		//uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (ShutterSettings.shutter_startrelay[i] -1)) & 3) && SRC_IGNORE != InfinitekGlobal.last_source && SRC_SHUTTER != InfinitekGlobal.last_source && SRC_PULSETIMER != InfinitekGlobal.last_source ;
    uint8   manual_relays_changed = ((ShutterGlobal.RelayCurrentMask >> (ShutterSettings.shutter_startrelay[i] - 1)) & 3) && SRC_SHUTTER != InfinitekGlobal.last_source && SRC_PULSETIMER != InfinitekGlobal.last_source ;
    //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Shtr%d, Source %s, Powerstate %ld, RelayMask %d, ManualChange %d"),
    //  i+1, GetTextIndexed(stemp1, sizeof(stemp1), InfinitekGlobal.last_source, kCommandSource), powerstate_local,ShutterGlobal.RelayCurrentMask,manual_relays_changed);
        ...
        case SHT_PULSE:
          if (Shutter[i].direction != 0 && powerstate_local) {
            ...
            powerstate_local = 0;
              ...
              i+1, Shutter[i].target_position, GetTextIndexed(stemp1, sizeof(stemp1), InfinitekGlobal.last_source, kCommandSource), powerstate_local,ShutterGlobal.RelayCurrentMask,manual_relays_changed);
          }
        break;
        default:
          InfinitekGlobal.last_source = SRC_SHUTTER; // avoid switch off in the next loop
...
void ShutterReportPosition(bool always, uint32_t index)
{
  ...
  uint32_t i = 0;
  uint32_t n = InfinitekGlobal.shutters_present;
  ...
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: rules_flag.shutter_moving: %d, moved %d"), InfinitekGlobal.rules_flag.shutter_moving, InfinitekGlobal.rules_flag.shutter_moved);
}
...
void ShutterRtc50mS(void)
{
  ...
  for (uint8_t i = 0; i < InfinitekGlobal.shutters_present; i++) {
    if (Shutter[i].direction) {
      ...
      switch (ShutterGlobal.position_mode) {
        ...
        case SHT_COUNTER:
          if (Shutter[i].accelerator) {
            ...
            InfinitekGlobal.pwm_value[i] = 512;
            pwm_apply = true;
          }
        break;
      }
...
void ShutterStartInit(uint32_t i, int32_t direction, int32_t target_pos)
{
  ...
  if ( ( ( (1 == direction) && ((Shutter[i].open_max - Shutter[i].real_position)  <= Shutter[i].min_realPositionChange))
    ...
    InfinitekGlobal.rules_flag.shutter_moving = 1;
    ...
    InfinitekGlobal.rules_flag.shutter_moved  = 0;
    ...
    InfinitekGlobal.save_data_counter = 900;
...
void ShutterToggle(bool dir)
{
  ...
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.shutters_present)) {
    ...
    XdrvMailbox.data_len      = 0;
    InfinitekGlobal.last_source = SRC_WEBGUI;
    CmndShutterPosition();
  }
}
...
{
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Change in: payload %s (%d), payload %d, idx %d, src %d"), XdrvMailbox.data , XdrvMailbox.data_len, XdrvMailbox.payload , XdrvMailbox.index, InfinitekGlobal.last_source );
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.shutters_present)) {
    if (XdrvMailbox.data_len > 0) {
...
void CmndShutterPosition(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.shutters_present)) {
    if (!(ShutterSettings.shutter_options[XdrvMailbox.index - 1] & 2)) {
      uint32_t index = XdrvMailbox.index - 1;
      //limit the payload
      AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Pos. payload <%s> (%d), payload %d, idx %d (%d), src %d"), XdrvMailbox.data , XdrvMailbox.data_len, XdrvMailbox.payload , XdrvMailbox.index, XdrvMailbox.usridx, InfinitekGlobal.last_source );
      ...
      target_pos_percent = ((ShutterSettings.shutter_options[index] & 1) && (    (SRC_SERIAL     != InfinitekGlobal.last_source) // 6
                                                                              && (SRC_WEBGUI     != InfinitekGlobal.last_source) // 7
                                                                              && (SRC_WEBCOMMAND != InfinitekGlobal.last_source) // 8
      ...
      if (XdrvMailbox.payload != -99) {
        ...
        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: lastsource %d:, real %d, target %d, tiltreal: %d, tilttarget: %d, payload %d"), InfinitekGlobal.last_source, Shutter[index].real_position ,Shutter[index].target_position,Shutter[index].tilt_real_pos, Shutter[index].tilt_target_pos,target_pos_percent);
      }
      if ( (target_pos_percent >= 0) && (target_pos_percent <= 100) &&
        ...
        if (Shutter[index].direction != new_shutterdirection) {
          ...
          switch (ShutterGlobal.position_mode) {
            ...
            case SHT_TIME:
              if (!ShutterGlobal.skip_relay_change) {
                if ( (InfinitekGlobal.power >> (ShutterSettings.shutter_startrelay[index] - 1)) & 3 > 0 ) {
...
void CmndShutterRelay(void)
{
  ...
  uint32_t end   = (!XdrvMailbox.usridx && !XdrvMailbox.data_len)?InfinitekGlobal.shutters_present:XdrvMailbox.index;
...
{
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: Change in: payload %s (%d), payload %d, idx %d, src %d"), XdrvMailbox.data , XdrvMailbox.data_len, XdrvMailbox.payload , XdrvMailbox.index, InfinitekGlobal.last_source );
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.shutters_present) && XdrvMailbox.data_len > 0) {
...
bool Xdrv27(uint32_t function)
{
  ...
  if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
    ...
    uint8_t  counterend      = XdrvMailbox.index == 0 ? InfinitekGlobal.shutters_present : XdrvMailbox.index;
    ...
    switch (function) {
      ...
      case FUNC_JSON_APPEND:
        if (!ShutterGlobal.sensor_data_reported || InfinitekGlobal.tele_period != 2) {          
  ...
  #ifdef USE_DOMOTICZ
            if ((0 == InfinitekGlobal.tele_period) && (0 == i)) {
      ...
      case FUNC_SET_POWER:
        ...
        //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: FUNC_SET_POWER Relaymask %d SwitchedRelay:%d by %s, payload %d, powermask %d"), ShutterGlobal.RelayOldMask, ShutterGlobal.LastChangedRelay,GetTextIndexed(stemp1, sizeof(stemp1), InfinitekGlobal.last_source, kCommandSource),XdrvMailbox.payload, InfinitekGlobal.power);
        save_powermatrix = InfinitekGlobal.power; // can be changed in ShutterRelayChanged
        if (!ShutterGlobal.LastChangedRelay) {
          ...
          InfinitekGlobal.power = save_powermatrix;
        }
        //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: FUNC_SET_POWER end. powermask %d"), InfinitekGlobal.power);
      break;
      case FUNC_SET_DEVICE_POWER:
        ...
        if (ShutterGlobal.skip_relay_change ) {
          ...
          if (ShutterGlobal.LastChangedRelay) ShutterGlobal.RelayOldMask = InfinitekGlobal.power ^=  1<<(ShutterGlobal.LastChangedRelay-1);
        }
        //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("SHT: FUNC_SET_DEVICE_POWER end. powermask %ld, current rel: %ld"), InfinitekGlobal.power, ShutterGlobal.RelayOldMask);
      break;
