/*
  xdrv_10_rules.ino - rule support for Infinitek
...
//      if (!ucommand.startsWith("BACKLOG")) { commands = "backlog " + commands; }  // Always use Backlog to prevent power race exception
      ...
      for (uint32_t i = 0; i < InfinitekGlobal.devices_present; i++) {
        ...
        RulesVarReplace(commands, stemp, String(bitRead(InfinitekGlobal.power, i)));
      }
      ...
      RulesVarReplace(commands, F("%TOPIC%"), InfinitekGlobal.mqtt_topic);
...
void RulesInit(void)
{
  ...
  InfinitekGlobal.rules_flag.data = 0;
  for (uint32_t i = 0; i < MAX_RULE_SETS; i++) {
    if (0 == GetRuleLen(i)) {
...
void RulesEvery50ms(void)
{
  if ((Settings->rule_enabled || BERRY_RULES) && !Rules.busy) {  // Any rule enabled
    ...
    if (-1 == Rules.new_power) { Rules.new_power = InfinitekGlobal.power; }
...
//#if defined(ESP32) && CONFIG_IDF_TARGET_ESP32 && defined(USE_ETHERNET)
      ...
      else if (InfinitekGlobal.rules_flag.eth_connected) {
        InfinitekGlobal.rules_flag.eth_connected = 0;
        strncpy_P(json_event, PSTR("{\"ETH\":{\"Connected\":1}}"), sizeof(json_event));
      }
      else if (InfinitekGlobal.rules_flag.eth_disconnected) {
        InfinitekGlobal.rules_flag.eth_disconnected = 0;
        strncpy_P(json_event, PSTR("{\"ETH\":{\"Disconnected\":1}}"), sizeof(json_event));
      }
#endif  // USE_ETHERNET
      else if (InfinitekGlobal.rules_flag.http_init) {
        InfinitekGlobal.rules_flag.http_init = 0;
        strncpy_P(json_event, PSTR("{\"HTTP\":{\"Initialized\":1}}"), sizeof(json_event));
      }
#ifdef USE_SHUTTER
      else if (InfinitekGlobal.rules_flag.shutter_moved) {
        InfinitekGlobal.rules_flag.shutter_moved = 0;
        strncpy_P(json_event, PSTR("{\"SHUTTER\":{\"Moved\":1}}"), sizeof(json_event));
      }
      else if (InfinitekGlobal.rules_flag.shutter_moving) {
        InfinitekGlobal.rules_flag.shutter_moving = 0;
...
void RulesEvery100ms(void) {
  if ((Settings->rule_enabled || BERRY_RULES) && !Rules.busy && (InfinitekGlobal.uptime > 4)) {  // Any rule enabled and allow 4 seconds start-up time for sensors (#3811)
    if (GetNextSensor()) {
...
void RulesEverySecond(void)
{
  ...
  if ((Settings->rule_enabled || BERRY_RULES) && !Rules.busy) {  // Any rule enabled
    if (RtcTime.valid) {
      if ((InfinitekGlobal.uptime > 60) && (RtcTime.minute != Rules.last_minute)) {  // Execute from one minute after restart every minute only once
...
*/
  ...
  for (auto &event_item : subscriptions) {     // Looking for all matched topics
    ...
    strcpy(etopic, event_item.topic);          // tele/Infinitek/SENSOR
    strcat(etopic, "/");                       // tele/Infinitek/SENSOR/
    if ((strcmp(ctopic, event_item.topic) == 0) ||         // Equal tele/Infinitek/SENSOR
        (strncmp(ctopic, etopic, strlen(etopic)) == 0)) {  // StartsWith tele/Infinitek/SENSOR/
