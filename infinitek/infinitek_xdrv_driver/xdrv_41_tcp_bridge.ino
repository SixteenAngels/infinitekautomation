...
InfinitekSerial *TCPSerial = nullptr;
...
void TCPInit(void) {
  if (PinUsed(GPIO_TCP_RX) && PinUsed(GPIO_TCP_TX)) {
    ...
    TCPSerial = new InfinitekSerial(Pin(GPIO_TCP_RX), Pin(GPIO_TCP_TX), InfinitekGlobal.seriallog_level ? 1 : 2, 0, TCP_BRIDGE_BUF_SIZE);   // set a receive buffer of 256 bytes
