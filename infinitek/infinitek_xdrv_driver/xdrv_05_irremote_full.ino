/*
  xdrv_05_irremote_full.ino - complete integration of IRremoteESP8266 for Infinitek
...
stdAc::state_t irac_prev_state; // this implementations only keeps one state so if you use a single Infinitek-ir device to command more than one AC it might not work
...
//    if ((now - ir_lasttime > IR_TIME_AVOID_DUPLICATE) && (UNKNOWN != results.decode_type) && (results.bits > 0)) {
    if (now - ir_lasttime > IR_TIME_AVOID_DUPLICATE) {
      ...
      ResponseJsonEndEnd();
      if (Settings->flag6.mqtt_disable_publish ) {  // SetOption147 - If it is activated, Infinitek will not publish IRReceived MQTT messages, but it will proccess event trigger rules
        XdrvRulesProcess(0);
      } else {
