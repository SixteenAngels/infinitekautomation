...
class InfinitekI2S : public AudioOutput
{
public:
...
void InfinitekI2S::setPinout(int32_t bclk, int32_t ws, int32_t dout, int32_t mclk, int32_t din,
...
bool InfinitekI2S::beginTx(void) {
...
bool InfinitekI2S::delTxHandle(void) {
  esp_err_t err = ESP_OK;
...
bool InfinitekI2S::delRxHandle(void) {
  esp_err_t err = ESP_OK;
...
bool InfinitekI2S::ConsumeSample(int16_t sample[2]) {
  return consumeSamples(sample, 1);
}

int32_t InfinitekI2S::consumeSamples(int16_t *samples, size_t count) {
...
bool InfinitekI2S::startI2SChannel(bool tx, bool rx) {
...
bool InfinitekI2S::updateClockConfig(void) {
...
uint32_t InfinitekI2S::startRx(void) {
...
int32_t InfinitekI2S::readMic(uint8_t *buffer, uint32_t size, bool dc_block, bool apply_gain, bool lowpass, uint32_t *peak_ptr) {
  uint32_t peak = 0;
...
int16_t InfinitekI2S::dcFilter(int16_t pcm_in) {
...
// 1 - alpha = 0x3A9C
int16_t InfinitekI2S::lowpassFilter(int16_t pcm_in) {
