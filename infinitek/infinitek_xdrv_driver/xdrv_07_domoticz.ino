/*
  xdrv_07_domoticz.ino - domoticz support for Infinitek
 ...
 *
 * Adds commands:
 * DzIdx<number> <idx>      - Set power number to Domoticz Idx allowing Domoticz to control Infinitek power
 * DzKeyIdx<number> <idx>   - Set button number to Domoticz Idx allowing Infinitek button as input to Domoticz
 * DzSwitchId<number> <idx> - Set switch number to Domoticz Idx allowing Infinitek switch as input to Domoticz
...
void MqttPublishDomoticzPowerState(uint8_t device) {
  if (Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    ...
    if ((device > InfinitekGlobal.devices_present) || (device > MAX_RELAYS_SET)) { return; }
    if (DomoticzRelayIdx(device -1)) {
      ...
      if (IsModuleIfan() && (device > 1)) {
        ...
        Response_P(DOMOTICZ_MESSAGE, (int)DomoticzRelayIdx(device -1), (InfinitekGlobal.power & (1 << (device -1))) ? 1 : 0, (InfinitekGlobal.light_type) ? svalue : "", DomoticzBatteryQuality(), DomoticzRssiQuality());
        MqttPublish(domoticz_in_topic);
#ifdef USE_SONOFF_IFAN
      }
#endif // USE_SONOFF_IFAN
...
void DomoticzMqttSubscribe(void) {
  uint8_t maxdev = (InfinitekGlobal.devices_present > MAX_RELAYS_SET) ? MAX_RELAYS_SET : InfinitekGlobal.devices_present;
...
int DomoticzIdx2Relay(uint32_t idx) {
  if (idx > 0) {
    uint32_t maxdev = (InfinitekGlobal.devices_present > MAX_RELAYS_SET) ? MAX_RELAYS_SET : InfinitekGlobal.devices_present;
    for (uint32_t i = 0; i < maxdev; i++) {
...
*/
  ...
  if (iscolordimmer && 10 == nvalue) {  // Color_SetColor
    ...
    if (InfinitekGlobal.light_type && (Settings->light_dimmer == nvalue) && ((InfinitekGlobal.power >> relay_index) &1)) {
  ...
  if (1 == nvalue || 0 == nvalue) {
    if (((InfinitekGlobal.power >> relay_index) &1) == (power_t)nvalue) {
      return true;  // Stop loop
    }
    char stemp1[10];
    snprintf_P(XdrvMailbox.topic, XdrvMailbox.index, PSTR("/" D_CMND_POWER "%s"), (InfinitekGlobal.devices_present > 1) ? itoa(relay_index +1, stemp1, 10) : "");
    snprintf_P(XdrvMailbox.data, XdrvMailbox.data_len, PSTR("%d"), nvalue);
  } else {
