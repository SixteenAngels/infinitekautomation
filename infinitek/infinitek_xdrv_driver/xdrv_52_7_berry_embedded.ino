...
const char be_berry_init_code[] =
  ...
  // Instantiate Infinitek object
  "Infinitek = Infinitek() "
  "def log(m,l) Infinitek.log(m,l) end "
  "def load(f) return Infinitek.load(f) end "

  // try to resize FS to max at first boot
  // "Infinitek.log('>>> bootcount=' + str(Infinitek.settings.bootcount), 2) "
  "if Infinitek.settings.bootcount == 0 "
...
#ifdef USE_I2C
  "Infinitek.wire1 = Wire(1) "
  "Infinitek.wire2 = Wire(2) "
  "wire1 = Infinitek.wire1 "
  "wire2 = Infinitek.wire2 "
#endif // USE_I2C
