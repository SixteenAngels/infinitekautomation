...
bool WireguardLoadConfig(const char *filename) {
  ...
  ini.getIPAddress("Infinitek", "Netmask", &Wireguard.config.netmask2);
  ...
  ini.getValueBool("Infinitek", "AutoConnect", Wireguard.auto_connect);
  // add allowedIPs
...
bool Xdrv80(uint32_t function) {
  bool result = false;
  if (InfinitekGlobal.no_autoexec) { return result; }  // do nothing in case of bootloop
