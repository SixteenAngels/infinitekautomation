/*
  xdrv_50_filesystem.ino - unified file system for Infinitek
...
// Called from Infinitek.ino at restart. This inits flash file only
void UfsInit(void) {
...
void UfsCheckSDCardInit(void) {
  ...
  if (InfinitekGlobal.spi_enabled && PinUsed(GPIO_SDCARD_CS)) {
...
void download_task(void *path) {
  ...
  //Webserver->sendHeader(F("X-Infinitek-Debug"), attachment);
  snprintf_P(attachment, sizeof(attachment), PSTR("attachment; filename=%s"), cp);
  Webserver->sendHeader(F("Content-Disposition"), attachment);
  WSSend(200, CT_APP_STREAM, "");
...
/*
// Moved to support_Infinitek.ino for earlier init to be used by scripter
#ifdef USE_SDCARD
    case FUNC_PRE_INIT:
      UfsCheckSDCardInit();
      break;
