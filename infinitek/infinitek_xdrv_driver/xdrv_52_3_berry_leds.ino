...
/*********************************************************************************************\
 * Functions from Infinitek WS2812 driver
 *
\*********************************************************************************************/
...
extern "C" {
  ...
  void * be_get_Infinitekled(bvm *vm) {
  ...
  int be_Infinitekled_call_native(bvm *vm);
  int be_Infinitekled_call_native(bvm *vm) {
    ...
    if (argc >= 2 && be_isint(vm, 2)) {
      ...
      if (0 == cmd) { // 00 : ctor         (leds:int, gpio:int) -> void
        if ((argc != 2) && !(argc >= 5 && be_isint(vm, 3) && be_isint(vm, 4) && be_isint(vm, 5))) {
          be_raise(vm, "value_error", "bad arguments for Infinitekled:ctor");
        }
        int32_t leds = -1;
        int32_t gpio = -1;
        int32_t led_type = 0;
        ...
        if (-1 == gpio) {
          // if GPIO is '-1'
          led_type = 0;
          Ws2812InitStrip();          // ensure the Infinitekled object is initialized, because Berry code runs before the driver is initialized
        } else {
          if (led_type < 1) { led_type = 1; }
          InfinitekLEDPusher * pusher = InfinitekLEDPusher::Create(hardware, gpio);
          if (pusher == nullptr) {
            be_raise(vm, "value_error", "LED interface not supported");
          }
          strip = new InfinitekLED(led_type, leds);
        ...
        // all other commands need a valid Infinitekled pointer
        InfinitekLED * strip = (InfinitekLED*) be_get_Infinitekled(vm);    // raises an exception if pointer is invalid
        ...
        switch (cmd) {
          ...
          case 2: // # 02 : show         void -> void
            ...
            if (Settings->flag6.berry_light_scheme &&
                (1 == InfinitekGlobal.light_driver)) {  // XLGT_01
