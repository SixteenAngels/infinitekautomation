/*
  xdrv_18_armtronix_dimmers.ino - Armtronix dimmers support for Infinitek
 ...
 * Checkout the Infinitek Wiki for information on how to flash this Atmega328 with the firmware
...
bool ArmtronixModuleSelected(void)
{
  ...
  InfinitekGlobal.light_type = LT_SERIAL2;
  return true;
}
...
void ArmtronixInit(void)
{
  ...
  ArmtronixSerial = new InfinitekSerial(Pin(GPIO_RXD), Pin(GPIO_TXD), 2);
...
bool Xdrv18(uint32_t function)
{
  ...
  if (ARMTRONIX_DIMMERS == InfinitekGlobal.module_type) {
    switch (function) {
      case FUNC_LOOP:
