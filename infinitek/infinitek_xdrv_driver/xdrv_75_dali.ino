/*
  xdrv_75_dali.ino - DALI support for Infinitek
  ...
  0.1.0.1 20241007  update    - To stablizie communication send DALI datagram twice like Busch-Jaeger does 
                              - Change DaliPower 0..2 to act like Infinitek Power (Off, On, Toggle)
 ...
 * DaliLight 0|1                                 - Enable Infinitek light control for DaliTarget device - default 1
 * DaliTarget <broadcast>|<device>|<group>       - Set Infinitek light control device (0, 1..64, 101..116) - default 0
 * 
...
bool DaliInit(uint32_t function) {
  ...
  InfinitekGlobal.light_type = LT_SERIAL1;       // Single channel
  return true;
#else
  return false;
#endif  // USE_LIGHT
...
void CmndDaliGroupSliders(void) {
  ...
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 16)) {
    ...
    InfinitekGlobal.restart_flag = 2;            // Restart to update GUI
  }
