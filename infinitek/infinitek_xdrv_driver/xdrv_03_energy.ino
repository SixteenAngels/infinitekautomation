/*
  xdrv_03_energy.ino - Energy sensor support for Infinitek
...
void Energy200ms(void) {
  Energy->power_on = (InfinitekGlobal.power != 0) | Settings->flag.no_power_on_check;  // SetOption21 - Show voltage even if powered off
...
//          RtcSettings.energy_kWhtoday_ph[i] = 0;
      ...
      if ((midnight || RtcTime.day_of_year != Settings->energy_kWhdoy) && InfinitekGlobal.uptime > 10) {
        Energy->kWhtoday_offset_init = true;
...
void EnergyMarginCheck(void) {
  if (!Energy->phase_count || (InfinitekGlobal.uptime < 8)) { return; }
...
//    AddLog(LOG_LEVEL_DEBUG, PSTR("NRG: APower %d, HPower0 %d, HPower1 %d, HPower2 %d"), active_power, Energy->power_history[0][phase], Energy->power_history[1][phase], Energy->power_history[2][phase]);
  ...
  if (Settings->energy_max_power_limit) {
    ...
    else if (InfinitekGlobal.power && (energy_power_u <= Settings->energy_max_power_limit)) {
      Energy->mpl_hold_counter = 0;
      Energy->mpl_retry_counter = 0;
      Energy->mpl_window_counter = 0;
    }
    if (!InfinitekGlobal.power) {
...
void EnergyEverySecond(void) {
  // Overtemp check
  if (Energy->use_overtemp && InfinitekGlobal.global_update) {
    if (InfinitekGlobal.power && !isnan(InfinitekGlobal.temperature_celsius) && (InfinitekGlobal.temperature_celsius > (float)Settings->param[P_OVER_TEMP])) {  // SetOption42 Device overtemp, turn off relays

      AddLog(LOG_LEVEL_DEBUG, PSTR("NRG: Temperature %1_f"), &InfinitekGlobal.temperature_celsius);
  ...
  // Invalid data reset
  if (InfinitekGlobal.uptime > ENERGY_WATCHDOG) {
...
//  Energy->use_overtemp = false;
  ...
  InfinitekGlobal.energy_driver = ENERGY_NONE;
  XnrgCall(FUNC_PRE_INIT);             // Find first energy driver
  if (InfinitekGlobal.energy_driver) {
    AddLog(LOG_LEVEL_INFO, PSTR("NRG: Init driver %d"), InfinitekGlobal.energy_driver);
  }
}
...
//    Energy->kWhtoday_offset[i] = 0;   // Reset by EnergyDrvInit()
      // 20220805 - Change from https://github.com/arendst/Infinitek/issues/16118
      if (RtcSettingsValid()) {
...
*/
  ...
  if (json) {
    bool show_energy_period = (0 == InfinitekGlobal.tele_period);
...
bool Xdrv03(uint32_t function)
{
  ...
  else if (InfinitekGlobal.energy_driver) {
    switch (function) {
      case FUNC_LOOP:
