/*
  xdrv_77_wizmote.ino - WiZ Smart Remote decoder support for Infinitek
...
void WizMoteCommissionInit(void) {
  WizMote.count = 0;
  WizMote.comm_window = InfinitekGlobal.uptime + WIZMOTE_COMMISSION_WINDOW;
...
void WizMoteResponse(void) {
...
#ifdef USE_WIZMOTE_COMMISSION
  if (WizMote.comm_window > InfinitekGlobal.uptime) {  // Max WIZMOTE_COMMISSION_WINDOW seconds after restart or enabling SO164
    if (((0 == WizMote.comm_id) && (2 == WizMote.index)) ||          // ON button commissioning
      ...
      if (3 == WizMote.count) {
        ...
        WizMote.comm_window = InfinitekGlobal.uptime;
  ...
  if (WizMote.comm_id == WizMote.id) {           // Commissioned id
    switch(WizMote.index) {
      ...
      case 8:      // Bright up
      case 9:      // Bright down
        if (InfinitekGlobal.light_type) {          // Any light
          char scmnd[16];
  ...
  if (Settings->flag6.mqtt_disable_publish ) {  // SetOption147 - If it is activated, Infinitek will not publish MQTT message, but it will proccess event trigger rules
    XdrvRulesProcess(0);
  } else {
...
void EspNowInit(void) {
  if (0 == Settings->flag4.network_wifi) {   // WiFi Off ..
    if (!InfinitekGlobal.global_state.wifi_down) { return; }  // .. and processed by WifiDisable()
    if (1 == WizMote.active) {
