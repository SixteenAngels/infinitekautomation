...
// call the event dispatcher from Infinitek object
...
int32_t callBerryEventDispatcher(const char *type, const char *cmd, int32_t idx, const char *payload, uint32_t data_len) {
  ...
  checkBeTop();
  be_getglobal(vm, PSTR("Infinitek"));
  if (!be_isnil(vm, -1)) {
...
// simple wrapper to call `Infinitek.<method_name>()`
static void callBerryInfinitekFunc(const char * method_name) {
  bvm *vm = berry.vm;
  if (be_getglobal(vm, "Infinitek")) {
...
// Simplified version of event loop. Just call `Infinitek.fast_loop()`
// `every_5ms` is a flag to wait at least 5ms between calss to `Infinitek.fast_loop()`
void callBerryFastLoop(bool every_5ms) {
  ...
  callBerryInfinitekFunc("fast_loop");
}

// call `Infinitek.run_immediate()`
void callBerryRunDeferred(void) {
  if (nullptr == berry.vm) { return; }
  callBerryInfinitekFunc("run_deferred");
}
...
void BrLoad(const char * script_name) {
  if (berry.vm == nullptr || InfinitekGlobal.no_autoexec) { return; }   // abort is berry is not running, or bootloop prevention kicked in
...
const char HTTP_BERRY_FORM_CMND[] PROGMEM =
  ...
  "<div contenteditable='false' class='br0 bro' readonly id='t1' cols='340' wrap='off'>"
    "<div class='br1'>Welcome to the Berry Scripting console. "
      "Check the <a href='https://Infinitek.github.io/docs/Berry/' target='_blank'>documentation</a>."
    "</div>"
  "</div>"
...
bool Xdrv52(uint32_t function)
{
  ...
  switch (function) {
    case FUNC_SLEEP_LOOP:
      if (InfinitekGlobal.berry_deferred_ready) {      // there are immediate functions registered, call them first
        callBerryRunDeferred();      // call `Infinitek.run_immediate()`
      }
      if (InfinitekGlobal.berry_fast_loop_enabled) {    // call only if enabled at global level
        callBerryFastLoop(true);      // call `Infinitek.fast_loop()` optimized for minimal performance impact
      }
      break;
    case FUNC_LOOP:
      ...
      if (InfinitekGlobal.berry_fast_loop_enabled) {    // call only if enabled at global level
        callBerryFastLoop(false);      // call `Infinitek.fast_loop()` optimized for minimal performance impact
      }
      break;
