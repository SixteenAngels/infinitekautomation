/*
  xdrv_24_Buzzer.ino - buzzer support for Infinitek
...
struct BUZZER {
  ...
  uint8_t tune_size = 0;
  uint8_t size = 0;
  uint8_t sleep;                   // Current copy of InfinitekGlobal.sleep
} Buzzer;
...
void BuzzerBeep(uint32_t count, uint32_t on, uint32_t off, uint32_t tune, uint32_t mode) {
  ...
  if (Buzzer.enable) {
    Buzzer.sleep = InfinitekGlobal.sleep;
    if (Settings->sleep > PWM_MAX_SLEEP) {
      InfinitekGlobal.sleep = PWM_MAX_SLEEP;   // Set a maxumum value of 10 milliseconds to ensure that buzzer periods are a bit more accurate
    } else {
      InfinitekGlobal.sleep = Settings->sleep;  // Or keep the current sleep if it's lower than 10
    }
  } else {
    InfinitekGlobal.sleep = Buzzer.sleep;      // Restore original sleep
    BuzzerSet(0);
  }
}
