/*
  xdrv_81_esp32_webcam.ino - ESP32 webcam support for Infinitek
...
//    }
  ...
  AddLog(LOG_LEVEL_DEBUG, PSTR("CAM: i2c_enabled_2: %d"), InfinitekGlobal.i2c_enabled[1]);

  if (!PinUsed(GPIO_WEBCAM_XCLK) || !PinUsed(GPIO_WEBCAM_PCLK) ||
      !PinUsed(GPIO_WEBCAM_VSYNC) || !PinUsed(GPIO_WEBCAM_HREF) ||
      ((!PinUsed(GPIO_WEBCAM_SIOD) || !PinUsed(GPIO_WEBCAM_SIOC)) && !InfinitekGlobal.i2c_enabled[1])    // preferred option is to reuse and share I2Cbus 2
      ) {
        pin_used = false;
  }
  return pin_used;
...
//esp_log_level_set("*", ESP_LOG_VERBOSE);
  ...
  if (WcPinUsed()) {
    ...
    if(InfinitekGlobal.i2c_enabled[1]){              // configure SIOD and SIOC as SDA,2 and SCL,2
...
//  AddLog(LOG_LEVEL_INFO, PSTR("CAM: heap check 2: %d"),ESP_getFreeHeap());
  ...
  InfinitekGlobal.camera_initialized = true;
...
#ifdef ENABLE_RTSPSERVER
    if (Settings->webcam_config.rtsp && !InfinitekGlobal.global_state.wifi_down && Wc.up) {
      if (!Wc.rtsp_start) {
