/*
  xdrv_22_sonoff_ifan.ino - sonoff iFan02, iFan03 and iFan04 support for Infinitek
...
bool IsModuleIfan(void) {
  return ((SONOFF_IFAN02 == InfinitekGlobal.module_type) || (SONOFF_IFAN03 == InfinitekGlobal.module_type));
}
...
uint8_t GetFanspeed(void) {
  if (Ifan.fanspeed_timer) {
    ...
    */
    uint8_t fanspeed = (uint8_t)(InfinitekGlobal.power &0xF) >> 1;
...
void SonoffIFanSetFanspeed(uint8_t fanspeed, bool sequence) {
  ...
  if (SONOFF_IFAN03 == InfinitekGlobal.module_type) {
    if (sequence) {
...
void SonoffIfanReceived(void) {
  ...
  uint8_t mode = InfinitekGlobal.serial_in_buffer[3];
  uint8_t action = InfinitekGlobal.serial_in_buffer[6];
  ...
  InfinitekGlobal.serial_in_buffer[5] = 0;                      // Ack
  InfinitekGlobal.serial_in_buffer[6] = 0;                      // Crc
  for (uint32_t i = 0; i < 7; i++) {
    if ((i > 1) && (i < 6)) { InfinitekGlobal.serial_in_buffer[6] += InfinitekGlobal.serial_in_buffer[i]; }
    Serial.write(InfinitekGlobal.serial_in_buffer[i]);
  }
}

bool SonoffIfanSerialInput(void) {
  if (SONOFF_IFAN03 != InfinitekGlobal.module_type) { return false; }
  ...
  if (Ifan.receive_flag) {
    ...
    if (InfinitekGlobal.serial_in_byte_counter == 8) {
      ...
      for (uint32_t i = 2; i < 7; i++) {
        crc += InfinitekGlobal.serial_in_buffer[i];
      }
      if (crc == InfinitekGlobal.serial_in_buffer[7]) {
        SonoffIfanReceived();
...
void SonoffIFanParameters(void) {
  if (InfinitekGlobal.gpio_optiona.ifan04_h) {     // GPIO Option_A8
...
void SonoffIfanUpdate(void) {
  ...
  if (Ifan.restart_flag && (4 == InfinitekGlobal.uptime) && (SONOFF_IFAN02 == InfinitekGlobal.module_type)) {  // Microcontroller needs 3 seconds before accepting commands
    ...
    SetDevicePower(InfinitekGlobal.power, SRC_RETRY);  // Set required power on state
  }
}
