/*
  xdrv_42_i2s_mp3mic.ino - Audio dac support for Infinitek
  ...
  public:
    ...
    virtual uint32_t begin(uint32_t samplingRate, uint32_t inputChannels) {
      ...
      if (info == nullptr) {return 8;}
	    info->set_writing_app("Infinitek");
	    // Add an audio track.
...
void I2sMicTask(void *arg){
  ...
  while (!audio_i2s_mp3.mic_stop) {
    ...
    audio_i2s_mp3.recdur = InfinitekGlobal.uptime - ctime;
