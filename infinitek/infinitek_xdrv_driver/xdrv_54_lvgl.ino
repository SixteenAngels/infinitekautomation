...
#include "Infinitek_lvgl_assets.h"    // force compilation of assets
...
bool Xdrv54(uint32_t function) {
  ...
  switch (function) {
    case FUNC_LOOP:
      if (lvgl_glue) {
        if (InfinitekGlobal.sleep > USE_LVGL_MAX_SLEEP) {
          InfinitekGlobal.sleep = USE_LVGL_MAX_SLEEP;   // sleep is max 10ms
        }
        lv_task_handler();
      }
      break;
