/*
  xdrv_121_gpioviewer.ino - GPIOViewer for Infinitek
 ...
 *    Infinitek hosted: https://ota.Infinitek.com/Infinitek/release-13.4.0/gpio_viewer_1_5/
 * 
 * Supported commands:
...
//#define GV_BASE_URL           "https://ota.Infinitek.com/Infinitek/gpioviewer/gpio_viewer_13_4_0/"  // [GvUrl]
#endif
...
int8_t GVDigitalPinToAnalogChannel(uint8_t pin) {
  ...
  if (pin < SOC_GPIO_PIN_COUNT) {
    for (uint8_t j = 0; j < SOC_ADC_MAX_CHANNEL_NUM; j++) {
      if (adc_channel_io_map[0][j] == pin) {  // Infinitek supports ADC1 only
        return channel;
      }
      channel++;
    }
...
//  GVWebClient.setSync(true);
  ...
  if (GV->sampling != 100) {
    GV->ticker.attach_ms(GV->sampling, GVMonitorTask);      // Use Infinitek Scheduler (100) or Ticker (20..99,101..1000)
  }
