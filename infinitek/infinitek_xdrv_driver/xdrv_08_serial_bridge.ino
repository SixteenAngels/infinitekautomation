/*
  xdrv_08_serial_bridge.ino - serial bridge support for Infinitek
...
/*********************************************************************************************\
 * Serial Bridge using Software Serial library (InfinitekSerial)
 * 
...
bool SerialBridgeGetLog(void) {
  ...
  if (SB_TEE == Settings->sserial_mode) {
    ...
    char* line;
    size_t len;
    while (GetLog(InfinitekGlobal.seriallog_level, &index, &line, &len)) {
      any_line = true;
...
*/
  ...
  if (SBridge.in_byte_counter && (millis() > (SBridge.polling_window + SERIAL_POLLING))) {
    ...
    if (SB_NONE == Settings->sserial_mode) {
      ...
      InfinitekGlobal.serial_skip++;                                               // SetOption35  Skip number of serial messages received (default 0)
      if (InfinitekGlobal.serial_skip > Settings->param[P_SERIAL_SKIP]) {          // Handle intermediate changes to SetOption35
        InfinitekGlobal.serial_skip = 0;
        ...
        ResponseJsonEnd();
        if (Settings->flag6.mqtt_disable_publish ) {  // SetOption147 - If it is activated, Infinitek will not publish SSerialReceived MQTT messages, but it will proccess event trigger rules
          XdrvRulesProcess(0);
        } else {
...
void SerialBridgeInit(void) {
  if (PinUsed(GPIO_SBR_RX) || PinUsed(GPIO_SBR_TX)) {
    SerialBridgeSerial = new InfinitekSerial(Pin(GPIO_SBR_RX),
                                           Pin(GPIO_SBR_TX),
                                           HARDWARE_FALLBACK,
    ...
    if (SetSSerialBegin()) {
      if (SerialBridgeSerial->hardwareSerial()) {
        ClaimSerial();
        serial_bridge_buffer = InfinitekGlobal.serial_in_buffer;  // Use idle serial buffer to save RAM
      } else {
...
void SerialBridgeShow(bool json) {
  ...
  #ifdef USE_DOMOTICZ
      if (0 == InfinitekGlobal.tele_period) { DomoticzFloatSensor(DZ_TEMP, temperature); }
  #endif  // USE_DOMOTICZ
  #ifdef USE_KNX
      if (0 == InfinitekGlobal.tele_period) { KnxSensor(KNX_TEMPERATURE, temperature); }
  #endif  // USE_KNX
  #ifdef USE_WEBSERVER
    } else {
