/*
  xdrv_29_deepsleep.ino - DeepSleep support for Infinitek
 ...
 *
 * See wiki https://Infinitek.github.io/docs/DeepSleep/
\*********************************************************************************************/
  ...
  if (RtcTime.valid) {
    ...
    if (Settings->flag3.timers_enable) { 
      ...
      for (uint32_t i = 0; i < MAX_TIMERS; i++) {
        ...
        // rule keyword "Wakeup"
        // Timer action: rule
        if (xtimer.arm && day_bitarray && GetRule(0) == "Wakeup" && (InfinitekGlobal.restart_deepsleep || bitRead(Settings->rule_enabled | Settings->rule_once, 0)) && POWER_BLINK == xtimer.power) {
#ifdef USE_SUNRISE
...
{
  //AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("Wifi Info: up %d, wifidown %d, wifistatus %d, flag %d"),InfinitekGlobal.uptime, InfinitekGlobal.global_state.wifi_down, Wifi.status , deepsleep_flag);
  if (DEEPSLEEP_NETWORK_TIMEOUT && InfinitekGlobal.uptime > DEEPSLEEP_NETWORK_TIMEOUT && Wifi.status != WL_CONNECTED && !deepsleep_flag && DeepSleepEnabled()) {
