/*
  xdrv_01_webserver.ino - webserver for Infinitek
...
#define RESTART_AFTER_INITIAL_WIFI_CONFIG         true   // Restart Infinitek after initial Wifi Config of a blank device
#endif                                                   //   If disabled, Infinitek will keep both the wifi AP and the wifi connection to the router
...
#define AFTER_INITIAL_WIFI_CONFIG_GO_TO_NEW_IP    true   //   the user will be redirected to the new IP of Infinitek (in the new Network).
...
const char HTTP_END[] PROGMEM =
  "<div style='text-align:right;font-size:11px;'><hr/><a href='https://github.com/arendst/Infinitek' target='_blank' style='color:#aaa;'>Infinitek %s %s " D_BY " Theo Arends</a></div>"
  "</div>"
  "</body>"
  "</html>";
...
//      Webserver->on(F("/u2"), HTTP_POST, HandleUploadDone, HandleUploadLoop);  // this call requires 2 functions so we keep a direct call
  ...
  if (Web.state != type) {
    ...
    InfinitekGlobal.rules_flag.http_init = 1;
    Web.state = type;
  }
}
...
bool HttpCheckPriviledgedAccess(bool autorequestauth = true) {
  ...
  if (!Settings->flag5.disable_referer_chk && !WifiIsInManagerMode()) {
    ...
    if (referer.length()) {
      referer.toUpperCase();
      String hostname = InfinitekGlobal.hostname;
...
void WSHeaderSend(void) {
  char server[32];
  snprintf_P(server, sizeof(server), PSTR("Infinitek/%s (%s)"), InfinitekGlobal.version, GetDeviceHardware().c_str());
...
//    WSContentSend_P(PSTR("body{background:%s;background-repeat:no-repeat;background-attachment:fixed;background-size:cover;}"), SettingsText(SET_CANVAS));
  ...
  if ( Settings->flag3.gui_hostname_ip || ( (WiFi.getMode() == WIFI_AP_STA) && (!Web.initial_config) )  ) {
    ...
    bool eip = false;
    if (lip || sip) {
      WSContentSend_P(PSTR("<h4>%s%s (%s%s%s)"),    // Infinitek.local (192.168.2.12, 192.168.4.1)
...
//#if defined(ESP32) && CONFIG_IDF_TARGET_ESP32 && defined(USE_ETHERNET)
    ...
    eip = EthernetHasIP();
    if (eip) {
      WSContentSend_P(PSTR("%s%s%s (%s)"),          // Infinitek-eth.local (192.168.2.13)
...
void WSContentStop(void) {
  ...
  WSContentSend_P(HTTP_END, InfinitekGlobal.version, InfinitekGlobal.image_name);
  WSContentEnd();
}
...
int32_t IsShutterWebButton(uint32_t idx) {
  ...
  if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
    for (uint32_t i = 0; i < InfinitekGlobal.shutters_present ; i++) {
...
void WebGetDeviceCounts(void) {
  Web.buttons_non_light_non_shutter = InfinitekGlobal.devices_present;
  ...
  if (Settings->flag3.shutter_mode) {  // SetOption80 - Enable shutter support
    ...
    for (uint32_t button_idx = 1; button_idx <= InfinitekGlobal.devices_present; button_idx++) {
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("HTP: DP %d, BNLNS %d, SB %08X"), InfinitekGlobal.devices_present, Web.buttons_non_light_non_shutter, Web.light_shutter_button_mask);
}
...
void HandleRoot(void) {
  ...
  */
  ...
  if (InfinitekGlobal.devices_present) {
    WebGetDeviceCounts();
      ...
      if (IsModuleIfan()) {
        ...
        for (uint32_t button_idx = 1; button_idx <= InfinitekGlobal.devices_present; button_idx++) {
          ...
          WSContentSend_P(HTTP_DEVICE_CONTROL, 100 / cols, button_idx, button_idx,
            ...
            (set_button) ? "" : (InfinitekGlobal.devices_present > 1) ? stemp : "");
          button_ptr++;
...
#ifdef USE_SHUTTER
    if (InfinitekGlobal.shutters_present) {  // Any shutter present - Show shutter buttons and slider
      WSContentSend_P(HTTP_TABLE100);      // "<table style='width:100%%'>"
      uint32_t shutter_button_idx;
      uint32_t shutter_button_idx_temp;
      for (uint32_t shutter_idx = 0; shutter_idx < InfinitekGlobal.shutters_present; shutter_idx++) {
        WSContentSend_P(PSTR("<tr>"));
...
//          AddLog(LOG_LEVEL_DEBUG, PSTR("SHT: j %d, shutter_idx %d, shutter_button_idx %d, shutter_idx %d, shutter_button_idx_temp %d"), j, shutter_idx, shutter_button_idx, shutter_idx, shutter_button_idx_temp);
...
#ifdef USE_LIGHT
    if (InfinitekGlobal.light_type) {        // Any light - Show light button and slider(s)
      uint32_t light_device = LightDevice();
      uint32_t light_devices = LightDevices();
      ...
      uint8_t light_subtype = InfinitekGlobal.light_type &7;
  ...
  uint32_t max_devices = InfinitekGlobal.devices_present;
  ...
  for (uint32_t idx = 1; idx <= max_devices; idx++) {
    bool not_active = !bitRead(InfinitekGlobal.power, idx -1);
...
bool HandleRootStatusRefresh(void) {
...
#endif  // USE_LIGHT
#ifdef USE_SHUTTER
  for (uint32_t j = 1; j <= InfinitekGlobal.shutters_present; j++) {
    uint8_t percent;
  ...
  if (InfinitekGlobal.devices_present) {
    ...
    for (uint32_t idx = 1; idx <= max_devices; idx++) {
      bool active = bitRead(InfinitekGlobal.power, idx -1);
...
#ifdef USE_SONOFF_IFAN
      if (IsModuleIfan()) {
        WSContentSend_P(HTTP_DEVICE_STATE, 36, (bitRead(InfinitekGlobal.power, 0)) ? PSTR("bold") : PSTR("normal"), 54, GetStateText(bitRead(InfinitekGlobal.power, 0)));
        ...
        for (uint32_t button_idx = 1; button_idx <= InfinitekGlobal.devices_present; button_idx++) {
          ...
          bool power_state = bitRead(InfinitekGlobal.power, button_idx -1);
...
void HandleModuleConfiguration(void) {
  ...
  for (uint32_t i = 0; i < nitems(template_gp.io); i++) {
    if (ValidGPIO(i, template_gp.io[i])) {
      WSContentSend_P(PSTR("sk(%d,%d);"), InfinitekGlobal.my_module.io[i], i);  // g0 - g17
    }
  }
  ...
  for (uint32_t i = 0; i < nitems(template_gp.io); i++) {
    if (ValidGPIO(i, template_gp.io[i])) {
      ...
      WSContentSend_P(PSTR("<tr><td style='width:116px'>%s <b>" D_GPIO "%d</b></td><td style='width:146px'><select id='g%d' onchange='ot(%d,this.value)'></select></td>"),
        (WEMOS==InfinitekGlobal.module_type)?stemp:"", i, i, i);
      WSContentSend_P(PSTR("<td style='width:54px'><select id='h%d'></select></td></tr>"), i);
    }
  }
  WSContentSend_P(PSTR("</table>"));
...
void HandleWifiConfiguration(void) {
  ...
  if (Webserver->hasArg(F("save")) && HTTP_MANAGER_RESET_ONLY != Web.state) {
    if ( WifiIsInManagerMode() ) {
      ...
      // As Infinitek is in this case in AP mode, a STA connection can be established too at the same time
...
//      InfinitekGlobal.blinks = 0;                          // Disable blinks initiated by WifiManager
      ...
      AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_CONNECTING_TO_AP " %s " D_AS " %s ..."),
        SettingsText(SET_STASSID1), InfinitekGlobal.hostname);
...
void HandleInformation(void) {
  ...
  WSContentSend_P(PSTR(D_PROGRAM_VERSION "}2%s %s %s"), InfinitekGlobal.version, InfinitekGlobal.image_name, GetCodeCores().c_str());
  ...
  uint32_t maxfn = (InfinitekGlobal.devices_present > MAX_FRIENDLYNAMES) ? MAX_FRIENDLYNAMES : InfinitekGlobal.devices_present;
#ifdef USE_SONOFF_IFAN
  if (IsModuleIfan()) { maxfn = 1; }
#endif  // USE_SONOFF_IFAN
  for (uint32_t i = 0; i < maxfn; i++) {
  ...
  if (Settings->flag4.network_wifi) {
    ...
    WSContentSend_P(PSTR("}1" D_AP "%d " D_INFORMATION "}2" D_SSID " %s<br>" D_RSSI " %d%%, %d dBm<br>" D_MODE " %s<br>" D_CHANNEL " %d<br>" D_BSSID " %s"), 
      ...
      WiFi.BSSIDstr().c_str());
    WSContentSeparatorIFat();
    WSContentSend_P(PSTR("}1" D_HOSTNAME "}2%s%s"), InfinitekGlobal.hostname, (Mdns.begun) ? PSTR(".local") : "");
#ifdef USE_IPV6
...
//#if defined(ESP32) && CONFIG_IDF_TARGET_ESP32 && defined(USE_ETHERNET)
  ...
  if (!InfinitekGlobal.global_state.eth_down) {
  ...
  if (Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    ...
    WSContentSend_P(PSTR("}1" D_MQTT_CLIENT "}2%s"), InfinitekGlobal.mqtt_client);
    ...
    WSContentSend_P(PSTR("}1" D_MQTT_FULL_TOPIC "}2%s"), GetTopic_P(stopic, CMND, InfinitekGlobal.mqtt_topic, ""));
  ...
  WSContentSend_P(PSTR("}1" D_FLASH_CHIP_ID "}20x%06X (" D_Infinitek_FLASHMODE ")"), ESP_getFlashChipId());
#ifdef ESP32
...
#if defined(USE_ZIGBEE_EZSP) || defined(USE_Infinitek_CLIENT) || defined(SHELLY_FW_UPGRADE) || defined(USE_RF_FLASH) || defined(USE_CCLOADER)
...
void HandleUpgradeFirmware(void) {
  ...
  Web.upload_file_type = UPL_Infinitek;
}
...
void HandleUploadDone(void) {
  ...
  WSContentStart_P(PSTR(D_INFORMATION));
  if (!Web.upload_error) {
    WSContentSend_P(HTTP_SCRIPT_RELOAD_TIME, (UPL_Infinitek == Web.upload_file_type) ? HTTP_OTA_RESTART_RECONNECT_TIME : HTTP_RESTART_RECONNECT_TIME);  // Refesh main web ui after OTA upgrade
  }
  WSContentSendStyle();
  ...
  if (Web.upload_error) {
    ...
    InfinitekGlobal.restart_flag = 2;  // Always restart to re-enable disabled features during update
    WSContentSend_P(HTTP_MSG_RSTRT);
    ShowWebSource(SRC_WEBGUI);
  }
  SettingsBufferFree();
...
void HandleUploadLoop(void) {
  ...
  if (Web.upload_error) {
    if (!upload_error_signalled) {
      if (UPL_Infinitek == Web.upload_file_type) { Update.end(); }
      UploadServices(1);
...
#ifdef USE_WEB_FW_UPGRADE
#ifdef USE_RF_FLASH
      else if ((SONOFF_BRIDGE == InfinitekGlobal.module_type) && (':' == upload.buf[0])) {  // Check if this is a RF bridge FW file
        BUploadInit(UPL_EFM8BB1);
      }
#endif  // USE_RF_FLASH
#ifdef USE_Infinitek_CLIENT
      else if (InfinitekClient_Available() && (':' == upload.buf[0])) {  // Check if this is a ARDUINO CLIENT hex file
        BUploadInit(UPL_InfinitekCLIENT);
      }
...
#ifdef ESP8266
      else if ((SONOFF_ZB_BRIDGE == InfinitekGlobal.module_type) && (0xEB == upload.buf[0])) {  // Check if this is a Zigbee bridge FW file
#endif  // ESP8266
#ifdef ESP32
...
#endif  // USE_ZIGBEE_EZSP
#endif  // USE_WEB_FW_UPGRADE
      else if (UPL_Infinitek == Web.upload_file_type) {
...
#endif  // USE_RF_FLASH
#ifdef USE_Infinitek_CLIENT
      if (UPL_InfinitekCLIENT == Web.upload_file_type) {
        error = InfinitekClient_Flash(data, BUpload.spi_hex_size);
      }
#endif  // USE_Infinitek_CLIENT
#ifdef SHELLY_FW_UPGRADE
...
void HandleHttpCommand(void) {
  ...
  if (!WebAuthenticate()) {
    ...
    if (!(!strcmp(tmp1, WEB_USERNAME) && !strcmp(tmp2, SettingsText(SET_WEBPWD)))) {
      ...
      // https://github.com/arendst/Infinitek/discussions/15420
  ...
  if (svalue.length() && (svalue.length() < MQTT_MAX_PACKET_SIZE)) {
    uint32_t curridx = InfinitekGlobal.log_buffer_pointer;
    InfinitekGlobal.templog_level = LOG_LEVEL_INFO;
    ...
    char* line;
    size_t len;
    while (GetLog(InfinitekGlobal.templog_level, &index, &line, &len)) {
    ...
    WSContentSend_P(PSTR("}"));
    InfinitekGlobal.templog_level = 0;
  } else {
...
void HandleConsoleRefresh(void) {
  ...
  WSContentBegin(200, CT_PLAIN);
  WSContentSend_P(PSTR("%d}1%d}1"), InfinitekGlobal.log_buffer_pointer, Web.reset_web_log_flag);
...
int WebSend(char *buffer) {
  // [Infinitek] POWER1 ON                                               --> Sends http://Infinitek/cm?cmnd=POWER1 ON
  // [192.168.178.86:80,admin:joker] POWER1 ON                        --> Sends http://hostname:80/cm?user=admin&password=joker&cmnd=POWER1 ON
  // [Infinitek] /any/link/starting/with/a/slash.php?log=123             --> Sends http://Infinitek/any/link/starting/with/a/slash.php?log=123
  // [Infinitek,admin:joker] /any/link/starting/with/a/slash.php?log=123 --> Sends http://Infinitek/any/link/starting/with/a/slash.php?log=123
...
void CmndWebGetConfig(void) {
  // WebGetConfig http://myserver:8000/Infinitek/conf/%id%.dmp where %id% is expanded to device mac address
  // WebGetConfig http://myserver:8000/Infinitek/conf/Config_demo_9.5.0.8.dmp
  if (XdrvMailbox.data_len > 0) {
...
#ifndef FIRMWARE_MINIMAL      // if Infinitek-minimal, read only and don't parse JSON
...
void CmndWebCanvas(void) {
  ...
  - WebCanvas url(http://ota.Infinitek.com/Infinitek/images/prf.png)           // Pride flag
  - WebCanvas url(http://ota.Infinitek.com/Infinitek/images/Infinitek_logo.png)  // Infinitek logo
  */
  if (XdrvMailbox.data_len > 0) {
...
//          InfinitekGlobal.blinks = 255;                    // Signal wifi connection with blinks
          if (MAX_WIFI_OPTION != Wifi.old_wificonfig) {
            InfinitekGlobal.wifi_state_flag = Settings->sta_config = Wifi.old_wificonfig;
          }
          InfinitekGlobal.save_data_counter = Wifi.save_data_counter;
          Settings->save_data = Wifi.save_data_counter;
          SettingsSaveAll();

          if ( Wifi.wifi_Test_Restart ) { InfinitekGlobal.restart_flag = 2; }
          ...
          switch (WiFi.status()) {
            ...
            default:  // WL_IDLE_STATUS and WL_DISCONNECTED - SSId in range but no answer from the router
              ...
              // If this error occurs twice, Infinitek will connect directly to the router without testing credentials.
              ...
              //   Infinitek will restart and try to connect in STA mode only (11b/g/n).
              //
              //   If it fails again, depending on the WIFICONFIG settings, the user will need to wait or will need to
              //   push 6 times the button to enable Infinitek AP mode again.
              if (Wifi.wifi_test_AP_TIMEOUT) {
                Wifi.wifiTest = WIFI_TEST_FINISHED;
