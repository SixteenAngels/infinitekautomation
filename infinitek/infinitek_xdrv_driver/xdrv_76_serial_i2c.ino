...
protected:
  ...
  InfinitekSerial serial;                     // InfinitekSerial instance to communicate with SC18IM704
...
public:
  ...
  TwoWireSerial(uint8_t bus_num, uint8_t _tx, uint8_t _rx) :
    tx(_tx),
    rx(_rx),
    serial(rx, tx),                         // InfinitekSerial constructor
  ...
  ~TwoWireSerial() {
    // nothing to do here:
    // - InfinitekSerial destructor is implcitly called after this
...
// - register serial bus with Infinitek
void I2CSerialInit(void) {
  ...
  for (uint32_t bus = 0; bus < MAX_I2C; bus++) {
    if (PinUsed(GPIO_I2C_SER_TX, bus) && PinUsed(GPIO_I2C_SER_RX, bus)) {
      ...
      if (InfinitekGlobal.i2c_enabled[bus]) {
  ...
  if (I2C_Serial.active) {
    ...
    if (I2C_Serial.wire_serial->beginSerial()) {
      InfinitekGlobal.i2c_enabled[I2C_Serial.bus] = true;          // enable at Infinitek level
...
def process_cmd_version():
    ignore_until_P()
    uart.write(b'Infinitek I2C uart bridge 1.0\x00')
    return None
