/*
  xdrv_73_6_lorawan_decode.ino - LoRaWan node decode support for Infinitek
...
#ifdef ESP8266
  if (!Settings->flag6.mqtt_disable_publish) {        // SetOption147 - If it is activated, Infinitek will not publish MQTT messages, but it will proccess event trigger rules
#else  // ESP32
  bool decode_successful = false;
  if (!decoded) {
    String mqtt_data = InfinitekGlobal.mqtt_data;       // Backup as being destroyed by berry
    uint32_t restart_flag = InfinitekGlobal.restart_flag; // Backup restart_flag
    InfinitekGlobal.restart_flag += 17;                 // Set to non-zero (default) state
    XdrvRulesProcess(0);                              // Apply berry decoding which may reset InfinitekGlobal.restart_flag
    decode_successful = (0 == InfinitekGlobal.restart_flag);
    InfinitekGlobal.restart_flag = restart_flag;        // Restore restart_flag
    InfinitekGlobal.mqtt_data = mqtt_data;              // Restore response data
  }
  if (!decode_successful &&
      !Settings->flag6.mqtt_disable_publish) {        // SetOption147 - If it is activated, Infinitek will not publish MQTT messages, but it will proccess event trigger rules
#endif  // ESP32
    if (Settings->flag4.zigbee_distinct_topics) {     // SetOption89  - (MQTT, Zigbee) Distinct MQTT topics per device for Zigbee (1) (#7835)
      ...
      if (Settings->flag5.zigbee_hide_bridge_topic) { // SetOption125 - (Zigbee) Hide bridge topic from zigbee topic (use with SetOption89) (1)
        ...
        snprintf_P(subtopic, sizeof(subtopic), PSTR("%s/%s"), InfinitekGlobal.mqtt_topic, stemp);
      }
      char stopic[TOPSZ];
