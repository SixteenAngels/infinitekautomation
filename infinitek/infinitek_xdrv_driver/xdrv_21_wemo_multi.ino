/*
  xdrv_21_wemo_multi.ino - multiple wemo support for Infinitek
...
private:
  ...
  void HandleUpnpEvent() {
    ...
    snprintf_P(event, sizeof(event), setup_xml.c_str(), state, bitRead(InfinitekGlobal.power, _deviceId - 1), state);
...
#ifdef USE_EMULATION_WEMO_DEBUG
        AddLog(LOG_LEVEL_DEBUG, PSTR("WMO: Adding handlers for %d devices"), InfinitekGlobal.devices_present);
#endif
