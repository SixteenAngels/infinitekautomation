/*
  xsns_56_rtc_chips.ino - RTC chip support for Infinitek
...
void RtcChipEverySecond(void) {
  ...
  if (InfinitekGlobal.global_state.network_down) { return; }  // Exception on ESP32 if network is down (#17338)
...
void CmndRtcNtpServer(void) {
  ...
  if ((XdrvMailbox.payload >= 0) && !InfinitekGlobal.global_state.network_down) {
