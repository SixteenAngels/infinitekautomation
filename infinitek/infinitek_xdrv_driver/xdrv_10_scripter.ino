/*
  xdrv_10_scripter.ino - script support for Infinitek
...
void Script_Init_UDP() {
  if (InfinitekGlobal.global_state.network_down) return;
  if (!glob_script_mem.udp_flags.udp_used) return;
...
// convert Infinitek time stamp to ul seconds
...
// convert seconds to Infinitek time stamp
    ...
    switch (vname[0]) {
      ...
      case 'a':
        ...
        if (!strncmp_XP(lp, XPSTR("ap("), 3)) {
          //InfinitekGlobal.restart_flag = 216;
      ...
      case 'b':
        if (!strncmp_XP(vname, XPSTR("boot"), 4)) {
          if (InfinitekGlobal.rules_flag.system_boot) {
            InfinitekGlobal.rules_flag.system_boot = 0;
            fvar = 1;
          }
          goto exit;
        }
...
extern void W8960_SetGain(uint8_t sel, uint16_t value);
        ...
        if (!strncmp_XP(vname, XPSTR("ctper"), 5)) {
          fvar = InfinitekGlobal.tele_period;
          goto exit;
        }
        break;
      case 'd':
...
_Pragma("GCC warning \"'EXT 1 wakeup' not supported using gpio mode\"")
      ...
      case 'e':
        ...
        if (!strncmp_XP(vname, XPSTR("ethdwn"), 6)) {
          fvar = InfinitekGlobal.global_state.eth_down;
          goto exit;
        }
        break;
      case 'f':
//#define DEBUG_FS
      ...
      case 'g':
        if (!strncmp_XP(vname, XPSTR("gtmp"), 4)) {
          fvar = InfinitekGlobal.temperature_celsius;
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("ghum"), 4)) {
          fvar = InfinitekGlobal.humidity;
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("gprs"), 4)) {
          fvar = InfinitekGlobal.pressure_hpa;
          goto exit;
        }
  ...
  exit_settable:
      ...
      case 'm':
        ...
        if (!strncmp_XP(vname, XPSTR("mqttc"), 5)) {
          if (InfinitekGlobal.rules_flag.mqtt_connected) {
            InfinitekGlobal.rules_flag.mqtt_connected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("mqttd"), 5)) {
          if (InfinitekGlobal.rules_flag.mqtt_disconnected) {
            InfinitekGlobal.rules_flag.mqtt_disconnected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("mqtts"), 5)) {
          fvar = !InfinitekGlobal.global_state.mqtt_down;
          goto exit;
        }
      ...
      case 'n':
        if (!strncmp_XP(vname, XPSTR("npwr"), 4)) {
          fvar = InfinitekGlobal.devices_present;
          goto exit;
        }
        break;
...
int32_t I2SPlayFile(const char *path, uint32_t decoder_type);
      ...
      case 'p':
        ...
        if (!strncmp_XP(lp, XPSTR("pd["), 3)) {
          ...
          if ((gpiopin < nitems(InfinitekGlobal.gpio_pin)) && (InfinitekGlobal.gpio_pin[gpiopin] > 0)) {
            fvar = InfinitekGlobal.gpio_pin[gpiopin];
            // skip ] bracket
            len++;
            goto exit;
          }
        ...
        if (!strncmp_XP(lp, XPSTR("pwr["), 4)) {
          GetNumericArgument(lp + 4, OPER_EQU, &fvar, gv);
          uint8_t index = fvar;
          if (index <= InfinitekGlobal.devices_present) {
            fvar = bitRead(InfinitekGlobal.power, index - 1);
          } else {
            fvar = -1;
          }
          len += 1;
...
*/
      ...
      case 's':
        ...
        if (!strncmp_XP(lp, XPSTR("sht["), 4)) {
          GetNumericArgument(lp + 4, OPER_EQU, &fvar, gv);
          uint8_t index = fvar;
          if (index <= InfinitekGlobal.shutters_present) {
        ...
        if (!strncmp_XP(lp, XPSTR("so("), 3)) {
          ...
          if (glob_script_mem.sp) {
            ...
            glob_script_mem.sp = new InfinitekSerial(rxpin, txpin, HARDWARE_FALLBACK, 0, rxbsiz);
...
#define SCRIPT_HSTR_LEN 32
          ...
          switch (sel) {
            case 0:
              ...
              if (glob_script_mem.spi.sclk < 0) {
                // attach to existing Infinitek SPI
                lp = GetNumericArgument(lp , OPER_EQU, &fvar, 0);
                fvar *= 1000000;
      ...
      case 't':
        ...
        if (!strncmp_XP(vname, XPSTR("tinit"), 5)) {
          fvar = InfinitekGlobal.rules_flag.time_init;
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("tset"), 4)) {
          fvar = InfinitekGlobal.rules_flag.time_set;
          goto exit;
        }
        ...
        if (!strncmp_XP(vname, XPSTR("topic"), 5)) {
          if (sp) strlcpy(sp, InfinitekGlobal.mqtt_topic, glob_script_mem.max_ssize);
          goto strexit;
        }
#ifdef USE_SCRIPT_TIMER
...
*/
        ...
        if (!strncmp_XP(vname, XPSTR("wific"), 5)) {
          if (InfinitekGlobal.rules_flag.wifi_connected) {
            InfinitekGlobal.rules_flag.wifi_connected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("wifid"), 5)) {
          if (InfinitekGlobal.rules_flag.wifi_disconnected) {
            InfinitekGlobal.rules_flag.wifi_disconnected = 0;
            fvar = 1;
          }
          goto exit;
        }
        if (!strncmp_XP(vname, XPSTR("wifis"), 5)) {
          fvar = !InfinitekGlobal.global_state.wifi_down;
          goto exit;
        }
  ...
  switch (sel) {
    case 0:
      if (val & 0x8000) {
        ...
        val &= 0x7fff;
        ow->ts = new InfinitekSerial(val & 0xff, (val >> 8) & 0x7f, 1, 0, 64);
        if (ow->ts) {
...
bool Is_gpio_used(uint8_t gpiopin) {
  if (gpiopin >= 0 && (gpiopin < nitems(InfinitekGlobal.gpio_pin)) && (InfinitekGlobal.gpio_pin[gpiopin] > 0)) {
    return true;
  }
  return false;
}
...
void ScripterEvery100ms(void) {
  ...
  if (bitRead(Settings->rule_enabled, 0) && (InfinitekGlobal.uptime > 4)) {
    if (GetNextSensor()) {
    ...
    if (*lp!=';') {
      ...
      *cp = 0;
      ...
      if (response) {
        ...
        *response += String(EncodeLightId(hue_devs + InfinitekGlobal.devices_present + 1))+"\":";
...
void Script_Handle_Hue(String path) {
  ...
  uint8_t index = device - InfinitekGlobal.devices_present - 1;
...
bool ScriptCommand(void) {
  ...
  if (-1 == command_code) {
    ...
    Response_P(PSTR("{\"%s\":\"%s\",\"StopOnError\":\"%s\",\"Free\":%d}"), command, GetStateText(bitRead(Settings->rule_enabled, 0)), GetStateText(bitRead(Settings->rule_stop, 0)),glob_script_mem.script_size - strlen(glob_script_mem.script_ram));
      ...
      XdrvMailbox.topic = (char*)"stat/Infinitek/SENSOR";
      ScriptMqttData();
      serviced = true;
#endif
#endif //SUPPORT_MQTT_EVENT
...
#ifdef USE_WEBSEND_RESPONSE
  InfinitekGlobal.mqtt_data = http.getString();

//#ifdef HTTP_DEBUG
  if (debug) {
    AddLog(LOG_LEVEL_INFO, PSTR("SCR: HTTP MQTT BUFFER %s"), InfinitekGlobal.mqtt_data.c_str());
  }
//#endif

//  AddLog(LOG_LEVEL_INFO, PSTR("JSON %s"), wd_jstr);
//  InfinitekGlobal.mqtt_data = wd_jstr;
...
int32_t call2pwl(const char *url) {
  ...
  InfinitekGlobal.mqtt_data = result;
...
uint32_t call2https(const char *host, const char *path) {
  //if (InfinitekGlobal.global_state.wifi_down) return 1;
  uint32_t status = 0;
