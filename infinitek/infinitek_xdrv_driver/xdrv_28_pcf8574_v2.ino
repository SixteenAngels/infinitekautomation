/*
  xdrv_28_pcf8574.ino - PCF8574 I2C support for Infinitek
 ...
 *
 * Mode 2:
 * Allows easy configuration by using a sequential list of pins configured as Infinitek
...
void Pcf8574Power(void) {
  ...
  uint32_t relay_max = InfinitekGlobal.devices_present;
  if (!Pcf8574.base) {
...
void Pcf8574SwitchRelay(void) {
  uint32_t devices_present = InfinitekGlobal.devices_present - Pcf8574.relay_offset;
  for (uint32_t i = 0; i < devices_present; i++) {
    ...
    if (Pcf8574.max_devices > 0 && Pcf8574_pin[i] < 99) {
      ...
      uint8_t _val = bitRead(InfinitekGlobal.rel_inverted, Pcf8574.relay_offset + i) ? !relay_state : relay_state;
...
void Pcf8574ModuleInitMode1(void) {
  ...
  for (uint32_t idx = 0; idx < Pcf8574.max_devices; idx++) { // suport up to 8 boards PCF8574
    ...
    for (uint32_t i = 0; i < 8; i++, gpio>>=1) {
      ...
      if (_result > 0) {
        ...
        bitWrite(InfinitekGlobal.rel_inverted, InfinitekGlobal.devices_present, Settings->flag3.pcf8574_ports_inverted);  // SetOption81 - Invert all ports on PCF8574 devices
        if (!Settings->flag.save_state && !Settings->flag3.no_power_feedback) {  // SetOption63 - Don't scan relay power state at restart - #5594 and #5663
          ...
          bitWrite(InfinitekGlobal.power, InfinitekGlobal.devices_present, power_state);
          bitWrite(Settings->power, InfinitekGlobal.devices_present, power_state);
        }
  ...
  //AddLog(LOG_LEVEL_DEBUG, PSTR("PCF: Settings->power=0x%08X, InfinitekGlobal.power=0x%08X"), Settings->power, InfinitekGlobal.power);
...
void Pcf8574ModuleInit(void) {
  ...
  if (Pcf8574.mode) {
    ...
    if (Pcf8574_pin) {
      Pcf8574.relay_offset = InfinitekGlobal.devices_present;
#ifdef USE_PCF8574_MODE2
