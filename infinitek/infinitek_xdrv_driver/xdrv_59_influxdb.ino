/*
  xdrv_59_influxdb.ino - Influxdb support for Infinitek
          ...
          if (value2.isObject()) {
            ...
            for (auto key3 : Object3) {
              ...
              if ((value != nullptr) && key2.isValid() && key3.isValid()) {
                ...
                // temperature,device=Infinitek1,sensor=DS18B20 value=24.44
                snprintf_P(linebuf, sizeof(linebuf), PSTR("%s,device=%s,sensor=%s value=%s\n"),
                  type, InfinitekGlobal.mqtt_topic, sensor, value);
                data += linebuf;
              }
            }
          } else {
...
//              AddLog(LOG_LEVEL_DEBUG, PSTR("IFX2: sensor %s (%s), type %s (%s)"), key1.getStr(), sensor, key2.getStr(), type);
              ...
              if (is_array) {
                ...
                for (auto val : arr) {
                  ...
                  snprintf_P(linebuf, sizeof(linebuf), PSTR("%s%d,device=%s,sensor=%s%s value=%s\n"),
                    type, i, InfinitekGlobal.mqtt_topic, sensor, sensor_id, val.getStr());
                  data += linebuf;
                }
              } else {
                ...
                snprintf_P(linebuf, sizeof(linebuf), PSTR("%s,device=%s,sensor=%s%s value=%s\n"),
                  type, InfinitekGlobal.mqtt_topic, sensor, sensor_id, value);
                data += linebuf;
              }
              sensor_id[0] = '\0';
            }
        ...
        if ((value != nullptr) && key1.isValid()) {
          ...
          snprintf_P(linebuf, sizeof(linebuf), PSTR("%s,device=%s,sensor=device value=%s\n"),
            type, InfinitekGlobal.mqtt_topic, value);
          data += linebuf;
        }
      }
    }
...
void InfluxDbPublishPowerState(uint32_t device) {
  Response_P(PSTR("{\"power%d\":\"%d\"}"), device, bitRead(InfinitekGlobal.power, device -1));
  InfluxDbProcessJson();
}
