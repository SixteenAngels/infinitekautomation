/*
  xdrv_23_zigbee.ino - zigbee support for Infinitek
...
void ZbSendSend(class JsonParserToken val_cmd, ZCLFrame & zcl) {
  ...
  if (val_cmd.isObject()) {
    ...
    if (cmd_size > 1) {
      ...
      const char * Infinitek_cmd = zigbeeFindCommand(key.getStr(), &local_cluster_id, &cmd_var);
      if (Infinitek_cmd == nullptr) {    // did we find the command?
      ...
      zigbeeCmdAddParams(zcl.payload, Infinitek_cmd, x, y, z);   // fill in parameters
    } else {
...
// ZbSend {"Device":"0x0000","Endpoint":1,"Write":{"Manufacturer":"Infinitek","ModelId":"Infinitek Z2T Router"}}
...
void CmndZbAttrDump(void) {
  ...
  for (uint32_t i = 0; i < sizeof(Z_Commands) / sizeof(Z_Commands[0]); i++) {
    ...
    const char * alias = Z_strings + pgm_read_word(&conv->Infinitek_cmd_offset);
...
#ifdef ESP32
    if (analogAttach(led_pin, InfinitekGlobal.ledlnk_inverted) >= 0) {
      analogWritePhase(led_pin, led_power, 0);
    }
#else
    AnalogWrite(led_pin, InfinitekGlobal.ledlnk_inverted ? 1023 - led_power : led_power);
#endif
  }
}
...
void CmndZbConfig(void) {
  ...
  if (strlen(XdrvMailbox.data) > 0) {
    ...
    if ( (zb_channel      != Settings->zb_channel) ||
      ...
      InfinitekGlobal.restart_flag = 2;    // save and reboot
    }
  }
...
bool Xdrv23(uint32_t function) {
  if (InfinitekGlobal.gpio_optiona.enable_ccloader) { return false; }
