/*
  xdrv_13_display.ino - Display support for Infinitek
              ...
              if (buttons[num]) {
                if (del) {
                  ...
                  if (!dis) {
                    ...
                    else buttons[num]->xdrawButton(bitRead(InfinitekGlobal.power, num));
                  }
                }
              }
              break;
            ...
            if (renderer) {
              ...
              if (buttons[num]) {
                if (!sbt) {
                  ...
                  if (!bflags) {
                    // power button
                    if (dflg) buttons[num]->xdrawButton(bitRead(InfinitekGlobal.power, num));
...
void DisplayLogBufferInit(void) {
  if (Settings->display_mode) {
    ...
    char buffer[40];
    snprintf_P(buffer, sizeof(buffer), PSTR(D_VERSION " %s%s"), InfinitekGlobal.version, InfinitekGlobal.image_name);
...
void DisplayLocalSensor(void) {
  if (((DM_LOCAL_SENSORS == Settings->display_mode) ||
      ...
      (0 == InfinitekGlobal.tele_period)) {
    char no_topic[1] = { 0 };
//    DisplayAnalyzeJson(InfinitekGlobal.mqtt_topic, ResponseData());  // Add local topic
...
//  ApplyDisplayDimmer();  // Not allowed here. Way too early in init sequence. Global power state has not been set at this point in time
  ...
  disp_device = InfinitekGlobal.devices_present;
...
void CmndDisplayModel(void) {
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < DISPLAY_MAX_DRIVERS)) {
    ...
    if (XdspCall(FUNC_DISPLAY_MODEL)) {
      InfinitekGlobal.restart_flag = 2;  // Restart to re-init interface and add/Remove MQTT subscribe
    } else {
...
void CmndDisplayWidth(void) {
  if (XdrvMailbox.payload > 0) {
    if (XdrvMailbox.payload != Settings->display_width) {
      ...
      InfinitekGlobal.restart_flag = 2;  // Restart to re-init width
    }
  }
...
void CmndDisplayHeight(void) {
  if (XdrvMailbox.payload > 0) {
    if (XdrvMailbox.payload != Settings->display_height) {
      ...
      InfinitekGlobal.restart_flag = 2;  // Restart to re-init height
    }
  }
    ...
    switch (function) {
      ...
      case FUNC_EVERY_SECOND:
        ...
        if (Settings->display_model && Settings->display_mode) {
          ...
          if (InfinitekGlobal.uptime > wait) {         // Allow time to display splash screen
