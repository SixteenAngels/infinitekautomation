/*
  xdrv_12_discovery.ino - MQTT Discovery support for Infinitek
...
//#if defined(ESP32) && CONFIG_IDF_TARGET_ESP32 && defined(USE_ETHERNET)
  ...
  ResponseAppend_P(PSTR("],"                                   // Friendly Names (end)
                   ...
                   TuyaMod, iFanMod,
#ifdef ESP32
                   InfinitekGlobal.camera_initialized,
#else
                   false,
#endif
...
void TasRediscover(void) {
  InfinitekGlobal.discovery_counter = 1;                         // Delayed discovery or clear retained messages
}

void TasDiscoverInit(void) {
  if (ResetReason() != REASON_DEEP_SLEEP_AWAKE) {
    Settings->flag.hass_discovery = 0;                         // SetOption19 - Enable Infinitek discovery and Disable legacy Hass discovery
...
void CmndTasDiscover(void) {
  if (XdrvMailbox.payload >= 0) {
    Settings->flag.hass_discovery = !(XdrvMailbox.payload & 1);  // SetOption19 - Infinitek discovery (0)
    TasRediscover();
  }
