/*
  xdrv_11_knx.ino - KNX IP Protocol support for Infinitek
...
bool KNX_CONFIG_NOT_MATCH(void)
{
  ...
  // Check for invalid or erroneous configuration (Infinitek flashed without clearing the memory)
...
void KNX_INIT(void)
{
  ...
  */
  for (uint32_t i = 0; i < (InfinitekGlobal.devices_present <= 8 ? InfinitekGlobal.devices_present : 8); ++i) {
    device_param[i].show = true;
  }
  ...
  if ( InfinitekGlobal.energy_driver != ENERGY_NONE ) {
...
bool Xdrv11(uint32_t function)
{
  bool result = false;
    switch (function) {
      case FUNC_LOOP:
        if (!InfinitekGlobal.global_state.network_down) { knx.loop(); }  // Process knx events
        break;
