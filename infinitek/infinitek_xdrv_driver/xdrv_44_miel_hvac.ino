/*
  xdrv_44_miel_hvac.ino - Mitsubishi Electric HVAC support for Infinitek
...
{
	InfinitekSerial *serial = sc->sc_serial;
	uint8_t cksum = 0;
	size_t i;
...
miel_hvac_input_settings(struct miel_hvac_softc *sc, const struct miel_hvac_data *d)
{
	...
	if (bitRead(InfinitekGlobal.power, sc->sc_device) != !!state)
...
miel_hvac_pre_init(void)
{
	...
	sc->sc_serial = new InfinitekSerial(Pin(GPIO_MIEL_HVAC_RX), Pin(GPIO_MIEL_HVAC_TX), 2);
