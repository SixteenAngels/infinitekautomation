/*
  xdrv_63_modbus_bridge.ino - modbus bridge support for Infinitek
...
/*********************************************************************************************\
 * Modbus Bridge using Modbus library (InfinitekModbus)
 *
...
InfinitekModbus *modbusBridgeModbus = nullptr;
...
/********************************************************************************************/
//
// Handles data received from Infinitek modbus wrapper and send this to (TCP and/or) MQTT client
//
  ...
  if (data_ready || error)
  {
    ...
    if (errorcode == ModbusBridgeError::noerror)
    {
      if (modbusBridge.type == ModbusBridgeType::mb_raw)
      {
        ...
        ResponseAppend_P(PSTR("]}"));
        ResponseJsonEnd();
        if (Settings->flag6.mqtt_disable_modbus ) {  // SetOption158  If it is activated, Infinitek will not publish ModbusReceived MQTT messages, but it will proccess event trigger rules
          XdrvRulesProcess(0);
        } else {
...
/********************************************************************************************/
//
// Inits the Infinitek modbus driver, sets serialport and if TCP enabled allocates a TCP buffer
//
void ModbusBridgeInit(void) {
  if (PinUsed(GPIO_MBR_RX) && PinUsed(GPIO_MBR_TX)) {
    modbusBridgeModbus = new InfinitekModbus(Pin(GPIO_MBR_RX), Pin(GPIO_MBR_TX), Pin(GPIO_MBR_TX_ENA));
...
{
  // See InfinitekModusConfig for possible options
