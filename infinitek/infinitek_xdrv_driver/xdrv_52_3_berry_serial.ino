...
\*********************************************************************************************/
extern "C" {
  InfinitekSerial * b_serial_get(struct bvm *vm) {
    be_getmember(vm, 1, ".p");
    InfinitekSerial * ow = (InfinitekSerial *) be_tocomptr(vm, -1);
    be_pop(vm, 1);
    return ow;
  }
  ...
  int32_t b_serial_init(struct bvm *vm) {
    ...
    if (argc >= 4 && be_isint(vm, 2) && be_isint(vm, 3) && be_isint(vm, 4)) {
      ...
      InfinitekSerial * ser = new InfinitekSerial(rx, tx, 0, 0, TM_SERIAL_BUFFER_SIZE, inverted);
  ...
  int32_t b_config_tx_en(struct bvm *vm) {
    be_getmember(vm, 1, ".p");
    InfinitekSerial * ser = (InfinitekSerial *) be_tocomptr(vm, -1);
    if (ser) {
