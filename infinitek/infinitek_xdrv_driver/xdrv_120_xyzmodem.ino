/*
  xdrv_120_xyzmodem.ino - XModem support for Infinitek
 ...
 *
 * Usage:
 *  Use a tool able supporting XModem file transfer and to connect either using serial or telnet with Infinitek.
 *   TeraTerm (When using telnet enable non-bare CR support with Infinitek command `Teraterm 1`)
 *   SyncTerm
 *   SecureCRT (When using Telnet set all Telnet Advanced Options OFF EXCEPT 'Server requires bare CR`)
 *
 *  To start XModem file transfer first execute Infinitek command XSend or XReceive,
...
int XYZModemAvailable(void) {
  switch (XYZModem.protocol) {
    case TXMP_SERIAL:
      return XYZModem.serial->available();   // InfinitekSerial
  ...
  if (XYZFile.file) {
    ...
    if (1 == state) {
      if (SettingsConfigRestore()) {                 // Process new settings and release buffer
        InfinitekGlobal.restart_flag = 2;              // Restart on valid settings
      }
    } else {
