/*
  support_network.ino - Network support for Infinitek
...
#endif // ESP8266
      Mdns.begun = (uint8_t)MDNS.begin(InfinitekGlobal.hostname);
      AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_MDNS "%s '%s.local'"), (Mdns.begun) ? PSTR(D_INITIALIZED) : PSTR(D_FAILED), InfinitekGlobal.hostname);
    }
  }
}
...
void MdnsAddServiceHttp(void) {
  if (1 == Mdns.begun) {
    ...
    MDNS.addServiceTxt("http", "tcp", "devicetype", "Infinitek");
  }
}
...
#endif
#endif
  return InfinitekGlobal.hostname;
}
