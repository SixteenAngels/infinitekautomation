/*
  support_Infinitek.ino - Core support for Infinitek
...
String ResolveToken(const char* input) {
  ...
  resolved.replace(F("%hostname%"), InfinitekGlobal.hostname);
...
char* GetTopic_P(char *stopic, uint32_t prefix, const char *topic, const char* subtopic)
{
  ...
  */
  ...
  if (InfinitekGlobal.fallback_topic_flag || (prefix > 3)) {
    ...
    fulltopic += F("/");
    if (fallback) {
      fulltopic += InfinitekGlobal.mqtt_client;
...
void ZeroCrossMomentStart(void) {
  if (!InfinitekGlobal.zc_interval) { return; }

#ifdef DEBUG_ZEROCROSS
  uint32_t dbg_interval = InfinitekGlobal.zc_interval;
  uint32_t dbg_zctime = InfinitekGlobal.zc_time;
  uint32_t dbg_starttime = micros();
#endif

  uint32_t trigger_moment = InfinitekGlobal.zc_time + InfinitekGlobal.zc_interval - InfinitekGlobal.zc_offset - InfinitekGlobal.zc_code_offset;
  while (TimeReachedUsec(trigger_moment)) {    // Trigger moment already passed so try next
    trigger_moment += InfinitekGlobal.zc_interval;
  }
  ...
  InfinitekGlobal.zc_code_offset = micros();
}

void ZeroCrossMomentEnd(void) {
  if (!InfinitekGlobal.zc_interval) { return; }
  InfinitekGlobal.zc_code_offset = (micros() - InfinitekGlobal.zc_code_offset) / 2;

#ifdef DEBUG_ZEROCROSS
  AddLog(LOG_LEVEL_DEBUG, PSTR("ZCD: CodeExecTime %d"), InfinitekGlobal.zc_code_offset * 2);
#endif
}
...
void ZeroCrossIsr(void) {
  ...
  InfinitekGlobal.zc_interval = ((int32_t) (time - InfinitekGlobal.zc_time));
  InfinitekGlobal.zc_time = time;
}

void ZeroCrossInit(uint32_t offset) {
  if (PinUsed(GPIO_ZEROCROSS)) {
    InfinitekGlobal.zc_offset = offset;
...
void SetLatchingRelay(power_t lpower, uint32_t state) {
  // InfinitekGlobal.power xx00 - toggle REL1 (Off) and REL3 (Off) - device 1 Off, device 2 Off
  // InfinitekGlobal.power xx01 - toggle REL2 (On)  and REL3 (Off) - device 1 On,  device 2 Off
  // InfinitekGlobal.power xx10 - toggle REL1 (Off) and REL4 (On)  - device 1 Off, device 2 On
  // InfinitekGlobal.power xx11 - toggle REL2 (On)  and REL4 (On)  - device 1 On,  device 2 On
  static power_t latching_power = 0;     // Power state at latching start

  if (state && !InfinitekGlobal.latching_relay_pulse) {  // Set latching relay to power if previous pulse has finished
    latching_power = lpower;
    InfinitekGlobal.latching_relay_pulse = 2;            // max 200mS (initiated by stateloop())
  }

  for (uint32_t i = 0; i < InfinitekGlobal.devices_present; i++) {
    uint32_t port = (i << 1) + ((latching_power >> i) &1);
    DigitalWrite(GPIO_REL1, port, bitRead(InfinitekGlobal.rel_inverted, port) ? !state : state);
  }
}

void SetDevicePower(power_t rpower, uint32_t source) {
  if (InfinitekGlobal.power_on_delay) {
    InfinitekGlobal.power_on_delay_state = rpower;
    return;
  }

  ShowSource(source);
  InfinitekGlobal.last_source = source;

  if (POWER_ALL_ALWAYS_ON == Settings->poweronstate) {  // All on and stay on
    InfinitekGlobal.power = POWER_MASK >> (POWER_SIZE - InfinitekGlobal.devices_present);
    rpower = InfinitekGlobal.power;
  }
  ...
  if (Settings->flag.interlock) {          // Allow only one or no relay set - CMND_INTERLOCK - Enable/disable interlock
    for (uint32_t i = 0; i < MAX_INTERLOCKS; i++) {
      power_t mask = 1;
      uint32_t count = 0;
      for (uint32_t j = 0; j < InfinitekGlobal.devices_present; j++) {
  ...
  if (rpower) {                           // Any power set
    InfinitekGlobal.last_power = rpower;
  }
  ...
  if (XdrvCall(FUNC_SET_DEVICE_POWER)) {  // Set power state and stop if serviced
...
#ifdef ESP8266
  else if ((SONOFF_DUAL == InfinitekGlobal.module_type) || (CH4 == InfinitekGlobal.module_type)) {
    Serial.write(0xA0);
    Serial.write(0x04);
  ...
  else if (EXS_RELAY == InfinitekGlobal.module_type) {
    ...
    for (uint32_t i = 0; i < InfinitekGlobal.devices_present; i++) {
      ...
      bool update = true;
      if (bitRead(InfinitekGlobal.rel_bistable, port)) {
        if (Settings->flag6.bistable_single_pin) {  // SetOption152 - (Power) Use single pin bistable
          if (0x80000000 == InfinitekGlobal.power_latching) {
            InfinitekGlobal.power_latching = InfinitekGlobal.power;  // Init last known state
          }
          update = (bitRead(InfinitekGlobal.power_latching, port) != state);
          if (update) {
            bitWrite(InfinitekGlobal.power_latching, port, state);
    ...
    if (InfinitekGlobal.rel_bistable) {             // If bistable relays in the mix reset them after 40ms
      ...
      for (uint32_t i = 0; i < port; i++) {       // Reset up to detected amount of ports
        if (bitRead(InfinitekGlobal.rel_bistable, i)) {
          if (Settings->flag6.bistable_single_pin) {  // SetOption152 - (Power) Use single pin bistable
            if (!bitRead(bistable, i)) {
              continue;
            }
          }
          DigitalWrite(GPIO_REL1, i, bitRead(InfinitekGlobal.rel_inverted, i) ? 1 : 0);
        }
      }
    }
  }
...
{
  if (InfinitekGlobal.power != InfinitekGlobal.last_power) {
    InfinitekGlobal.power = InfinitekGlobal.last_power;
    SetDevicePower(InfinitekGlobal.power, source);
    if (publish_power) {
...
// state 16 = POWER_SHOW_STATE = Show power state
  ...
  if (((state >= POWER_OFF) && (state <= POWER_TOGGLE)) || (POWER_OFF_FORCE == state))  {
    power_t current_power = InfinitekGlobal.power;
    power_t all_on = POWER_MASK >> (POWER_SIZE - InfinitekGlobal.devices_present);
    switch (state) {
    case POWER_OFF:
      // Keep locked bits and set all other to 0
      InfinitekGlobal.power &= Settings->power_lock; 
      break;
    case POWER_ON:
      // Keep locked bits and set all other to 1
      InfinitekGlobal.power = (InfinitekGlobal.power & Settings->power_lock) | (all_on & ~Settings->power_lock);
      break;
    case POWER_TOGGLE:
      // Keep locked bits and toggle all other
      InfinitekGlobal.power ^= ~Settings->power_lock & all_on;
      break;
    case POWER_OFF_FORCE:
      // Set all off even if locked on (Used by overtemp and overcurrent)
      InfinitekGlobal.power = 0; 
      break;
    }
#ifdef USE_SONOFF_IFAN
    if (IsModuleIfan()) {
      // Do not touch Fan relays
      InfinitekGlobal.power &= 0x0001;
      InfinitekGlobal.power |= (current_power & 0xFFFE);
    }
...
{
#ifdef ESP8266
  if (MOTOR == InfinitekGlobal.module_type) {
  ...
  if (POWER_ALL_ALWAYS_ON == Settings->poweronstate) {
    ...
    power_t devices_mask = POWER_MASK >> (POWER_SIZE - InfinitekGlobal.devices_present);
    if (ResetReasonPowerOn()) {
      InfinitekGlobal.power_latching = 0;   // Single pin latching relay is powered off after re-applying power
      switch (Settings->poweronstate) {
      case POWER_ALL_OFF:
      case POWER_ALL_OFF_PULSETIME_ON:
        InfinitekGlobal.power = 0;
        SetDevicePower(InfinitekGlobal.power, SRC_RESTART);
        break;
      case POWER_ALL_ON:  // All on
        InfinitekGlobal.power = devices_mask;
        SetDevicePower(InfinitekGlobal.power, SRC_RESTART);
        break;
      case POWER_ALL_SAVED_TOGGLE:
        InfinitekGlobal.power = (Settings->power & devices_mask) ^ POWER_MASK;
        if (Settings->flag.save_state) {  // SetOption0 - Save power state and use after restart
          SetDevicePower(InfinitekGlobal.power, SRC_RESTART);
        }
        break;
      case POWER_ALL_SAVED:
        InfinitekGlobal.power = Settings->power & devices_mask;
        if (Settings->flag.save_state) {  // SetOption0 - Save power state and use after restart
          SetDevicePower(InfinitekGlobal.power, SRC_RESTART);
        }
        break;
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("PWR: PowerOnState %d restored"), Settings->poweronstate);
  ...
  for (uint32_t i = 0; i < InfinitekGlobal.devices_present; i++) {
    ...
    if (!Settings->flag3.no_power_feedback &&  // SetOption63 - Don't scan relay power state at restart - #5594 and #5663
        !InfinitekGlobal.power_on_delay          // SetOption47 - Delay switching relays to reduce power surge at power on
#ifdef USE_SHUTTER
      ...
      if ((port < MAX_RELAYS) && PinUsed(GPIO_REL1, port)) {
        if (bitRead(InfinitekGlobal.rel_bistable, port)) {
          ...
          bitWrite(InfinitekGlobal.power, i, digitalRead(Pin(GPIO_REL1, port)) ^ bitRead(InfinitekGlobal.rel_inverted, port));
        }
      }
      port++;
    }
#endif  // ESP8266
    if (bitRead(InfinitekGlobal.power, i) || (POWER_ALL_OFF_PULSETIME_ON == Settings->poweronstate)) {
      SetPulseTimer(i % MAX_PULSETIMERS, Settings->pulse_timer[i % MAX_PULSETIMERS]);
    }
  }

  InfinitekGlobal.blink_powersave = InfinitekGlobal.power;
#ifdef USE_RULES
  RulesEvery50ms();
#endif
}
...
{
	for (uint32_t i = 0; i < InfinitekGlobal.leds_present; i++) {
		SetLedPowerIdx(i, bitRead(InfinitekGlobal.led_power, i));
	}
}
...
*/
  if (PinUsed(GPIO_LED1, led)) {
    ...
    if (state) {
      state = 1;
      InfinitekGlobal.led_power |= mask;
    } else {
      InfinitekGlobal.led_power &= (0xFF ^ mask);
    }
    uint16_t pwm = 0;
...
#endif
        analogWrite(Pin(GPIO_LED1, led), bitRead(InfinitekGlobal.led_inverted, led) ? Settings->pwm_range - pwm : pwm);
    } else {
      DigitalWrite(GPIO_LED1, led, bitRead(InfinitekGlobal.led_inverted, led) ? !state : state);
    }
  }
#ifdef USE_BUZZER
  if (led == 0) {
...
*/
    ...
    power_t mask = 1;
    for (uint32_t i = 0; i < InfinitekGlobal.leds_present; i++) {  // Map leds to power
      bool tstate = (InfinitekGlobal.power & mask);
...
void SetLedLink(uint32_t state) {
  ...
  uint32_t led_inv = InfinitekGlobal.ledlnk_inverted;
  ...
  if (Settings->flag4.powered_off_led) InfinitekGlobal.restore_powered_off_led_counter = 3;
...
void DebugLed(uint32_t mode) {
  ...
  if (PinUsed(GPIO_LEDLNK)) {
    ...
    digitalWrite(Pin(GPIO_LEDLNK), (InfinitekGlobal.ledlnk_inverted) ? !mode : mode);
  }
}

void SetPulseTimer(uint32_t index, uint32_t time)
{
  InfinitekGlobal.pulse_timer[index] = (time > 111) ? millis() + (1000 * (time - 100)) : (time > 0) ? millis() + (100 * time) : 0L;
}

uint32_t GetPulseTimer(uint32_t index)
{
  long time = TimePassedSince(InfinitekGlobal.pulse_timer[index]);
  if (time < 0) {
    time *= -1;
...
// state 10 = POWER_DELAYED = button released delayed
  ...
  if (Settings->flag.mqtt_enabled && MqttIsConnected() && (strlen(key_topic) != 0) && strcmp(key_topic, "0")) {  // SetOption3 - Enable MQTT
    if (!key && (device > InfinitekGlobal.devices_present)) {
    ...
    if (CLEAR_RETAIN == state) {
      ...
      if ((Settings->flag3.button_switch_force_local ||      // SetOption61 - Force local operation when button/switch topic is set
           !strcmp(InfinitekGlobal.mqtt_topic, key_topic) ||
           !strcmp(SettingsText(SET_MQTT_GRP_TOPIC), key_topic)) &&
          (POWER_TOGGLE == state)) {
        state = ~(InfinitekGlobal.power >> (device -1)) &1;   // POWER_OFF or POWER_ON
      }
...
#ifdef USE_PWM_DIMMER
  if (PWM_DIMMER != InfinitekGlobal.module_type || (!result && !Settings->flag3.mqtt_buttons)) {
#endif  // USE_PWM_DIMMER
...
#ifdef USE_PWM_DIMMER
    if (PWM_DIMMER == InfinitekGlobal.module_type) result = true;
  }
#endif  // USE_PWM_DIMMER
  return result;
}
...
//  if (1049 == LANGUAGE_LCID) { return; }
...
#ifdef USE_SONOFF_IFAN
  if (IsModuleIfan()) {
    InfinitekGlobal.blink_mask &= 1;       // No blinking on the fan relays
  ...
  if ((device < 1) || (device > InfinitekGlobal.devices_present)) {
    device = 1;
  }
  InfinitekGlobal.active_device = device;
  ...
  if (state <= POWER_TOGGLE) {
    if ((InfinitekGlobal.blink_mask & mask)) {
      InfinitekGlobal.blink_mask &= (POWER_MASK ^ mask);  // Clear device mask
      MqttPublishPowerBlinkState(device);
    }

    if (Settings->flag.interlock &&      // CMND_INTERLOCK - Enable/disable interlock
        !interlock_mutex &&
        ((POWER_ON == state) || ((POWER_TOGGLE == state) && !(InfinitekGlobal.power & mask)))
       ) {
      ...
      for (uint32_t i = 0; i < MAX_INTERLOCKS; i++) {
        if (Settings->interlock[i] & mask) {  // Find interlock group
          for (uint32_t j = 0; j < InfinitekGlobal.devices_present; j++) {
            power_t imask = 1 << j;
            if ((Settings->interlock[i] & imask) && (InfinitekGlobal.power & imask) && (mask != imask)) {
    ...
    power_t old_power = InfinitekGlobal.power;
#endif  // USE_DEVICE_GROUPS
    switch (state) {
    case POWER_OFF: {
      InfinitekGlobal.power &= (POWER_MASK ^ mask);
      break; }
    case POWER_ON:
      InfinitekGlobal.power |= mask;
      break;
...
#ifdef USE_DEVICE_GROUPS
    if (InfinitekGlobal.power != old_power && SRC_REMOTE != source && SRC_RETRY != source) {
      power_t dgr_power = InfinitekGlobal.power;
...
#endif  // USE_DOMOTICZ
#ifdef USE_KNX
    KnxUpdatePowerState(device, InfinitekGlobal.power);
#endif  // USE_KNX
    ...
    SetPulseTimer((device -1) % MAX_PULSETIMERS, (((POWER_ALL_OFF_PULSETIME_ON == Settings->poweronstate) ? ~InfinitekGlobal.power : InfinitekGlobal.power) & mask) ? Settings->pulse_timer[(device -1) % MAX_PULSETIMERS] : 0);
  }
  else if (POWER_BLINK == state) {
    if (!(InfinitekGlobal.blink_mask & mask)) {
      InfinitekGlobal.blink_powersave = (InfinitekGlobal.blink_powersave & (POWER_MASK ^ mask)) | (InfinitekGlobal.power & mask);  // Save state
      InfinitekGlobal.blink_power = (InfinitekGlobal.power >> (device -1))&1;  // Prep to Toggle
    }
    InfinitekGlobal.blink_timer = millis() + 100;
    InfinitekGlobal.blink_counter = ((!Settings->blinkcount) ? 64000 : (Settings->blinkcount *2)) +1;
    InfinitekGlobal.blink_mask |= mask;    // Set device mask
    MqttPublishPowerBlinkState(device);
    return;
  }
  else if (POWER_BLINK_STOP == state) {
    bool flag = (InfinitekGlobal.blink_mask & mask);
    InfinitekGlobal.blink_mask &= (POWER_MASK ^ mask);  // Clear device mask
    MqttPublishPowerBlinkState(device);
    if (flag) {
      ExecuteCommandPower(device, (InfinitekGlobal.blink_powersave >> (device -1))&1, SRC_IGNORE);  // Restore state
    }
    return;
  }
  if (publish_power) {
...
void StopAllPowerBlink(void)
{
  ...
  for (uint32_t i = 1; i <= InfinitekGlobal.devices_present; i++) {
    mask = 1 << (i -1);
    if (InfinitekGlobal.blink_mask & mask) {
      InfinitekGlobal.blink_mask &= (POWER_MASK ^ mask);  // Clear device mask
      MqttPublishPowerBlinkState(i);
      ExecuteCommandPower(i, (InfinitekGlobal.blink_powersave >> (i -1))&1, SRC_IGNORE);  // Restore state
    }
  }
}
...
void MqttShowState(void)
{
  ...
  ResponseAppend_P(PSTR(",\"" D_JSON_HEAPSIZE "\":%d,\"SleepMode\":\"%s\",\"Sleep\":%u,\"LoadAvg\":%u,\"MqttCount\":%u"),
    ...
    InfinitekGlobal.sleep, InfinitekGlobal.loop_load_avg, MqttConnectCount());
  ...
  for (uint32_t i = 1; i <= InfinitekGlobal.devices_present; i++) {
    ...
    if ((LightDevice()) && (i >= LightDevice())) {
      ...
      ResponseAppend_P(PSTR(",\"%s\":\"%s\""), GetPowerDevice(stemp1, i, sizeof(stemp1), Settings->flag.device_index_enable),  // SetOption26 - Switch between POWER or POWER1
                                               GetStateText(bitRead(InfinitekGlobal.power, i-1)));
#ifdef USE_SONOFF_IFAN
      if (IsModuleIfan()) {
  ...
  if (InfinitekGlobal.pwm_present) {
    ResponseAppend_P(PSTR(","));
    MqttShowPWMState();
  }
  
  char *hostname = InfinitekGlobal.hostname;
  ...
  if (!InfinitekGlobal.global_state.wifi_down) {
    ...
    if (static_cast<uint32_t>(WiFi.localIP()) != 0) {
      hostname = InfinitekGlobal.hostname;     // Overrule ethernet as primary IP connection
...
void GetSensorValues(void) {
  ...
  for (uint32_t type = 0; type < 3; type++) {
    if (!Settings->global_sensor_index[type] || InfinitekGlobal.user_globals[type]) { continue; }
...
//        AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: %s value%d = %2_f"), key, idx, &new_value);
    ...
    if (value != -9999) {
      switch (type) {
        case 0:  // Temperature
          InfinitekGlobal.temperature_celsius = ConvertTempToCelsius(value);
          break;
        case 1:  // Humidity
          InfinitekGlobal.humidity = value;
          break;
        case 2:  // Pressure
          InfinitekGlobal.pressure_hpa = ConvertHgToHpa(value);
          break;
      }
      InfinitekGlobal.global_update = InfinitekGlobal.uptime;
    }
  }
}
...
bool MqttShowSensor(bool call_show_sensor) {
  ...
  if (InfinitekGlobal.global_update && Settings->flag.mqtt_add_global_info) {  // SetOption2 (MQTT) Add global temperature/humidity/pressure info to JSON sensor message
    if ((InfinitekGlobal.humidity > 0) || !isnan(InfinitekGlobal.temperature_celsius) || (InfinitekGlobal.pressure_hpa != 0)) {
      uint32_t add_comma = 0;
      ResponseAppend_P(PSTR(",\"Global\":{"));
      if (!isnan(InfinitekGlobal.temperature_celsius)) {
        float t = ConvertTempToFahrenheit(InfinitekGlobal.temperature_celsius);
        ResponseAppend_P(PSTR("\"" D_JSON_TEMPERATURE "\":%*_f"),
          Settings->flag2.temperature_resolution, &t);
        add_comma++;
      }
      if (InfinitekGlobal.humidity > 0) {
        ResponseAppend_P(PSTR("%s\"" D_JSON_HUMIDITY "\":%*_f"),
          (add_comma)?",":"", Settings->flag2.humidity_resolution, &InfinitekGlobal.humidity);
        add_comma++;
      }
      if (2 == add_comma) {
        float dewpoint = CalcTempHumToDew(InfinitekGlobal.temperature_celsius, InfinitekGlobal.humidity);
        ResponseAppend_P(PSTR(",\"" D_JSON_DEWPOINT "\":%*_f"),
          Settings->flag2.temperature_resolution, &dewpoint);
#ifdef USE_HEAT_INDEX
        float heatindex = CalcTemHumToHeatIndex(InfinitekGlobal.temperature_celsius, InfinitekGlobal.humidity);
        ResponseAppend_P(PSTR(",\"" D_JSON_HEATINDEX "\":%*_f"),
          Settings->flag2.temperature_resolution, &heatindex);
#endif  // USE_HEAT_INDEX
      }
      if (InfinitekGlobal.pressure_hpa != 0) {
        float p = ConvertPressure(InfinitekGlobal.pressure_hpa);
        float s = ConvertPressureForSeaLevel(InfinitekGlobal.pressure_hpa);
...
{
  InfinitekGlobal.uptime++;

  if (POWER_CYCLE_TIME == InfinitekGlobal.uptime) {
    UpdateQuickPowerCycle(false);
  }

  if (BOOT_LOOP_TIME == InfinitekGlobal.uptime) {
    RtcRebootReset();
  ...
  if (InfinitekGlobal.power_on_delay) {
    if (1 == Settings->param[P_POWER_ON_DELAY2]) {       // SetOption47 1
      ...
      if (!InfinitekGlobal.global_state.network_down) {
        InfinitekGlobal.power_on_delay = 0;
      }
    }
    else if (2 == Settings->param[P_POWER_ON_DELAY2]) {  // SetOption47 2
      // Allow relay power on once mqtt is available
      if (!InfinitekGlobal.global_state.mqtt_down) {
        InfinitekGlobal.power_on_delay = 0;
      }
    }
    else {                                               // SetOption47 3..255
      // Allow relay power on after x seconds
      InfinitekGlobal.power_on_delay--;
    }
    if (!InfinitekGlobal.power_on_delay && InfinitekGlobal.power_on_delay_state) {
      // Set relays according to last SetDevicePower() request
      SetDevicePower(InfinitekGlobal.power_on_delay_state, SRC_SO47);
    }
  }

  if (InfinitekGlobal.mqtt_cmnd_blocked_reset) {
    InfinitekGlobal.mqtt_cmnd_blocked_reset--;
    if (!InfinitekGlobal.mqtt_cmnd_blocked_reset) {
      InfinitekGlobal.mqtt_cmnd_blocked = 0;             // Clean up MQTT cmnd loop block
    }
  }

  if (InfinitekGlobal.seriallog_timer) {
    InfinitekGlobal.seriallog_timer--;
    if (!InfinitekGlobal.seriallog_timer) {
      if (InfinitekGlobal.seriallog_level) {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_APPLICATION D_SERIAL_LOGGING_DISABLED));
      }
      InfinitekGlobal.seriallog_level = 0;
    }
  }

  if (InfinitekGlobal.syslog_timer) {  // Restore syslog level
    InfinitekGlobal.syslog_timer--;
    if (!InfinitekGlobal.syslog_timer) {
  ...
  if ((InfinitekGlobal.init_state >= INIT_GPIOS) && PinUsed(GPIO_HEARTBEAT)) {
    digitalWrite(Pin(GPIO_HEARTBEAT), ~InfinitekGlobal.heartbeat_inverted &1);
    delayMicroseconds(50);
    digitalWrite(Pin(GPIO_HEARTBEAT), InfinitekGlobal.heartbeat_inverted);
  }

  // Teleperiod
  if (Settings->tele_period || (3601 == InfinitekGlobal.tele_period)) {
    if (InfinitekGlobal.tele_period >= 9999) {
      if (!InfinitekGlobal.global_state.network_down) {
        InfinitekGlobal.tele_period = 0;  // Allow teleperiod once wifi is connected
      }
    } else {
      InfinitekGlobal.tele_period++;
      if (InfinitekGlobal.tele_period >= Settings->tele_period) {
        InfinitekGlobal.tele_period = 0;
        ...
        if (!(InfinitekGlobal.tele_period % 10)) {
          for (uint32_t type = 0; type < 3; type++) {
            if (!Settings->global_sensor_index[type] || InfinitekGlobal.user_globals[type]) { continue; }
            GetSensorValues();
            break;
...
#ifdef ESP32
  if (11 == InfinitekGlobal.uptime) {  // Perform one-time ESP32 houskeeping
...
void Every100mSeconds(void)
{
  ...
  if (InfinitekGlobal.latching_relay_pulse) {
    InfinitekGlobal.latching_relay_pulse--;
    if (!InfinitekGlobal.latching_relay_pulse) {
#ifdef ESP8266
      if (EXS_RELAY == InfinitekGlobal.module_type) {
  ...
  for (uint32_t i = 0; i < MAX_PULSETIMERS; i++) {
    if (InfinitekGlobal.pulse_timer[i] != 0L) {           // Timer active?
      if (TimeReached(InfinitekGlobal.pulse_timer[i])) {  // Timer finished?
        InfinitekGlobal.pulse_timer[i] = 0L;              // Turn off this timer
        for (uint32_t j = 0; (i + j) < InfinitekGlobal.devices_present; j = j +MAX_PULSETIMERS) {
  ...
  if (InfinitekGlobal.blink_mask) {
    if (TimeReached(InfinitekGlobal.blink_timer)) {
      SetNextTimeInterval(InfinitekGlobal.blink_timer, 100 * Settings->blinktime);
      InfinitekGlobal.blink_counter--;
      if (!InfinitekGlobal.blink_counter) {
        StopAllPowerBlink();
      } else {
        InfinitekGlobal.blink_power ^= 1;
        power_now = (InfinitekGlobal.power & (POWER_MASK ^ InfinitekGlobal.blink_mask)) | ((InfinitekGlobal.blink_power) ? InfinitekGlobal.blink_mask : 0);
...
// As the max amount of sleep = 250 mSec this loop should always be taken...
  ...
  InfinitekGlobal.state_250mS++;
  InfinitekGlobal.state_250mS &= 0x3;

  InfinitekGlobal.global_state.network_down = (InfinitekGlobal.global_state.wifi_down && InfinitekGlobal.global_state.eth_down) ? 1 : 0;

  if (!Settings->flag.global_state) {                      // SetOption31 - Control link led blinking
    if (InfinitekGlobal.global_state.data &0x03) {                        // Network or MQTT problem
      if (InfinitekGlobal.global_state.mqtt_down) { blinkinterval = 7; }  // MQTT problem so blink every 2 seconds (slowest)
      if (InfinitekGlobal.global_state.network_down) { blinkinterval = 3; }  // Network problem so blink every second (slow)
      InfinitekGlobal.blinks = 201;                         // Allow only a single blink in case the problem is solved
    }
  }
  if (InfinitekGlobal.blinks || InfinitekGlobal.restart_flag || InfinitekGlobal.ota_state_flag) {
    if (InfinitekGlobal.restart_flag || InfinitekGlobal.ota_state_flag) {  // Overrule blinks and keep led lit
      InfinitekGlobal.blinkstate = true;                                  // Stay lit
    } else {
      blinkspeed--;
      if (!blinkspeed) {
        blinkspeed = blinkinterval;                       // Set interval to 0.2 (default), 1 or 2 seconds
        InfinitekGlobal.blinkstate ^= 1;                                  // Blink
      }
    }
    if ((!(Settings->ledstate &0x08)) && ((Settings->ledstate &0x06) || (InfinitekGlobal.blinks > 200) || (InfinitekGlobal.blinkstate))) {
      SetLedLink(InfinitekGlobal.blinkstate);                            // Set led on or off
    }
    if (!InfinitekGlobal.blinkstate) {
      InfinitekGlobal.blinks--;
      if (200 == InfinitekGlobal.blinks) { InfinitekGlobal.blinks = 0; }  // Disable blink
    }
  }
  if (Settings->ledstate &1 && (PinUsed(GPIO_LEDLNK) || !(InfinitekGlobal.blinks || InfinitekGlobal.restart_flag || InfinitekGlobal.ota_state_flag)) ) {
    bool tstate = InfinitekGlobal.power & Settings->ledmask;
#ifdef ESP8266
    if ((SONOFF_TOUCH == InfinitekGlobal.module_type) || (SONOFF_T11 == InfinitekGlobal.module_type) || (SONOFF_T12 == InfinitekGlobal.module_type) || (SONOFF_T13 == InfinitekGlobal.module_type)) {
      tstate = (!InfinitekGlobal.power) ? 1 : 0;           // As requested invert signal for Touch devices to find them in the dark
    }
#endif  // ESP8266
    SetLedPower(tstate);
  }
...
\*-------------------------------------------------------------------------------------------*/
  ...
  switch (InfinitekGlobal.state_250mS) {
  case 0:                                                 // Every x.0 second
    if (InfinitekGlobal.ota_state_flag && CommandsReady()) {
      InfinitekGlobal.ota_state_flag--;
      if (2 == InfinitekGlobal.ota_state_flag) {
...
//        if (Settings->webserver) StopWebserver();       // 20230107 No more need for disabling webserver during OTA
        ...
        if (ota_retry_counter) {
          ...
          if (RtcSettings.ota_loader) {
            ...
            // Replace Infinitek                                         with Infinitek-minimal
            // Replace Infinitek-DE                                      with Infinitek-minimal
            // Replace Infinitek.bin                                     with Infinitek-minimal.bin
            // Replace Infinitek.xyz                                     with Infinitek-minimal.xyz
            // Replace Infinitek.bin.gz                                  with Infinitek-minimal.bin.gz
            // Replace Infinitek.xyz.gz                                  with Infinitek-minimal.xyz.gz
            // Replace Infinitek.ino.bin                                 with Infinitek-minimal.ino.bin
            // Replace Infinitek.ino.bin.gz                              with Infinitek-minimal.ino.bin.gz
            // Replace http://domus1:80/api/arduino/Infinitek.bin        with http://domus1:80/api/arduino/Infinitek-minimal.bin
            // Replace http://domus1:80/api/arduino/Infinitek.bin.gz     with http://domus1:80/api/arduino/Infinitek-minimal.bin.gz
            // Replace http://domus1:80/api/arduino/Infinitek-DE.bin.gz  with http://domus1:80/api/arduino/Infinitek-minimal.bin.gz
            // Replace http://domus1:80/api/ard-uino/Infinitek-DE.bin.gz with http://domus1:80/api/ard-uino/Infinitek-minimal.bin.gz
            // Replace http://192.168.2.17:80/api/arduino/Infinitek.bin  with http://192.168.2.17:80/api/arduino/Infinitek-minimal.bin
            // Replace http://192.168.2.17/api/arduino/Infinitek.bin.gz  with http://192.168.2.17/api/arduino/Infinitek-minimal.bin.gz
...
//AddLog(LOG_LEVEL_DEBUG, PSTR("OTA: File type [%s]"), ech);
            ...
            char *pch = strrchr(bch, '-');                             // Find last dash (-) and ignore remainder - handles Infinitek-DE
            if (pch == nullptr) { pch = ech; }                         // No dash so ignore filetype
            *pch = '\0';                                               // full_ota_url = http://domus1:80/api/arduino/Infinitek
          ...
          if (InfinitekGlobal.ota_factory) {
            char *bch = strrchr(full_ota_url, '/');            // Only consider filename after last backslash prevent change of urls having "-" in it
            if (bch == nullptr) { bch = full_ota_url; }        // No path found so use filename only
            char *ech = strchr(bch, '.');                      // Find file type in filename (none, .ino.bin, .ino.bin.gz, .bin, .bin.gz or .gz)
            if (ech == nullptr) { ech = full_ota_url + strlen(full_ota_url); }  // Point to '/0' at end of full_ota_url becoming an empty string
...
//#ifdef CONFIG_IDF_TARGET_ESP32C3
          ...
          char version[50];
          snprintf_P(version, sizeof(version), PSTR("%s%s"), InfinitekGlobal.version, InfinitekGlobal.image_name);
          ...
          if (!OTAclient.begin(full_ota_url)) {
            ...
            httpUpdateLight.setFactory(InfinitekGlobal.ota_factory);
...
#endif  // ESP8266
#endif  // FIRMWARE_MINIMAL
            InfinitekGlobal.ota_state_flag = 2;             // Upgrade failed - retry
          }
        }
      }
      if (90 == InfinitekGlobal.ota_state_flag) {           // Allow MQTT to reconnect
        InfinitekGlobal.ota_state_flag = 0;
        Response_P(PSTR("{\"" D_CMND_UPGRADE "\":\""));
        if (ota_result) {
          ResponseAppend_P(PSTR(D_JSON_SUCCESSFUL ". " D_JSON_RESTARTING));
          InfinitekGlobal.restart_flag = 5;                 // Allow time for webserver to update console
        } else {
  ...
  case 1:                                                 // Every x.25 second
    ...
    if (InfinitekGlobal.save_data_counter && CommandsReady()) {
      InfinitekGlobal.save_data_counter--;
      if (InfinitekGlobal.save_data_counter <= 0) {
        if (Settings->flag.save_state) {                   // SetOption0 - Save power state and use after restart
          ...
          if (!((Settings->power &mask) == (InfinitekGlobal.power &mask))) {
            Settings->power = InfinitekGlobal.power;
          }
        } else {
          Settings->power = 0;
        }
        if (!InfinitekGlobal.restart_flag) { SettingsSave(0); }
        InfinitekGlobal.save_data_counter = Settings->save_data;
      }
    }

    if (InfinitekGlobal.restart_flag && CommandsReady()) {
      if ((214 == InfinitekGlobal.restart_flag) ||          // Reset 4
          (215 == InfinitekGlobal.restart_flag) ||          // Reset 5
          (216 == InfinitekGlobal.restart_flag)) {          // Reset 6
...
//        if (216 == InfinitekGlobal.restart_flag) {
          // Backup mqtt host, port, client, username and password
//        }
        if ((215 == InfinitekGlobal.restart_flag) ||        // Reset 5
            (216 == InfinitekGlobal.restart_flag)) {        // Reset 6
        ...
        if (216 == InfinitekGlobal.restart_flag) {          // Reset 6
        ...
        InfinitekGlobal.restart_flag = 3;                   // Finish backlog then Restart 1
      }
      else if (213 == InfinitekGlobal.restart_flag) {       // Reset 3
        SettingsSdkErase();  // Erase flash SDK parameters
        InfinitekGlobal.restart_flag = 2;                   // Restart 1
      }
      else if (212 == InfinitekGlobal.restart_flag) {       // Reset 2
        SettingsErase(0);    // Erase all flash from program end to end of physical flash
        InfinitekGlobal.restart_flag = 211;                 // Reset 1
      }

      if (211 == InfinitekGlobal.restart_flag) {            // Reset 1
        SettingsDefault();
        InfinitekGlobal.restart_flag = 3;                   // Finish backlog then Restart 1
      }

      if (2 == InfinitekGlobal.restart_flag) {              // Restart 1
        XsnsXdrvCall(FUNC_ABOUT_TO_RESTART);
        SettingsSaveAll();
      }

      InfinitekGlobal.restart_flag--;
      if (InfinitekGlobal.restart_flag <= 0) {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_APPLICATION "%s"), (InfinitekGlobal.restart_halt) ? PSTR("Halted") : (InfinitekGlobal.restart_deepsleep) ? PSTR("Sleeping") : PSTR(D_RESTARTING));
        EspRestart();
      }
    }
    break;
  case 2:                                                 // Every x.5 second
    if (Settings->flag4.network_wifi) {
      WifiCheck(InfinitekGlobal.wifi_state_flag);
...
//#ifdef CONFIG_IDF_TARGET_ESP32C3
        ...
        if (1 == RtcSettings.ota_loader) {
          ...
          InfinitekGlobal.ota_state_flag = 6;               // Allow time for webserver to update console
        }
#endif  // FIRMWARE_MINIMAL
...
void ArduinoOTAInit(void)
{
  ...
  {
    if ((LOG_LEVEL_DEBUG <= InfinitekGlobal.seriallog_level)) {
  ...
  ArduinoOTA.onError([](ota_error_t error)
  {
    ...
    */
    ...
    if ((LOG_LEVEL_DEBUG <= InfinitekGlobal.seriallog_level) && arduino_ota_progress_dot_count) { TasConsole.println(); }
    switch (error) {
  ...
  ArduinoOTA.onEnd([]()
  {
    if ((LOG_LEVEL_DEBUG <= InfinitekGlobal.seriallog_level)) { TasConsole.println(); }
...
//    yield();
    delay(0);
    InfinitekGlobal.serial_in_byte = Serial.read();

    if (0 == InfinitekGlobal.serial_in_byte_counter) {
      serial_buffer_overrun = false;
    }
    else if ((InfinitekGlobal.serial_in_byte_counter == INPUT_BUFFER_SIZE)
#ifdef ESP8266
             || Serial.hasOverrun()
#endif
                                                             ) {
...
\*-------------------------------------------------------------------------------------------*/
    if ((SONOFF_DUAL == InfinitekGlobal.module_type) || (CH4 == InfinitekGlobal.module_type)) {
      InfinitekGlobal.serial_in_byte = ButtonSerial(InfinitekGlobal.serial_in_byte);
    }
#endif  // ESP8266
/*-------------------------------------------------------------------------------------------*/

    if (XdrvCall(FUNC_SERIAL)) {
      InfinitekGlobal.serial_in_byte_counter = 0;
      Serial.flush();
      return;
    }
...
#ifdef USE_XYZMODEM
    if (XYZModemStart(TXMP_TASCONSOLE, InfinitekGlobal.serial_in_byte)) { return; }
#endif  // USE_XYZMODEM

/*-------------------------------------------------------------------------------------------*/

    if (InfinitekGlobal.serial_in_byte > 127 && !Settings->flag.mqtt_serial_raw) {   // Discard binary data above 127 if no raw reception allowed - CMND_SERIALSEND3
      InfinitekGlobal.serial_in_byte_counter = 0;
      Serial.flush();
      return;
    }
    if (!Settings->flag.mqtt_serial) {                                             // SerialSend active - CMND_SERIALSEND and CMND_SERIALLOG
      if (isprint(InfinitekGlobal.serial_in_byte)) {                                 // Any char between 32 and 127
        if (InfinitekGlobal.serial_in_byte_counter < INPUT_BUFFER_SIZE -1) {         // Add char to string if it still fits
          InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter++] = InfinitekGlobal.serial_in_byte;
        } else {
          serial_buffer_overrun = true;                                            // Signal overrun but continue reading input to flush until '\n' (EOL)
        }
      }
    } else {
      if (InfinitekGlobal.serial_in_byte || Settings->flag.mqtt_serial_raw) {        // Any char between 1 and 127 or any char (0 - 255) - CMND_SERIALSEND3
        bool in_byte_is_delimiter =                                                // Char is delimiter when...
          (((Settings->serial_delimiter < 128) && (InfinitekGlobal.serial_in_byte == Settings->serial_delimiter)) || // Any char between 1 and 127 and being delimiter
          ((Settings->serial_delimiter == 128) && !isprint(InfinitekGlobal.serial_in_byte))) &&   // Any char not between 32 and 127
          !Settings->flag.mqtt_serial_raw;                                         // In raw mode (CMND_SERIALSEND3) there is never a delimiter

        if ((InfinitekGlobal.serial_in_byte_counter < INPUT_BUFFER_SIZE -1) &&       // Add char to string if it still fits and ...
            !in_byte_is_delimiter) {                                               // Char is not a delimiter
          InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter++] = InfinitekGlobal.serial_in_byte;
        }

        if ((InfinitekGlobal.serial_in_byte_counter >= INPUT_BUFFER_SIZE -1) ||      // Send message when buffer is full or ...
...
\*-------------------------------------------------------------------------------------------*/
    if (SONOFF_SC == InfinitekGlobal.module_type) {
      if (InfinitekGlobal.serial_in_byte == '\x1B') {                                // Sonoff SC status from ATMEGA328P
        InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter] = 0;  // Serial data completed
        SonoffScSerialInput(InfinitekGlobal.serial_in_buffer);
        InfinitekGlobal.serial_in_byte_counter = 0;
        Serial.flush();
        return;
      }
...
/*-------------------------------------------------------------------------------------------*/
    ...
    if (!Settings->flag.mqtt_serial && (InfinitekGlobal.serial_in_byte == '\n')) {   // CMND_SERIALSEND and CMND_SERIALLOG
      InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter] = 0;    // Serial data completed
      SetMinimumSeriallog();
      if (serial_buffer_overrun) {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_COMMAND "Serial buffer overrun"));
      } else {
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_COMMAND "%s"), InfinitekGlobal.serial_in_buffer);
        ExecuteCommand(InfinitekGlobal.serial_in_buffer, SRC_SERIAL);
      }
      InfinitekGlobal.serial_in_byte_counter = 0;
  ...
  if (Settings->flag.mqtt_serial && InfinitekGlobal.serial_in_byte_counter && (millis() > (serial_polling_window + SERIAL_POLLING))) {  // CMND_SERIALSEND and CMND_SERIALLOG
    InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter] = 0;      // Serial data completed
    bool assume_json = (!Settings->flag.mqtt_serial_raw && (InfinitekGlobal.serial_in_buffer[0] == '{'));
    ...
    if (assume_json) {
      ResponseAppend_P(InfinitekGlobal.serial_in_buffer);
    } else {
      ResponseAppend_P(PSTR("\""));
      if (Settings->flag.mqtt_serial_raw) {
        ResponseAppend_P(PSTR("%*_H"), InfinitekGlobal.serial_in_byte_counter, InfinitekGlobal.serial_in_buffer);
      } else {
        ResponseAppend_P(EscapeJSONString(InfinitekGlobal.serial_in_buffer).c_str());
      }
      ResponseAppend_P(PSTR("\""));
    }
    ResponseJsonEnd();
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: Used GPIOs %d"), GPIO_SENSOR_END);
  ...
  for (uint32_t i = 0; i < nitems(Settings->my_gp.io); i++) {
    if ((Settings->my_gp.io[i] >= AGPIO(GPIO_SENSOR_END)) && (Settings->my_gp.io[i] < AGPIO(GPIO_USER))) {
      ...
      InfinitekGlobal.my_module.io[i] = Settings->my_gp.io[i];       // Set User selected Module sensors
    }
    if ((template_gp.io[i] > GPIO_NONE) && (template_gp.io[i] < AGPIO(GPIO_USER))) {
      InfinitekGlobal.my_module.io[i] = template_gp.io[i];               // Force Template override
    }
  }

  for (uint32_t i = 0; i < nitems(InfinitekGlobal.my_module.io); i++) {
    uint32_t mpin = ValidPin(i, InfinitekGlobal.my_module.io[i]);
    ...
    if (mpin) {                                     // Above GPIO_NONE
      ...
      if ((mpin >= AGPIO(GPIO_OPTION_A)) && (mpin < (AGPIO(GPIO_OPTION_A) + MAX_OPTIONS_A))) {
        bitSet(InfinitekGlobal.gpio_optiona.data, mpin - AGPIO(GPIO_OPTION_A));
        mpin = GPIO_NONE;
      }
#ifdef ROTARY_V1
        ...
        InfinitekGlobal.emulated_module_type = pgm_read_byte(kModuleEmulationList + (mpin - AGPIO(GPIO_OPTION_E)));
        SetModuleType();
        mpin = GPIO_NONE;
      }
...
//  AddLog(LOG_LEVEL_DEBUG, PSTR("DBG: InfinitekGlobal.gpio_pin %*_V"), nitems(InfinitekGlobal.gpio_pin), (uint8_t*)InfinitekGlobal.gpio_pin);

  if (ResetReasonPowerOn()) {
    InfinitekGlobal.power_on_delay = Settings->param[P_POWER_ON_DELAY2];  // SetOption47 - Delay switching relays to reduce power surge at power on
    if (InfinitekGlobal.power_on_delay) {
      ...
      for (uint32_t i = 0; i < MAX_RELAYS; i++) {
        if (PinUsed(GPIO_REL1, i)) {
          DigitalWrite(GPIO_REL1, i, bitRead(InfinitekGlobal.rel_inverted, i) ? 1 : 0);  // Off
        }
      }
...
#ifdef ESP8266
  if ((2 == Pin(GPIO_TXD)) || (H801 == InfinitekGlobal.module_type)) { Serial.set_tx(2); }
  SetSerialSwap();
#endif
  ...
  InfinitekGlobal.soft_spi_enabled = sspi_mosi + sspi_miso;
  AddLogSpi(0, Pin(GPIO_SSPI_SCLK), Pin(GPIO_SSPI_MOSI), Pin(GPIO_SSPI_MISO));

#ifdef USE_SPI
#ifdef ESP8266
  if (!InfinitekGlobal.soft_spi_enabled) {
    ...
    InfinitekGlobal.spi_enabled = spi_mosi + spi_miso;
    if (!InfinitekGlobal.spi_enabled) {
      ...
      InfinitekGlobal.spi_enabled = (valid_cs) ? SPI_MOSI_MISO : SPI_NONE;
    }
    if (InfinitekGlobal.spi_enabled) {
      InfinitekGlobal.my_module.io[12] = AGPIO(GPIO_SPI_MISO);
      SetPin(12, AGPIO(GPIO_SPI_MISO));
      InfinitekGlobal.my_module.io[13] = AGPIO(GPIO_SPI_MOSI);
      SetPin(13, AGPIO(GPIO_SPI_MOSI));
      InfinitekGlobal.my_module.io[14] = AGPIO(GPIO_SPI_CLK);
  ...
  InfinitekGlobal.spi_enabled2 = spi_mosi + spi_miso;
...
*/
  ...
  DigitalWrite(GPIO_HEARTBEAT, 0, InfinitekGlobal.heartbeat_inverted);
...
/*
  if (PinUsed(GPIO_I2C_SCL) && PinUsed(GPIO_I2C_SDA)) {
    InfinitekGlobal.i2c_enabled[0] = I2cBegin(Pin(GPIO_I2C_SDA), Pin(GPIO_I2C_SCL));
#ifdef ESP32
    if (InfinitekGlobal.i2c_enabled[0]) {
  ...
  if (PinUsed(GPIO_I2C_SCL, 1) && PinUsed(GPIO_I2C_SDA, 1)) {
    InfinitekGlobal.i2c_enabled[1] = I2cBegin(Pin(GPIO_I2C_SDA, 1), Pin(GPIO_I2C_SCL, 1), 1);
    if (InfinitekGlobal.i2c_enabled[1]) {
...
*/
  ...
  for (uint32_t bus = 0; bus < max_bus; bus++) {
    if (PinUsed(GPIO_I2C_SCL, bus) && PinUsed(GPIO_I2C_SDA, bus)) {
      if (I2cBegin(Pin(GPIO_I2C_SDA, bus), Pin(GPIO_I2C_SCL, bus), bus)) {
        if (0 == bus) { 
          InfinitekGlobal.i2c_enabled[0] = true;
        }
#ifdef USE_I2C_BUS2
        else { 
          InfinitekGlobal.i2c_enabled[1] = true;
        }
  ...
  InfinitekGlobal.devices_present = 0;
  uint32_t bi_device = 0;
  for (uint32_t i = 0; i < MAX_RELAYS; i++) {
    if (PinUsed(GPIO_REL1, i)) {
      InfinitekGlobal.devices_present++;
#ifdef ESP8266
      if (EXS_RELAY == InfinitekGlobal.module_type) {
        if (i &1) { InfinitekGlobal.devices_present--; }
      }
#endif  // ESP8266
      if (!Settings->flag6.bistable_single_pin) {  // SetOption152 - (Power) Use single pin bistable
        if (bitRead(InfinitekGlobal.rel_bistable, i)) {
  ...
  InfinitekGlobal.light_type = LT_BASIC;                     // Use basic PWM control if SetOption15 = 0

  if (XdrvCall(FUNC_MODULE_INIT)) {                        // Init and claim single module (like tuya, armtronix, ifan, light)
    // Serviced
  }
#ifdef ESP8266
  else if (YTF_IR_BRIDGE == InfinitekGlobal.module_type) {
    ClaimSerial();  // Stop serial loopback mode
  }
  else if (SONOFF_DUAL == InfinitekGlobal.module_type) {
    UpdateDevicesPresent(2);
    SetSerial(19200, TS_SERIAL_8N1);
  }
  else if (CH4 == InfinitekGlobal.module_type) {
...
#ifdef USE_ARILUX_RF
      if ((3 == i) && (InfinitekGlobal.leds_present < 2) && !PinUsed(GPIO_ARIRFSEL)) {
        SetPin(Pin(GPIO_LED1, i), AGPIO(GPIO_ARIRFSEL));  // Legacy support where LED4 was Arilux RF enable
      } else {
#endif
        InfinitekGlobal.leds_present++;
        DigitalWrite(GPIO_LED1, i, bitRead(InfinitekGlobal.led_inverted, i));
#ifdef USE_ARILUX_RF
      }
#endif
    }
  }
  DigitalWrite(GPIO_LEDLNK, 0, InfinitekGlobal.ledlnk_inverted);

#ifdef USE_PWM_DIMMER
  if (PWM_DIMMER == InfinitekGlobal.module_type && PinUsed(GPIO_REL1)) { InfinitekGlobal.devices_present--; }
#endif  // USE_PWM_DIMMER
