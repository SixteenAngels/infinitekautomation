/*
  support_a_spi.ino - SPI support for Infinitek
...
#ifdef ESP32
  if (2 == bus) {                // SPI bus 2
    if (InfinitekGlobal.spi_enabled2) {
...
void AddLogSpi(uint32_t hardware, int clk, int mosi, int miso) {
  ...
  uint32_t enabled = InfinitekGlobal.soft_spi_enabled;
  char hwswbus[8];
  if (hardware) {
#ifdef ESP8266
    strcpy_P(hwswbus, PSTR("Hard"));
    enabled = InfinitekGlobal.spi_enabled;
#endif      
#ifdef ESP32
    strcpy_P(hwswbus, PSTR("Bus0"));
    hwswbus[3] += (char)hardware;
    enabled = (1 == hardware) ? InfinitekGlobal.spi_enabled : InfinitekGlobal.spi_enabled2;
#endif
  } else {
