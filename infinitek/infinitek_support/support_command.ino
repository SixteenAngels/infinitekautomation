/*
  support_command.ino - command support for Infinitek
...
const char kInfinitekCommands[] PROGMEM = "|"  // No prefix
  // SetOptions synonyms
  D_SO_WIFINOSLEEP "|"
  // Other commands
...
SO_SYNONYMS(kInfinitekSynonyms,
  127,
);
...
{
  // Test WIFI Connection to Router if Infinitek is in AP mode since in AP mode, a STA connection can be established
    ...
    if (Wifi.wifiTest != WIFI_TESTING) { // Start Test
      ...
      if (pos != nullptr) {
        ...
        if (WIFI_NOT_TESTING == Wifi.wifiTest) {
          ...
          InfinitekGlobal.wifi_state_flag = Settings->sta_config = WIFI_MANAGER;
          Wifi.save_data_counter = InfinitekGlobal.save_data_counter;
        }

        Wifi.wifi_test_counter = 9;   // seconds to test user's proposed AP
        Wifi.wifiTest = WIFI_TESTING;
        InfinitekGlobal.save_data_counter = 0;               // Stop auto saving data - Updating Settings
        Settings->save_data = 0;
        InfinitekGlobal.sleep = 0;                           // Disable sleep
        InfinitekGlobal.restart_flag = 0;                    // No restart
        InfinitekGlobal.ota_state_flag = 0;                  // No OTA
        ...
        AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_CONNECTING_TO_AP " %s " D_AS " %s ..."),
          ssid_test, InfinitekGlobal.hostname);
...
void ExecuteCommand(const char *cmnd, uint32_t source)
{
  ...
  while (*pos && (isalpha(*pos) || isdigit(*pos) || '_' == *pos || '/' == *pos)) {
    if ('/' == *pos) {            // Skip possible cmnd/Infinitek/ preamble
      start = pos + 1;
    }
    pos++;
  }
...
// topicBuf:        cmnd/Infinitek/power1  dataBuf: toggle  = Mqtt command using topic
// topicBuf:       cmnd/Infiniteks/power1  dataBuf: toggle  = Mqtt command using a group topic
...
void CommandHandler(char* topicBuf, char* dataBuf, uint32_t data_len) {
  ...
  InfinitekGlobal.fallback_topic_flag = GetFallbackTopicFlag(topicBuf);
  ...
  if (type != nullptr) {
    ...
    if ((i > 1) && ('_' == type[0])) {
      ...
      InfinitekGlobal.no_mqtt_response = true;
    }
  } else {                               // type = nullptr
...
//  snprintf_P(command_line, sizeof(command_line), PSTR("%s%s%s%s"), 
  ...
  if (strlen(type)) {
    if (Settings->ledstate &0x02) { InfinitekGlobal.blinks++; }

//    InfinitekGlobal.backlog_timer = millis() + (100 * MIN_BACKLOG_DELAY);
    InfinitekGlobal.backlog_timer = millis() + Settings->param[P_BACKLOG_DELAY];  // SetOption34
    ...
    if (!Script_SubCmd()) {              // Allow override Infinitek cmds
#endif  // USE_SCRIPT_SUB_COMMAND
      if (!DecodeCommand(kInfinitekCommands, InfinitekCommand, kInfinitekSynonyms)) {
  ...
  if (!strcmp(ResponseData(), "_1")) {
    ...
    InfinitekGlobal.no_mqtt_response = false;  // Make sure to report commands starting with underline
  ...
  if (ResponseLength()) {
    if (InfinitekGlobal.no_mqtt_response){  // If it is activated, Infinitek will not publish MQTT messages, but it will proccess event trigger rules
      XdrvRulesProcess(0);
    } else {
      MqttPublishPrefixTopicRulesProcess_P(RESULT_OR_STAT, type);
    }
  }
  InfinitekGlobal.fallback_topic_flag = false;
  InfinitekGlobal.no_mqtt_response = false;
}
...
void CmndBacklog(void) {
  ...
  if (XdrvMailbox.data_len) {
    InfinitekGlobal.backlog_nodelay = (0 == (XdrvMailbox.index & 0x01));           // Backlog0, Backlog2
    InfinitekGlobal.backlog_no_mqtt_response = (2 == (XdrvMailbox.index & 0x02));  // Backlog2, Backlog3
...
//    ResponseCmndChar(D_JSON_APPENDED);
    ResponseClear();
    InfinitekGlobal.backlog_timer = millis();
  } else {
...
void CmndDelay(void) {
  ...
  if (XdrvMailbox.payload == -1) {
    InfinitekGlobal.backlog_timer = millis() + (1000 - RtcMillis());  // Next second (#18984)
  }
  else if ((XdrvMailbox.payload >= (MIN_BACKLOG_DELAY / 100)) && (XdrvMailbox.payload <= 3600)) {
    InfinitekGlobal.backlog_timer = millis() + (100 * XdrvMailbox.payload);
  }
  uint32_t bl_delay = 0;
  long bl_delta = TimePassedSince(InfinitekGlobal.backlog_timer);
...
void CmndPower(void)
{
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= InfinitekGlobal.devices_present)) {
...
void CmndPowerLock(void) {
  ...
  if (XdrvMailbox.index <= InfinitekGlobal.devices_present) {
    if (XdrvMailbox.payload >= 0) {
      XdrvMailbox.payload &= 1;
      if (0 == XdrvMailbox.index) {  // Control all bits
        Settings->power_lock = (XdrvMailbox.payload) ? (1 << InfinitekGlobal.devices_present) -1 : 0;
      } else {                       // Control individual bits
        bitWrite(Settings->power_lock, XdrvMailbox.index -1, XdrvMailbox.payload & 1);
      }
    }
    char stemp1[33];
    ext_snprintf_P(stemp1, sizeof(stemp1), PSTR("%*_b"), InfinitekGlobal.devices_present, Settings->power_lock);
...
void CmndStatus(void)
{
  ...
  if (!InfinitekGlobal.energy_driver && (9 == payload)) { return; }
#ifndef FIRMWARE_MINIMAL
  ...
  if ((0 == payload) || (-99 == payload)) {
    uint32_t maxfn = (InfinitekGlobal.devices_present > MAX_FRIENDLYNAMES) ? MAX_FRIENDLYNAMES : (!InfinitekGlobal.devices_present) ? 1 : InfinitekGlobal.devices_present;
    ...
    Response_P(PSTR("{\"" D_CMND_STATUS "\":{\"" D_CMND_MODULE "\":%d,\"" D_CMND_DEVICENAME "\":\"%s\",\"" D_CMND_FRIENDLYNAME "\":[%s],\""
                          ...
                          InfinitekGlobal.mqtt_topic, SettingsText(SET_MQTT_BUTTON_TOPIC),
                          InfinitekGlobal.devices_present, InfinitekGlobal.power,
                          InfinitekGlobal.devices_present, Settings->power_lock,
...
#endif
                          "}}"),
                          InfinitekGlobal.baudrate, GetSerialConfig().c_str(), SettingsText(SET_MQTT_GRP_TOPIC), SettingsText(SET_OTAURL),
  ...
  if ((0 == payload) || (2 == payload)) {
    Response_P(PSTR("{\"" D_CMND_STATUS D_STATUS2_FIRMWARE "\":{\"" D_JSON_VERSION "\":\"%s%s%s\",\"" D_JSON_BUILDDATETIME "\":\"%s\""
                          ...
                          "%s}}"),
                          InfinitekGlobal.version, InfinitekGlobal.image_name, GetCodeCores().c_str(), GetBuildDateAndTime().c_str()
#ifdef ESP8266
                          , ESP.getBootVersion()
#endif
                          , ESP.getSdkVersion(),
  ...
  if ((0 == payload) || (4 == payload)) {
    Response_P(PSTR("{\"" D_CMND_STATUS D_STATUS4_MEMORY "\":{\"" D_JSON_PROGRAMSIZE "\":%d,\"" D_JSON_FREEMEMORY "\":%d,\"" D_JSON_HEAPSIZE "\":%d,\""
                          ...
                          ",\"FlashFrequency\":%d,\"" D_JSON_FLASHMODE "\":\"" D_Infinitek_FLASHMODE "\""),
...
//#if defined(ESP32) && CONFIG_IDF_TARGET_ESP32 && defined(USE_ETHERNET)
  ...
  if (((0 == payload) || (6 == payload)) && Settings->flag.mqtt_enabled) {  // SetOption3 - Enable MQTT
    ...
    Response_P(PSTR("{\"" D_CMND_STATUS D_STATUS6_MQTT "\":{\"" D_CMND_MQTTHOST "\":\"%s\",\"" D_CMND_MQTTPORT "\":%d,\"" D_CMND_MQTTCLIENT D_JSON_MASK "\":\"%s\",\""
                          ...
                          InfinitekGlobal.mqtt_client, EscapeJSONString(SettingsText(SET_MQTT_USER)).c_str(), MqttConnectCount(), mqtt_tls, MQTT_MAX_PACKET_SIZE, Settings->mqtt_keepalive, Settings->mqtt_socket_timeout);
  ...
  if (InfinitekGlobal.energy_driver) {
    // Status 9 - StatusPTH
...
void CmndGlobalTemp(void) {
  if (2 == XdrvMailbox.index) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
      ...
      InfinitekGlobal.user_globals[0] = 0;
    }
    ...
    if (XdrvMailbox.data_len > 0) {
      ...
      if ((temperature >= -50.0f) && (temperature <= 100.0f)) {
        InfinitekGlobal.temperature_celsius = temperature;
        InfinitekGlobal.global_update = InfinitekGlobal.uptime;
        InfinitekGlobal.user_globals[0] = 1;
      }
    }
    ResponseCmndFloat(InfinitekGlobal.temperature_celsius, 1);
  }
}
...
void CmndGlobalHum(void) {
  if (2 == XdrvMailbox.index) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
      ...
      InfinitekGlobal.user_globals[1] = 0;
    }
    ...
    if (XdrvMailbox.data_len > 0) {
      ...
      if ((humidity >= 0.0f) && (humidity <= 100.0f)) {
        InfinitekGlobal.humidity = humidity;
        InfinitekGlobal.global_update = InfinitekGlobal.uptime;
        InfinitekGlobal.user_globals[1] = 1;
      }
    }
    ResponseCmndFloat(InfinitekGlobal.humidity, 1);
  }
}
...
void CmndGlobalPress(void) {
  if (2 == XdrvMailbox.index) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
      ...
      InfinitekGlobal.user_globals[2] = 0;
    }
    ...
    if (XdrvMailbox.data_len > 0) {
      ...
      if ((pressure >= 0.0f) && (pressure <= 1200.0f)) {
        InfinitekGlobal.pressure_hpa = pressure;
        InfinitekGlobal.global_update = InfinitekGlobal.uptime;
        InfinitekGlobal.user_globals[2] = 1;
      }
    }
    ResponseCmndFloat(InfinitekGlobal.pressure_hpa, 1);
  }
}
...
void CmndSleep(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 251)) {
    ...
    InfinitekGlobal.sleep = XdrvMailbox.payload;
    WiFiSetSleepMode();
  }
  Response_P(S_JSON_COMMAND_NVALUE_ACTIVE_NVALUE, XdrvMailbox.command, Settings->sleep, InfinitekGlobal.sleep);
...
#ifdef ESP32
    InfinitekGlobal.ota_factory = false;  // Reset in case of failed safeboot upgrade
#endif  // ESP32 and WEBCLIENT_HTTPS
    InfinitekGlobal.ota_state_flag = 3;
    char stemp1[TOPSZ];
    Response_P(PSTR("{\"%s\":\"" D_JSON_VERSION " %s " D_JSON_FROM " %s\"}"), XdrvMailbox.command, InfinitekGlobal.version, GetOtaUrl(stemp1, sizeof(stemp1)));
  }
#if defined(ESP32) && defined(USE_WEBCLIENT_HTTPS)
  else if (EspSingleOtaPartition() && !EspRunningFactoryPartition() && (1 == XdrvMailbox.data_len) && (2 == XdrvMailbox.payload)) {
    InfinitekGlobal.ota_factory = true;
    InfinitekGlobal.ota_state_flag = 3;
    ResponseCmndChar(PSTR("Safeboot"));
  }
#endif  // ESP32 and WEBCLIENT_HTTPS
  else {
    Response_P(PSTR("{\"%s\":\"" D_JSON_ONE_OR_GT "\"}"), XdrvMailbox.command, InfinitekGlobal.version);
  }
}
...
void CmndSeriallog(void)
{
  ...
  Response_P(S_JSON_COMMAND_NVALUE_ACTIVE_NVALUE, XdrvMailbox.command, Settings->seriallog_level, InfinitekGlobal.seriallog_level);
}
...
void CmndRestart(void)
{
  ...
  case 1:
    InfinitekGlobal.restart_flag = 2;
    ResponseCmndChar(PSTR(D_JSON_RESTARTING));
    break;
  case 2:
    InfinitekGlobal.restart_flag = 2;
    InfinitekGlobal.restart_halt = true;
  ...
  case 9:
    InfinitekGlobal.restart_flag = 2;
    InfinitekGlobal.restart_deepsleep = true;
...
{
#ifdef ESP8266
  if (InfinitekGlobal.module_type != MOTOR)
#endif  // ESP8266
  {
...
void CmndBlinktime(void)
{
  if ((XdrvMailbox.payload > 1) && (XdrvMailbox.payload <= 3600)) {
    ...
    if (InfinitekGlobal.blink_timer > 0) {
      InfinitekGlobal.blink_timer = millis() + (100 * XdrvMailbox.payload);
    }
  }
...
void CmndBlinkcount(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 32000)) {
    ...
    if (InfinitekGlobal.blink_counter) { InfinitekGlobal.blink_counter = Settings->blinkcount *2; }
  }
      ...
      if (1 == ptype) {                    // SetOption32 .. 49
        ...
        if ((XdrvMailbox.payload >= param_low) && (XdrvMailbox.payload <= param_high)) {
          ...
          if (P_RGB_REMAP == pindex) {
            ...
            InfinitekGlobal.restart_flag = 2;  // SetOption37 needs a reboot in most cases
          }
#endif
        ...
        if (XdrvMailbox.payload <= 1) {
          if (2 == ptype) {                // SetOption0 .. 31
            ...
            if (12 == pindex) {            // stop_flash_rotate
              InfinitekGlobal.stop_flash_rotate = XdrvMailbox.payload;
              SettingsSave(2);
            }
#ifdef USE_HOME_ASSISTANT
...
#endif  // USE_HOME_ASSISTANT
#ifdef USE_Infinitek_DISCOVERY
            if (19 == pindex) {
              TasRediscover();
            }
#endif  // USE_Infinitek_DISCOVERY
          }
            ...
            switch (pindex) {
              case 5:                      // SetOption55
                if (0 == XdrvMailbox.payload) {
                  InfinitekGlobal.restart_flag = 2;  // Disable mDNS needs restart
                }
                break;
            ...
            switch (pindex) {
              ...
              case 5:                      // SetOption87 - (PWM Dimmer) Turn red LED on (1) when powered off
                InfinitekGlobal.restore_powered_off_led_counter = 1;
                break;
  ...
  if (strchr(XdrvMailbox.data, '{') == nullptr) {  // If no JSON it must be parameter
    if ((XdrvMailbox.payload > 0) && (XdrvMailbox.payload <= MAXMODULE)) {
      ...
      if (ValidTemplateModule(XdrvMailbox.payload)) {
        ...
        if (USER_MODULE == Settings->module) { InfinitekGlobal.restart_flag = 2; }
      }
    }
...
#endif  // ESP32
        if (InfinitekGlobal.my_module.io[j] > GPIO_NONE) {
          Settings->user_template.gp.io[i] = InfinitekGlobal.my_module.io[j];
        }
        j++;
      }
    }
...
#ifndef FIRMWARE_MINIMAL      // if Infinitek-minimal, `Template` is read-only
...
void CmndBaudrate(void)
{
  if (XdrvMailbox.payload >= 300) {
    ...
    InfinitekGlobal.baudrate = (XdrvMailbox.payload & 0xFFFF) * 300;
    SetSerialBaudrate(InfinitekGlobal.baudrate);
  }
  ResponseCmndNumber(InfinitekGlobal.baudrate);
}

void CmndSerialConfig(void)
{
  // See InfinitekSerialConfig for possible options
...
void CmndSyslog(void)
{
  ...
  Response_P(S_JSON_COMMAND_NVALUE_ACTIVE_NVALUE, XdrvMailbox.command, Settings->syslog_level, InfinitekGlobal.syslog_level);
}
...
void CmndNtpServer(void)
{
  ...
  //        InfinitekGlobal.restart_flag = 2;  // Issue #3890
        InfinitekGlobal.ntp_force_sync = true;
      }
...
void CmndWifiConfig(void)
{
  if ((XdrvMailbox.payload >= WIFI_RESTART) && (XdrvMailbox.payload < MAX_WIFI_OPTION)) {
    ...
    InfinitekGlobal.wifi_state_flag = Settings->sta_config;
...
void CmndInterlock(void)
{
  ...
  uint32_t max_relays = InfinitekGlobal.devices_present;
  if (InfinitekGlobal.light_type) { max_relays--; }
    ...
    if (XdrvMailbox.data_len > 0) {
      if (strchr(XdrvMailbox.data, ',') != nullptr) {           // Interlock entry
        ...
        if (Settings->flag.interlock) {
          SetDevicePower(InfinitekGlobal.power, SRC_IGNORE);                    // Remove multiple relays if set
        }
      }
#ifdef USE_SHUTTER
...
void CmndTeleperiod(void)
{
  ...
  InfinitekGlobal.tele_period = (Settings->tele_period) ? Settings->tele_period : 3601;  // Show teleperiod data also on empty command
...
void CmndReset(void)
{
  switch (XdrvMailbox.payload) {
  case 1:
    InfinitekGlobal.restart_flag = 211;
    ResponseCmndChar(PSTR(D_JSON_RESET_AND_RESTARTING));
    break;
  case 2 ... 6:
    InfinitekGlobal.restart_flag = 210 + XdrvMailbox.payload;
...
void CmndLedPower(void) {
  // If GPIO_LEDLINK (used for network status) then allow up to 4 GPIO_LEDx control using InfinitekGlobal.led_power
  ...
  if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_LEDS)) {
    ...
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= 2)) {
      ...
      InfinitekGlobal.blinks = 0;
      if (!PinUsed(GPIO_LEDLNK)) {
        SetLedPower(Settings->ledstate &8);
      } else {
        SetLedPowerIdx(XdrvMailbox.index -1, (InfinitekGlobal.led_power & mask));
      }
    }
    bool state = bitRead(InfinitekGlobal.led_power, XdrvMailbox.index -1);
    if (!PinUsed(GPIO_LEDLNK)) {
...
void CmndSetSensor(void)
{
  if (XdrvMailbox.index < MAX_XSNS_DRIVERS) {
    if (XdrvMailbox.payload >= 0) {
      ...
      if (1 == XdrvMailbox.payload) {
        InfinitekGlobal.restart_flag = 2;  // To safely re-enable a sensor currently most sensor need to follow complete restart init cycle
      }
    }
