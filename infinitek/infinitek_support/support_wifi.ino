/*
  support_wifi.ino - wifi support for Infinitek
...
void WifiSetMode(WiFiMode_t wifi_mode) {
  ...
  if (wifi_mode != WIFI_OFF) {
    WiFi.hostname(InfinitekGlobal.hostname);  // ESP32 needs this here (before WiFi.mode) for core 2.0.0
 ...
 * - InfinitekGlobal.sleep: Global sleep setting
 ...
 * - InfinitekGlobal.wifi_stay_asleep: Flag to maintain sleep state
 * 
 ...
 * See https://github.com/arendst/Infinitek/issues/2559
 */

// Sleep explanation: https://github.com/esp8266/Arduino/blob/3f0c601cfe81439ce17e9bd5d28994a7ed144482/libraries/ESP8266WiFi/src/ESP8266WiFiGeneric.cpp#L255
/*
  if (InfinitekGlobal.sleep && Settings->flag3.sleep_normal) {  // SetOption60 - Enable normal sleep instead of dynamic sleep
...
//#endif
  if (0 == InfinitekGlobal.sleep || wifi_no_sleep) {
    if (!InfinitekGlobal.wifi_stay_asleep) {
...
//  if (WiFiHelper::getPhyMode() != WIFI_PHY_MODE_11G) { WiFiHelper::setPhyMode(WIFI_PHY_MODE_11G); }  // B/G
  ...
  WiFi.hostname(InfinitekGlobal.hostname);  // ESP8266 needs this here (after WiFi.mode)
  ...
  AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_CONNECTING_TO_AP "%d %s%s " D_IN_MODE " %s " D_AS " %s..."),
    Settings->sta_active +1, SettingsText(SET_STASSID1 + Settings->sta_active), stemp, WifiGetPhyMode().c_str(), InfinitekGlobal.hostname);
  ...
  if (Settings->flag5.wait_for_wifi_result) {  // SetOption142 - (Wifi) Wait 1 second for wifi connection solving some FRITZ!Box modem issues (1)
    WiFi.waitForConnectResult(1000);  // https://github.com/arendst/Infinitek/issues/14985
  }
 ...
 * - InfinitekGlobal.global_state.wifi_down (inverted state)
 * - InfinitekGlobal.global_state.network_down (cleared when WiFi is up)
 * 
...
{
  if (state == InfinitekGlobal.global_state.wifi_down) {
    if (state) {
      InfinitekGlobal.rules_flag.wifi_connected = 1;
      Wifi.link_count++;
      Wifi.downtime += UpTime() - Wifi.last_event;
    } else {
      InfinitekGlobal.rules_flag.wifi_disconnected = 1;
      Wifi.last_event = UpTime();
    }
  }
  InfinitekGlobal.global_state.wifi_down = state ^1;
  if (!InfinitekGlobal.global_state.wifi_down) {
    InfinitekGlobal.global_state.network_down = 0;
  }
}
...
//        SettingsSdkErase();  //  Disabled v6.1.0b due to possible bad wifi connects
      ...
      if ((WL_CONNECTED == WiFi.status()) && WifiHasIP() && !Wifi.config_type) {
        ...
        if (Settings->flag3.use_wifi_rescan) {  // SetOption57 - Scan wifi network every 44 minutes for configured AP's
          if (!(InfinitekGlobal.uptime % (60 * WIFI_RESCAN_MINUTES))) {
...
int WifiState(void)
{
  ...
  if (!InfinitekGlobal.global_state.wifi_down) { state = WIFI_RESTART; }
...
//#ifdef ESP8266
  // https://github.com/arendst/Infinitek/issues/16061#issuecomment-1216970170
...
void WifiDisable(void) {
  ...
  InfinitekGlobal.global_state.wifi_down = 1;
}
 ...
 * 2. Halt (InfinitekGlobal.restart_halt): Enters an infinite loop with watchdog feeding
 * 3. Deep sleep (InfinitekGlobal.restart_deepsleep): Enters deep sleep mode
 * 
...
void EspRestart(void) {
  ...
  if (InfinitekGlobal.restart_halt) {  // Restart 2
    while (1) {
  ...
  else if (InfinitekGlobal.restart_deepsleep) {  // Restart 9
  #ifdef USE_DEEPSLEEP
    DeepSleepStart();
 ...
 * 
 * The function is used throughout Infinitek for all DNS resolution needs,
 ...
 *    - Forced sync requests via InfinitekGlobal.ntp_force_sync
 * 
...
void WifiPollNtp() {
  ...
  if (InfinitekGlobal.global_state.network_down || Rtc.user_time_entry) { return; }

  uint8_t uptime_minute = (InfinitekGlobal.uptime / 60) % 60;  // 0 .. 59
  ...
  uint8_t offset = (InfinitekGlobal.uptime < 30) ? RtcTime.second + ntp_run_time : (((ESP_getChipId() & 0xF) * 3) + 3) ;

  if ( (((offset == RtcTime.second) && ( (RtcTime.year < 2016) ||                  // Never synced
                                         (ntp_sync_minute == uptime_minute))) ||   // Re-sync every hour
       InfinitekGlobal.ntp_force_sync ) ) {                                          // Forced sync

    InfinitekGlobal.ntp_force_sync = false;
