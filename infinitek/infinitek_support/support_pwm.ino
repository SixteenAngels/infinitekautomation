/*
  support_pwm.ino - command support for Infinitek
...
void PwmRearmChanges(void) {
  for (uint32_t i = 0; i < MAX_PWMS; i++) {
    ...
    InfinitekGlobal.pwm_value[i] = -1;          // no change wanted
    InfinitekGlobal.pwm_phase[i] = -1;          // no change wanted
  }
}
...
//       InfinitekGlobal.pwm_cur_value[i] = Settings->pwm_value[i];    // retrieve in Legacy pool for 0..4
//     } else {
//       InfinitekGlobal.pwm_cur_value[i] = Settings->pwm_value_ext[i - MAX_PWMS_LEGACY];    // retrieve in Legacy pool for 5..15
//     }
//     InfinitekGlobal.pwm_cur_phase[i] = 0;       // no phase shift for now, will be recomputed at first push to GPIOs
//   }
...
void PwmSaveToSettings(void) {
  for (uint32_t i = 0; i < MAX_PWMS; i++) {
    if (i < MAX_PWMS_LEGACY) {
      Settings->pwm_value[i] = InfinitekGlobal.pwm_cur_value[i];    // store in Legacy pool for 0..4
    } else {
      Settings->pwm_value_ext[i - MAX_PWMS_LEGACY] = InfinitekGlobal.pwm_cur_value[i];    // retrieve in Legacy pool for 5..15
    }
  }
}
...
// As input, `InfinitekGlobal.pwm_value[]` and `InfinitekGlobal.pwm_phase[]` contain the new expected values
// or `-1` if no change.
...
void PwmApplyGPIO(bool force_update_all) {
  ...
  for (uint32_t i = 0; i < MAX_PWMS; i++) {
    ...
    uint32_t pwm_val = InfinitekGlobal.pwm_cur_value[i];      // logical value of PWM, 0..1023
    uint32_t pwm_phase = InfinitekGlobal.pwm_cur_phase[i];    // pwm_phase is the logical phase of the active pulse, ignoring inverted
    ...
    if (PinUsed(GPIO_PWM1, i)) {
      ...
      if (InfinitekGlobal.pwm_value[i] >= 0) { pwm_val = InfinitekGlobal.pwm_value[i]; }    // new value explicitly specified
      if (pwm_val > range) { pwm_val = range; } // prevent overflow

      // compute phase
      if (InfinitekGlobal.pwm_phase[i] >= 0) {
        pwm_phase = InfinitekGlobal.pwm_phase[i]; // if explicit set explicitly, 
      ...
      // AddLog(LOG_LEVEL_INFO, "PWM: i=%i used=%i pwm_val=%03X vs %03X pwm_phase=%03X vs %03X", i, PinUsed(GPIO_PWM1, i), pwm_val, InfinitekGlobal.pwm_cur_value[i], pwm_phase, InfinitekGlobal.pwm_cur_phase[i]);

      if (force_update_all || (pwm_val != InfinitekGlobal.pwm_cur_value[i]) || (pwm_phase != InfinitekGlobal.pwm_cur_phase[i])) {
    ...
    InfinitekGlobal.pwm_cur_value[i] = pwm_val;
    InfinitekGlobal.pwm_cur_phase[i] = pwm_phase;
  }
  // AddLog(LOG_LEVEL_INFO, "PWM: Val=%03X-%03X-%03X-%03X-%03X Phase=%03X-%03X-%03X-%03X-%03X Range=%03X",
  //                         InfinitekGlobal.pwm_cur_value[0], InfinitekGlobal.pwm_cur_value[1], InfinitekGlobal.pwm_cur_value[2], InfinitekGlobal.pwm_cur_value[3],
  //                         InfinitekGlobal.pwm_cur_value[4],
  //                         InfinitekGlobal.pwm_cur_phase[0], InfinitekGlobal.pwm_cur_phase[1], InfinitekGlobal.pwm_cur_phase[2], InfinitekGlobal.pwm_cur_phase[3],
  //                         InfinitekGlobal.pwm_cur_phase[4],
  //                         Settings->pwm_range
  //                         );
...
void CmndPwm(void)
{
  if (InfinitekGlobal.pwm_present && (XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_PWMS)) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= Settings->pwm_range) && PinUsed(GPIO_PWM1, XdrvMailbox.index -1)) {
      InfinitekGlobal.pwm_value[XdrvMailbox.index - 1] = XdrvMailbox.payload;
      PwmApplyGPIO(false);
    }
    Response_P(PSTR("{"));
...
void GpioInitPwm(void) {
  ...
  for (uint32_t i = 0; i < MAX_PWMS; i++) {     // Basic PWM control only
    if (PinUsed(GPIO_PWM1, i)) {
      analogAttach(Pin(GPIO_PWM1, i), bitRead(InfinitekGlobal.pwm_inverted, i));

      if (i < InfinitekGlobal.light_type) {
        // force PWM GPIOs to black
        InfinitekGlobal.pwm_value[i] = 0;
      } else {
        InfinitekGlobal.pwm_present = true;
        if (i < MAX_PWMS_LEGACY) {
          InfinitekGlobal.pwm_value[i] = Settings->pwm_value[i];
        } else {
          InfinitekGlobal.pwm_value[i] = Settings->pwm_value_ext[i - MAX_PWMS_LEGACY];
        }
      }
    }
  }
...
void CmndPwm(void)
{
  if (InfinitekGlobal.pwm_present && (XdrvMailbox.index > 0) && (XdrvMailbox.index <= MAX_PWMS)) {
    if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload <= Settings->pwm_range) && PinUsed(GPIO_PWM1, XdrvMailbox.index -1)) {
      ...
      AnalogWrite(Pin(GPIO_PWM1, pwm_index), bitRead(InfinitekGlobal.pwm_inverted, pwm_index) ? Settings->pwm_range - XdrvMailbox.payload : XdrvMailbox.payload);
    }
    Response_P(PSTR("{"));
    MqttShowPWMState();  // Render the PWM status to MQTT
    ResponseJsonEnd();
  ...
  for (uint32_t i = 0; i < MAX_PWMS; i++) {     // Basic PWM control only
    if (PinUsed(GPIO_PWM1, i)) {
      ...
      if (i < InfinitekGlobal.light_type) {
        ...
        AnalogWrite(Pin(GPIO_PWM1, i), bitRead(InfinitekGlobal.pwm_inverted, i) ? Settings->pwm_range : 0);
      } else {
        InfinitekGlobal.pwm_present = true;
        if (i < MAX_PWMS_LEGACY) {
          AnalogWrite(Pin(GPIO_PWM1, i), bitRead(InfinitekGlobal.pwm_inverted, i) ? Settings->pwm_range - Settings->pwm_value[i] : Settings->pwm_value[i]);
        } else {
          AnalogWrite(Pin(GPIO_PWM1, i), bitRead(InfinitekGlobal.pwm_inverted, i) ? Settings->pwm_range - Settings->pwm_value_ext[i] : Settings->pwm_value_ext[i]);
        }
      }
    }
  }
