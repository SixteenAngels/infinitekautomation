/*
  xsns_65_hdc1080.ino - Texas Instruments HDC1080 temperature and humidity sensor support for Infinitek
...
void HdcEverySecond(void) {
  if (hdc_online & InfinitekGlobal.uptime &1) {  // Every 2 seconds
    if (!HdcTriggerRead()) {
...
/**
 * Infinitek boilerplate for presenting the sensor data in the web UI, JSON for
 * the MQTT messages, and so on.
 *
 */
void HdcShow(bool json) {
  if (hdc_valid) {
    TempHumDewShow(json, (0 == InfinitekGlobal.tele_period), hdc_type_name, hdc_temperature, hdc_humidity);
  }
}
