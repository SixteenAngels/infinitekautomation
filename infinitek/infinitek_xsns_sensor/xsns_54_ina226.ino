/*
  xsns_54_ina226.ino - INA226 Current Sensor support for Infinitek
...
*    c. Sensor54 2 saves the settings and restarts Infinitek. The device should show up after the system boots again.
  ...
  if (p1 < 10 || p1 >= 50){
    ...
    switch (p1){
      ...
      case 2: // Save and restart
        ...
        Response_P(PSTR("{\"Sensor54-Command-Result\":{\"Restart_flag\":%d}}"),InfinitekGlobal.restart_flag);
        break;
...
/**
 * The callback function Xsns_57() interfaces Infinitek with the sensor driver.
 *
 * It provides the Infinitek callback IDs.
 *
 * @param   byte    callback_id  Infinitek function ID.
...
bool Xsns54(uint32_t callback_id)
{
  ...
  // Check which callback ID is called by Infinitek
  switch (callback_id) {
