/*
  xsns_21_sgp30.ino - SGP30 gas and air quality sensor support for Infinitek
...
void Sgp30Update(void)  // Perform every second to ensure proper operation of the baseline compensation algorithm
{
  ...
  if (InfinitekGlobal.global_update && (InfinitekGlobal.humidity > 0) && !isnan(InfinitekGlobal.temperature_celsius)) {
    // abs hum in mg/m3
    sgp30_abshum = sgp30_AbsoluteHumidity(InfinitekGlobal.temperature_celsius, InfinitekGlobal.humidity);
...
void Sgp30Show(bool json)
{
  if (sgp30_ready) {
    ...
    if (json) {
      ...
      ResponseJsonEnd();
#ifdef USE_DOMOTICZ
      if (0 == InfinitekGlobal.tele_period) DomoticzSensor(DZ_AIRQUALITY, sgp.eCO2);
#endif  // USE_DOMOTICZ
#ifdef USE_WEBSERVER
    } else {
