/*
  xsns_18_pms5003.ino - PMS3003, PMS5003, PMS7003 particle concentration sensor support for Infinitek
...
bool PmsReadData(void)
{
  ...
  if (!Pms.discovery_triggered) {
    InfinitekGlobal.discovery_counter = 1;      // Force discovery
...
void PmsInit(void) {
  ...
  if (PinUsed(GPIO_PMS5003_RX)) {
    PmsSerial = new InfinitekSerial(Pin(GPIO_PMS5003_RX), (PinUsed(GPIO_PMS5003_TX)) ? Pin(GPIO_PMS5003_TX) : -1, 1);
