/*
  xsns_09_bmp.ino - BMP pressure, temperature, humidity and gas sensor support for Infinitek
...
#ifdef USE_I2C_BUS2
        if (InfinitekGlobal.i2c_enabled[1]) {           // Second bus enabled
...
#ifdef USE_DOMOTICZ
        if ((0 == InfinitekGlobal.tele_period) && (0 == bmp_idx)) {  // We want the same first sensor to report to Domoticz in case a read is missed
