    ...
    if (pccsd->refresh_count >= EVERYNSECONDS) {
      ...
      if (pccsd->ccsinstance.available()) {
        if (!pccsd->ccsinstance.readData()){
          ...
          if ((InfinitekGlobal.global_update) &&
              (InfinitekGlobal.humidity > 0) &&
              (!isnan(InfinitekGlobal.temperature_celsius))) {
            pccsd->ccsinstance.setEnvironmentalData((uint8_t)InfinitekGlobal.humidity,
                                                    InfinitekGlobal.temperature_celsius);
          }
          pccsd->reset_count = 0;
        }
      } else {
