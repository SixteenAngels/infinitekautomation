/*
  xsns_78_xezo.ino - EZO family I2C driver support for Infinitek
...
struct EZOManager {
  void Command()
  {
    ...
    // PS: This should ideally be done through the Infinitek mailbox
    if (at[0] == '-') {
  ...
  void EverySecond()
  {
    ...
    if (count < 0) {
      ...
      if (InfinitekGlobal.uptime >= next) {
        count++;

        if (count == -1) {
          DetectRequest();
          next = InfinitekGlobal.uptime + 1;
        } else if (count == 0) {
          ProcessDetection();
        }
      }
