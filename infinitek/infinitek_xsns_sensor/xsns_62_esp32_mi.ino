/*
  xsns_62_esp32_mi.ino - MI-BLE-sensors via ESP32 support for Infinitek
...
void MI32Init(void) {
  ...
  if (InfinitekGlobal.global_state.wifi_down && InfinitekGlobal.global_state.eth_down) {
  ...
  if (!MI32.mode.init) {
  ...
  #endif
    const std::string name(InfinitekGlobal.hostname);
    ...
    AddLog(LOG_LEVEL_INFO,PSTR("M32: Init BLE device: %s"),InfinitekGlobal.hostname);
    MI32.mode.init = 1;
...
if(decryptRet!=0){
  ...
  switch(_payload.type){
    case 0x0002:
      AddLog(LOG_LEVEL_DEBUG_MORE,PSTR("M32: obj id to pair: %02x"),_payload.objID); // pretty pointless for Infinitek
      break;
    case 0x1001:
...
void MI32Show(bool json)
{
  if (json) {
    ...
    if(InfinitekGlobal.masterlog_level == LOG_LEVEL_DEBUG_MORE) return; // we want to announce sensors unlinked to the ESP, check for LOG_LEVEL_DEBUG_MORE is medium-safe
    MI32suspendScanTask();
    for (uint32_t i = 0; i < MIBLEsensors.size(); i++) {
      ...
      if(MI32.mode.triggeredTele == 1){
        ...
        if (MIBLEsensors[i].feature.Btn == 1){
          if(MIBLEsensors[i].eventType.Btn == 1){
            MI32ShowContinuation(&commaflg);
            ResponseAppend_P(PSTR("\"Button%u\":%u"),MIBLEsensors[i].Btn,MIBLEsensors[i].BtnType + 1); //internal type is Xiaomi/Homekit 0,1,2 -> Infinitek 1,2,3
          }
        }
