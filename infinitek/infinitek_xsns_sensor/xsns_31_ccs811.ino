/*
  xsns_31_ccs811.ino - CCS811 gas and air quality sensor support for Infinitek
    ...
    if (ccs.available()) {
      if (!ccs.readData()){
        ...
        if (InfinitekGlobal.global_update && (InfinitekGlobal.humidity > 0) && !isnan(InfinitekGlobal.temperature_celsius)) {
          ccs.setEnvironmentalData((uint8_t)InfinitekGlobal.humidity, InfinitekGlobal.temperature_celsius);
        }
        ecnt = 0;
      }
    } else {
...
#ifdef USE_DOMOTICZ
      if (0 == InfinitekGlobal.tele_period) DomoticzSensor(DZ_AIRQUALITY, eCO2);
#endif  // USE_DOMOTICZ
#ifdef USE_WEBSERVER
    } else {
