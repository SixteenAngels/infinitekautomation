/*
  xsns_75_prometheus.ino - Web based information for Infinitek
...
void WritePromMetric(const char *name, uint8_t flags, const char *value, va_list labels) {
  PGM_P const prefix = PSTR("Infinitek_");
  PGM_P tmp;
  String lval;
...
void HandleMetrics(void) {
  ...
  WritePromMetricInt32(PSTR("info"), kPromMetricGauge, 1,
    PSTR("version"), InfinitekGlobal.version,
    PSTR("image"), InfinitekGlobal.image_name,
  ...
  WritePromMetricInt32(PSTR("uptime_seconds"), kPromMetricGauge, InfinitekGlobal.uptime, nullptr);
  ...
  if (!isnan(InfinitekGlobal.temperature_celsius)) {
    WritePromMetricDec(PSTR("global_temperature_celsius"), kPromMetricGauge,
      InfinitekGlobal.temperature_celsius, Settings->flag2.temperature_resolution,
      nullptr);
  }

  if (InfinitekGlobal.humidity != 0) {
    WritePromMetricDec(PSTR("global_humidity_percentage"), kPromMetricGauge,
      InfinitekGlobal.humidity, Settings->flag2.humidity_resolution,
      nullptr);
  }

  if (InfinitekGlobal.pressure_hpa != 0) {
    WritePromMetricDec(PSTR("global_pressure_hpa"), kPromMetricGauge,
      InfinitekGlobal.pressure_hpa, Settings->flag2.pressure_resolution,
      nullptr);
  }
  ...
  for (uint32_t device = 0; device < InfinitekGlobal.devices_present; device++) {
    ...
    WritePromMetricInt32(namebuf, kPromMetricGauge,
      (InfinitekGlobal.power & mask), nullptr);
  }
