/*
  xsns_79_as608.ino - AS608 and R503 fingerprint sensor support for Infinitek
 ...
 *
 * Uses Adafruit-Fingerprint-sensor-library with InfinitekSerial
 *
 * Changes made to Adafruit_Fingerprint.h and Adafruit_Fingerprint.cpp:
 * - Replace SoftwareSerial with InfinitekSerial
...
void As608Init(void) {
  if (PinUsed(GPIO_AS608_RX) && PinUsed(GPIO_AS608_TX)) {
    As608Serial = new InfinitekSerial(Pin(GPIO_AS608_RX), Pin(GPIO_AS608_TX), 0);
...
void As608Loop(void) {
  if (InfinitekGlobal.uptime < 6) { return; }  // Alow time for initial led breathing
  uint32_t p = 0;
