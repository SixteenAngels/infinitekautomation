/*
  xsns_22_sr04.ino - SR04 ultrasonic sensor support for Infinitek
...
struct {
  ...
  InfinitekSerial* sonar_serial = nullptr;
} SR04[MAX_SR04];
...
void Sr04TModeDetect(void) {
  for (uint32_t i = 0; i < MAX_SR04; i++) {
    ...
    SR04[i].sonar_serial = new InfinitekSerial(sr04_echo_pin, sr04_trig_pin, 1);
...
void Sr04TReading(uint32_t i) {
  if (InfinitekGlobal.uptime < 3) { return; }
  if (SR04[i].valid) {
    SR04[i].valid--;
  } else {
    SR04[i].distance = 0;
