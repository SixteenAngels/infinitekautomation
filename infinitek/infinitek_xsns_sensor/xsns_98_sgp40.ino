/*
  xsns_98_sgp40.ino - SGP40 gas and air quality sensor support for Infinitek
 ...
 * Adaption for Infinitek: Jean-Pierre Deschamps
 *
 * I2C Address: 0x59
\*********************************************************************************************/
...
{
  sgp40_ready = false;
  if (InfinitekGlobal.global_update && (InfinitekGlobal.humidity > 0) && !isnan(InfinitekGlobal.temperature_celsius)) {
    // abs hum in mg/m3
    sgp40_abshum = sgp40_AbsoluteHumidity(InfinitekGlobal.temperature_celsius, InfinitekGlobal.humidity);
  }
  sgp40_ready = true;

  // these should normally be stored permanently and used for fast restart
  if (!(InfinitekGlobal.uptime%SAVE_PERIOD)) {
    // store settings every N seconds
    raw_base = sgp40.measureRaw(InfinitekGlobal.temperature_celsius, InfinitekGlobal.humidity);
    voc_index = sgp40.measureVocIndex(InfinitekGlobal.temperature_celsius, InfinitekGlobal.humidity);
  }
}
...
void Sgp40Show(bool json)
{
  if (sgp40_ready) {
    char abs_hum[33];
    bool ahum_available = InfinitekGlobal.global_update && (InfinitekGlobal.humidity > 0) && !isnan(InfinitekGlobal.temperature_celsius);
    if (ahum_available) {
    ...
    if (json) {
      ...
      ResponseJsonEnd();
#ifdef USE_DOMOTICZ
      if (0 == InfinitekGlobal.tele_period) {
