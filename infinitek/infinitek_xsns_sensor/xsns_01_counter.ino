/*
  xsns_01_counter.ino - Counter sensors (water meters, electricity meters etc.) sensor support for Infinitek
  ...
  for (uint32_t i = 0; i < MAX_COUNTERS; i++) {
    if (PinUsed(GPIO_CNTR1, i)) {
      ...
      if (json) {
        ...
        header = true;
#ifdef USE_DOMOTICZ
        if ((0 == InfinitekGlobal.tele_period) && (1 == dsxflg)) {
          DomoticzSensor(DZ_COUNT, RtcSettings.pulse_counter[i]);
          dsxflg++;
        }
#endif  // USE_DOMOTICZ
        if ((0 == InfinitekGlobal.tele_period ) && (Settings->flag3.counter_reset_on_tele)) {
