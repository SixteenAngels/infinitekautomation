/*
  xsns_02_analog.ino - ADC support for Infinitek
...
#endif
  for (pin; pin < nitems(InfinitekGlobal.gpio_pin); pin++) {
    uint32_t adc_type = InfinitekGlobal.gpio_pin[pin] >> 5;
    switch(adc_type) {
      case GPIO_ADC_MQ:
      ...
      case GPIO_ADC_CURRENT:
        ...
        Adc[Adcs.present].index = InfinitekGlobal.gpio_pin[pin] & 0x001F;
        Adcs.present++;
    ...
    switch (adc_type) {
      ...
      case GPIO_ADC_TEMP: {
        ...
        if (json) {
          ...
          if ((0 == InfinitekGlobal.tele_period) && (!domo_flag[ADC_TEMP])) {
#ifdef USE_DOMOTICZ
...
#ifdef USE_DOMOTICZ
          if ((0 == InfinitekGlobal.tele_period) && (!domo_flag[ADC_LIGHT])) {
...
#ifdef USE_DOMOTICZ
          if ((0 == InfinitekGlobal.tele_period) && (!domo_flag[ADC_CT_POWER])) {
      ...
      case GPIO_ADC_VOLTAGE: 
#if defined(ESP32) && defined(USE_ENERGY_SENSOR)
        if (InfinitekGlobal.energy_driver != XNRG_33)
#endif  // ESP32 and USE_ENERGY_SENSOR
        {
  ...
  switch (function) {
    ...
    case FUNC_PRE_INIT: {
        ...
        if (voltage_count && current_count) {
          ...
          InfinitekGlobal.energy_driver = XNRG_33;
        }
      }
      break;
  }
