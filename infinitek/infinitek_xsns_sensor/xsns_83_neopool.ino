/*
  xsns_83_neopool.ino - Sugar Valley NeoPool Control System Modbus support for Infinitek
 ...
 * Infinitek is a Modbus client (formerly known as a master).
...
#define NEOPOOL_UNDEF_UINT16  0xFFFF
  ...
  #define NEOPOOL_InfinitekMODBUS_ERROR_NUM_MAX 15 // 0-14 - see InfinitekModbus.h class InfinitekModbus highest error #
  // counting modbus and data error
  struct {
    uint32_t time;                // time where counting started
    uint32_t mb_requests;         // request count
                                  // result count:
    uint32_t mb_results[NEOPOOL_InfinitekMODBUS_ERROR_NUM_MAX + 1];
 ...
 *              0 - sync with Infinitek local time
 *              1 - sync with Infinitek utc time
...
void NeoPoolInit(void) {
  ...
  if (PinUsed(GPIO_NEOPOOL_RX) && PinUsed(GPIO_NEOPOOL_TX)) {
    NeoPoolModbus = new InfinitekModbus(Pin(GPIO_NEOPOOL_RX), Pin(GPIO_NEOPOOL_TX));
