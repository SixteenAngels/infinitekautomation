/*
  xsns_14_sht3x.ino - SHT3X, SHT4X and SHTCX temperature and humidity sensor support for Infinitek
...
void Sht3xShow(bool json) {
  ...
  for (uint32_t idx = 0; idx < sht3x_count; idx++) {
    if (sht3x_sensors[idx].valid) {
      ...
      TempHumDewShow(json, ((0 == InfinitekGlobal.tele_period) && (0 == idx)), types, sht3x_sensors[idx].temp, sht3x_sensors[idx].humi);
    }
  }
}
