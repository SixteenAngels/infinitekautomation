/*
  xlgt_02_my92x1.ino - led support for Infinitek
...
void My92x1ModuleSelected(void)
{
  if (PinUsed(GPIO_DCKI) && PinUsed(GPIO_DI)) {
    ...
    My92x1.model = 2;
    InfinitekGlobal.light_type = LT_RGBW;                 // RGBW (2 chips) as used in Lohas
#ifdef ESP8266
    if (AILIGHT == InfinitekGlobal.module_type) {         // RGBW (1 chip) as used in Ailight
      My92x1.model = 0;
//      InfinitekGlobal.light_type = LT_RGBW;
    }
    else if (SONOFF_B1 == InfinitekGlobal.module_type) {  // RGBWC (2 chips) as used in Sonoff B1
      My92x1.model = 1;
    ...
    InfinitekGlobal.light_driver = XLGT_02;
