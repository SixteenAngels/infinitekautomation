/*
  xlgt_05_sonoff_l1.ino - Sonoff L1 led support for Infinitek
...
#define SONOFF_L1_START_DELAY                // Sync Nuvotron power state with Infinitek on power up
...
bool SnfL1SerialInput(void) {
  if (InfinitekGlobal.serial_in_byte != 0x1B) {
    if (InfinitekGlobal.serial_in_byte_counter >= SONOFF_L1_BUFFER_SIZE) {
      InfinitekGlobal.serial_in_byte_counter = 0;
    }
    if (InfinitekGlobal.serial_in_byte_counter || (!InfinitekGlobal.serial_in_byte_counter && ('A' == InfinitekGlobal.serial_in_byte))) {  // A from AT
      InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter++] = InfinitekGlobal.serial_in_byte;
    }
  } else {
    InfinitekGlobal.serial_in_buffer[InfinitekGlobal.serial_in_byte_counter++] = 0x00;
...
#ifdef SONOFF_L1_DEBUG1
    AddLog(LOG_LEVEL_DEBUG, PSTR("SL1: Rcvd '%s'"), InfinitekGlobal.serial_in_buffer);
#endif
    if (!strncmp(InfinitekGlobal.serial_in_buffer +3, "RESULT", 6)) {
...
/*
      ...
      char *end_str;
      char *string = InfinitekGlobal.serial_in_buffer +10;
...
//      AddLog(LOG_LEVEL_DEBUG, PSTR("Sl1: ++Busy %d"), Snfl1.busy);
    ...
    else if (!strncmp(InfinitekGlobal.serial_in_buffer +3, "UPDATE", 6)) {
      char cmnd_dimmer[20];
      char cmnd_color[20];
      char *end_str;
      char *string = InfinitekGlobal.serial_in_buffer +10;
...
bool SnfL1ModuleSelected(void) {
  if (SONOFF_L1 == InfinitekGlobal.module_type) {
    if (PinUsed(GPIO_RXD) && PinUsed(GPIO_TXD)) {
      ...
      if (Snfl1.buffer) {
        ...
        InfinitekGlobal.light_driver = XLGT_05;
