/*
  xlgt_01_ws2812.ino - led string support for Infinitek
...
// select the right pixel size
  ...
  const uint16_t kTasLed_PixelSize = InfinitekLed_4_WRGB;
#else
  const uint16_t kTasLed_PixelSize = InfinitekLed_3_RGB;
#endif

// select the right pixel order
#if (USE_WS2812_CTYPE == NEO_GRB)
  const uint16_t kTasLed_PixelOrder = InfinitekLed_GRB;
#elif (USE_WS2812_CTYPE == NEO_BRG)
  const uint16_t kTasLed_PixelOrder = InfinitekLed_BRG;
#elif (USE_WS2812_CTYPE == NEO_RBG)
  const uint16_t kTasLed_PixelOrder = InfinitekLed_RBG;
  #elif (USE_WS2812_CTYPE == NEO_RGBW)
  const uint16_t kTasLed_PixelOrder = InfinitekLed_RGB;
...
const uint16_t kTasLed_PixelWhite = InfinitekLed_xxxW;
...
// select timing
  ...
  const uint16_t kTasLed_Timing = InfinitekLed_WS2812;
#elif (USE_WS2812_HARDWARE == NEO_HW_SK6812)
  const uint16_t kTasLed_Timing = InfinitekLed_SK6812;
...
const uint32_t kTasLed_Hardware = InfinitekLed_HW_Default;   // use whatever is available
...
void Ws2812DDP(void)
{
  ...
  if (InfinitekGlobal.restart_flag || InfinitekGlobal.global_state.network_down) return;
...
void Ws2812ShowScheme(void)
{
  ...
  switch (scheme) {
    case 0:  // Clock
      if ((1 == InfinitekGlobal.state_250mS) || (Ws2812.show_next)) {
        Ws2812Clock();
        Ws2812.show_next = 0;
      }
      break;
...
// convert the Settings to a Led type compatible with InfinitekLED
uint16_t Ws2812SettingsToLedType(void) {
  ...
  if (Settings->mbflag2.light_pixels_order != 0) {
    led_type = (led_type & 0xFF00) | (Settings->mbflag2.light_pixels_order << 4)
                                    | (Settings->mbflag2.light_pixels_w_first ? InfinitekLed_Wxxx : 0)
                                    | (Settings->mbflag2.light_pixels_rgbw ? InfinitekLed_4_WRGB : InfinitekLed_3_RGB);
  }
  return led_type;
}
...
bool Ws2812InitStrip(void)
{
  ...
  if (PinUsed(GPIO_WS2812)) {  // RGB led
    ...
    InfinitekLEDPusher * pusher = InfinitekLEDPusher::Create(kTasLed_Hardware, gpio);
    if (pusher == nullptr) {
      AddLog(LOG_LEVEL_ERROR, "LED: No hardware supported");
      return false;
    }
    uint16_t led_type = Ws2812SettingsToLedType();
    strip = new InfinitekLED(led_type, Settings->light_pixels);
...
#endif
    InfinitekGlobal.light_driver = XLGT_01;
  }
}
...
void CmndPixelType(void)
{
  if ((XdrvMailbox.payload >= 0) && (XdrvMailbox.payload < 32)) {
    ...
    if (reboot) {
      InfinitekGlobal.restart_flag = 2;             // force restart if number of channels changed
    }
