/*
  xnrg_05_pzem_ac.ino - PZEM-014,016 Modbus AC energy sensor support for Infinitek
...
void PzemAcEverySecond(void)
{
  ...
  if (data_ready) {
    ...
    if (PzemAc.phase == Energy->phase_count -1) {
      if (InfinitekGlobal.uptime > (PZEM_AC_STABILIZE * ENERGY_MAX_PHASES)) {
        EnergyUpdateTotal();
      }
    }
  }
  ...
  if (0 == PzemAc.send_retry || data_ready) {
    ...
    PzemAc.send_retry--;
    if ((Energy->phase_count > 1) && (0 == PzemAc.send_retry) && (InfinitekGlobal.uptime < (PZEM_AC_STABILIZE * ENERGY_MAX_PHASES))) {
      ...
      if (InfinitekGlobal.discovery_counter) {
        InfinitekGlobal.discovery_counter += ENERGY_WATCHDOG + 1;  // Don't send Discovery yet, delay by 4s + 1s
      }
    }
  }
}
...
{
  PzemAcModbus = new InfinitekModbus(Pin(GPIO_PZEM016_RX), Pin(GPIO_PZEM0XX_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void PzemAcDrvInit(void)
{
  if (PinUsed(GPIO_PZEM016_RX) && PinUsed(GPIO_PZEM0XX_TX)) {
    InfinitekGlobal.energy_driver = XNRG_05;
  }
}
...
bool Xnrg05(uint32_t function)
{
  ...
  switch (function) {
    case FUNC_ENERGY_EVERY_SECOND:
      if (InfinitekGlobal.uptime > 4) { PzemAcEverySecond(); }  // Fix start up issue #5875
      break;
