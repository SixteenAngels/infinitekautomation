/*
  xnrg_04_mcp39f501.ino - MCP39F501 energy sensor support for Infinitek
...
InfinitekSerial *McpSerial = nullptr;
...
void McpSnsInit(void)
{
  ...
  McpSerial = new InfinitekSerial(Pin(GPIO_MCP39F5_RX), Pin(GPIO_MCP39F5_TX), 1);
  if (McpSerial->begin(MCP_BAUDRATE)) {
    if (McpSerial->hardwareSerial()) {
      ClaimSerial();
      mcp_buffer = InfinitekGlobal.serial_in_buffer;  // Use idle serial buffer to save RAM
    } else {
...
void McpDrvInit(void)
{
  if (PinUsed(GPIO_MCP39F5_RX) && PinUsed(GPIO_MCP39F5_TX)) {
    ...
    InfinitekGlobal.energy_driver = XNRG_04;
  }
}
