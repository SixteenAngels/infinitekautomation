/*
  xnrg_30_dummy.ino - Dummy energy sensor support for Infinitek
...
void NrgDummyEverySecond(void) {
  if (Energy->power_on) {  // Powered on
    for (uint32_t channel = 0; channel < Energy->phase_count; channel++) {
      ...
      if (bitRead(InfinitekGlobal.power, channel)) {        // Emulate power read only if device is powered on
...
void NrgDummyDrvInit(void) {
  uint32_t phase_count = (Settings->param[P_DUMMY_RELAYS] > 0) ? Settings->param[P_DUMMY_RELAYS] : InfinitekGlobal.devices_present;  // SetOption48 - (Energy) Support energy dummy relays
  if (InfinitekGlobal.gpio_optiona.dummy_energy && phase_count) {
    ...
    InfinitekGlobal.energy_driver = XNRG_30;
  }
}
