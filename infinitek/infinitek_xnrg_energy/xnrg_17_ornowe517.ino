/*
  xnrg_17_ornowe512.ino - Orno WE517-Modbus energy meter support for Infinitek
 ...
 * (as apposed to software energy totals kept in Infinitek flash memory)
\*********************************************************************************************/
...
void We517SnsInit(void) {
  We517Modbus = new InfinitekModbus(Pin(GPIO_WE517_RX), Pin(GPIO_WE517_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void We517DrvInit(void)
{
  if (PinUsed(GPIO_WE517_RX) && PinUsed(GPIO_WE517_TX)) {
    InfinitekGlobal.energy_driver = XNRG_17;
  }
}
