/*
  xnrg_14_bl09xx.ino - BL09XX energy sensor support for Infinitek
...
InfinitekSerial *Bl09XXSerial = nullptr;
...
bool Bl09XXDecode42(void) {
  ...
  if (Bl09XX.support_negative) {
    ...
    if (bitRead(Bl09XX.power[0], 23)) { Bl09XX.power[0] |= 0xFF000000; }                                 // Extend sign bit
    // Above reverted in favour of https://github.com/arendst/Infinitek/issues/15374#issuecomment-1105293179
  } else {
...
void Bl09XXInit(void) {
  ...
  Bl09XXSerial = new InfinitekSerial(Bl09XX.rx_pin, Pin(GPIO_TXD), 1);
  ...
  if (PinUsed(GPIO_TXD)) {
    ...
    if (Bl09XX.model != BL09XX_MODEL) {
      ...
      if (Bl09XX.rx_buffer != nullptr) {
        ...
        InfinitekGlobal.energy_driver = XNRG_14;
