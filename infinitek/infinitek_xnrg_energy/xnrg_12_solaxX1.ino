/*
  xnrg_12_solaxX1.ino - Solax X1 inverter RS485 support for Infinitek
...
void solaxX1_CyclicTask(void) { // Every 100/250 milliseconds
  ...
  if (solaxX1Serial->available()) {
    ...
    if (solaxX1_global.MeterMode) { // Metermode
      ...
      LastMeterTime = InfinitekGlobal.uptime;
  ...
  if(solaxX1_global.MeterMode) {
    if (InfinitekGlobal.uptime > LastMeterTime + 20) solaxX1_SwitchMeterMode(false); // Switch back to normal mode, when no Meter request is received for 20 sec.
    return;
  }
...
void solaxX1_SnsInit(void) {
  ...
  solaxX1Serial = new InfinitekSerial(Pin(GPIO_SOLAXX1_RX), Pin(GPIO_SOLAXX1_TX), 1, 1, SOLAXX1_BUFFERSIZE);
...
void solaxX1_DrvInit(void) {
  if (PinUsed(GPIO_SOLAXX1_RX) && PinUsed(GPIO_SOLAXX1_TX)) {
    InfinitekGlobal.energy_driver = XNRG_12;
...
void solaxX1_Show(uint32_t function) {
  ...
  switch (function) {
    case FUNC_JSON_APPEND:
      ...
      if (0 == InfinitekGlobal.tele_period && solaxX1.temperature < 100) { DomoticzSensor(DZ_TEMP, solaxX1.temperature); }
#endif // USE_DOMOTICZ
      break;
#ifdef USE_WEBSERVER
    case FUNC_WEB_COL_SENSOR: {
      ...
      if (solaxX1.runMode != -1) LastOnlineTime = InfinitekGlobal.uptime;
      if (InfinitekGlobal.uptime < LastOnlineTime + 300) { // Hide numeric live data, when inverter is offline for more than 5 min
#ifdef SOLAXX1_PV2
