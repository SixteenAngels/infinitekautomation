...
void NrgBerryEverySecond(void) {
  if (InfinitekGlobal.gpio_optiona.berry_energy) {    // active only if OPTION_A 9
...
void NrgBerryDrvInit(void) {
  if (InfinitekGlobal.gpio_optiona.berry_energy) {    // active only if OPTION_A 9
    ...
    InfinitekGlobal.energy_driver = XNRG_28;    // if OPTION_A 9 is set, mark this driver as the active driver, so it can be tested in Berry
  }
}
