...
extra dirs:
  ...
  lib_extra_dirs          = ${env:Infinitek32_base.lib_extra_dirs} lib/libesp32, lib/lib_basic, lib/lib_display, lib/lib_ssl, lib/lib_rf, lib/lib_i2c, lib/lib_div
...
{
  BioPduModbus = new InfinitekModbus(Pin(GPIO_BIOPDU_PZEM016_RX), Pin(GPIO_BIOPDU_PZEM0XX_TX));
...
void BioPduDrvInit(void)
{
  ...
  {
    InfinitekGlobal.energy_driver = XNRG_24;
...
bool Xnrg24(uint32_t function)
{
  ...
  case FUNC_ENERGY_EVERY_SECOND:
    if (InfinitekGlobal.uptime > (BIOPDU_STABILIZE * BIOPDU_MAX_PHASES))
    {
      BioPduEverySecond();
    }
    break;
