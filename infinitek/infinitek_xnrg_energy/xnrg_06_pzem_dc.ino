/*
  xnrg_06_pzem_dc.ino - PZEM-003,017 Modbus DC energy sensor support for Infinitek
...
void PzemDcEverySecond(void)
{
  ...
  if (data_ready) {
    ...
    if (PzemDc->channel == Energy->phase_count -1) {
      if (InfinitekGlobal.uptime > (PZEM_DC_STABILIZE * ENERGY_MAX_PHASES)) {
        EnergyUpdateTotal();
      }
    }
  }
  ...
  if (0 == PzemDc->send_retry || data_ready) {
    ...
    PzemDc->send_retry--;
    if ((Energy->phase_count > 1) && (0 == PzemDc->send_retry) && (InfinitekGlobal.uptime < (PZEM_DC_STABILIZE * ENERGY_MAX_PHASES))) {
...
{
  PzemDc->modbus = new InfinitekModbus(Pin(GPIO_PZEM017_RX), Pin(GPIO_PZEM0XX_TX), Pin(GPIO_NRG_MBS_TX_ENA));
...
void PzemDcDrvInit(void)
{
  if (PinUsed(GPIO_PZEM017_RX) && PinUsed(GPIO_PZEM0XX_TX)) {
    ...
    InfinitekGlobal.energy_driver = XNRG_06;
  }
}
...
bool Xnrg06(uint32_t function)
{
  ...
  switch (function) {
    case FUNC_ENERGY_EVERY_SECOND:
      if (InfinitekGlobal.uptime > 4) { PzemDcEverySecond(); }  // Fix start up issue #5875
      break;
