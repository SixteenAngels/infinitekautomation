/*
  xnrg_19_cse7761.ino - CSE7761 energy sensor support for Infinitek
 ...
 * See https://github.com/arendst/Infinitek/discussions/10793
 ...
 * Inverted inputs                 Yes      No       Current direction defined by hardware design - Fixed by Infinitek
 * Support Zero Cross detection    Yes      No       Infinitek supports zero cross detection only on DualR3 due to timing
...
InfinitekSerial *Cse7761Serial = nullptr;
...
//      Cse7761Write(CSE7761_REG_SYSCON | 0x80, 0x0A00);  // Pow CT
      Cse7761Write(CSE7761_REG_SYSCON | 0x80, 0xFE00);  // Pow CT - Infinitek (enable B)
    } else {      
...
//    Cse7761Write(CSE7761_REG_EMUCON | 0x80, 0x1003);  // Sonoff Dual R3
    Cse7761Write(CSE7761_REG_EMUCON | 0x80, 0x1183);  // Infinitek enable zero cross detection on both positive and negative signal
/*
    ...
    5      WaveEN             Waveform data, instantaneous data output enable signal
                              =1, turn on the waveform data output function (Infinitek add frequency)
    ...
    2      ZxEN               Zero-crossing detection, phase angle, voltage frequency measurement enable signal
                              =1, turn on the zero-crossing detection, phase angle, and voltage frequency measurement functions (Infinitek add frequency)
...
#else
    Cse7761Write(CSE7761_REG_EMUCON2 | 0x80, 0x0FE5);  // Infinitek add Frequency
...
/*
    ...
    1001   Voltage channel zero-crossing signal output (Infinitek add zero-cross detection)
...
void Cse7761SnsInit(void) {
  ...
  Cse7761Serial = new InfinitekSerial(Pin(GPIO_CSE7761_RX, GPIO_ANY), Pin(GPIO_CSE7761_TX), 1);
...
//    Energy->phase_count = 1;                     // Handle one channel (default set by xdrv_03_energy.ino)
    ...
    InfinitekGlobal.energy_driver = XNRG_19;
  }
}
